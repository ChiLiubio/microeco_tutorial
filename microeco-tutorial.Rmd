--- 
title: "Tutorial for R microeco package (v0.20.0)"
...
Creating `trans_alpha` object can return two data.frame with prefix 'data_': `data_alpha` and `data_stat`.
...
d1 <- clone(dataset)
# eliminate the boundary 0 and 1
d1$taxa_abund$Phylum %<>% {. + 1e-10} %>% {./(1 + 2e-10)}
t1 <- trans_diff$new(dataset = d1, taxa_level = "Phylum", method = "glmm", formula = "Group + (1|Type)", 
	family = glmmTMB::beta_family(link = "logit"), filter_thres = 0.001)
View(t1$res_diff)
```
