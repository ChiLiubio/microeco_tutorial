...
mt$cal_alphadiv(PD = FALSE)
# return alpha_diversity in the object
class(mt$alpha_diversity)
```

```{r, echo = TRUE, eval = FALSE}
# save alpha_diversity to a directory
mt$save_alphadiv(dirpath = "alpha_diversity")
```
...
t1 <- trans_diff$new(dataset = mt, method = "metagenomeSeq", group = "Group", taxa_level = "Genus")
...
# 'ALDEx2_t'
t1 <- trans_diff$new(dataset = mt, method = "ALDEx2_t", group = "Group", taxa_level = "Phylum")
t1$plot_diff_abund(use_number = 1:20)
...
t1 <- trans_diff$new(dataset = mt, method = "ancombc2", group = "Group", taxa_level = "Genus", filter_thres = 0.01)
...
t1 <- trans_env$new(dataset = mt, add_data = env_data_16S[, 4:11])
```
...
# now mt$sample_table has the whole data
mt
```
...
```{r, echo = TRUE}
t1 <- trans_env$new(dataset = mt, env_cols = 8:15)
```
...
t2 <- trans_diff$new(dataset = mt, method = "rf", group = "Group", taxa_level = "Genus")
...
t1$cal_cor(add_abund_table = mt$alpha_diversity)
...
t1$plot_scatterfit(
	x = "pH", 
	y = mt$beta_diversity$bray[rownames(t1$data_env), rownames(t1$data_env)], 
	type = "cor",
...
t1$plot_scatterfit(
    x = mt$beta_diversity$bray[rownames(t1$data_env), rownames(t1$data_env)],
	y = "pH",
	type = "lm", 
	group = "Group", 
...
tmp <- unlist(mt$taxa_abund$Genus["k__Bacteria|p__Actinobacteria|c__Actinobacteria|o__Micrococcales|f__Micrococcaceae|g__Arthrobacter", ])
...
d2 <- t1$norm(method = "CSS")
# Total sum scaling, dividing counts by the sequencing depth
d2 <- t1$norm(method = "TSS")
# rarefaction
d2 <- t1$norm(method = "rarefy")
d2 <- t1$norm(method = "SRS", sample.size = 500)
