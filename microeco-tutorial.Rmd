--- 
title: "Tutorial for R microeco package (v0.6.0)"
...
Besides, we also develop the file2meco package (https://github.com/ChiLiubio/file2meco) for the data input from some tools easily
...
If you are interested in the class feature, read more from 'Advanced R' book (https://adv-r.hadley.nz/).
...
2. install the packages in advance. We recommend this solution if you are interest in most of the methods in the microeco package and want to run all the examples in tutorial.
...
packages <- c("MASS", "GUniFrac", "ggpubr", "randomForest", "ggdendro", "ggrepel", "agricolae", "gridExtra", "picante", "pheatmap", "igraph", "rgexf", "ggalluvial")
...
The only thing need to do is to download the blast tool (**ignore this if the blast tool is in the path**) and Ref99NR/Ref100NR database (select one).
...
## trans_abund class
　The trans_abund class is used to transform taxonomic abundance data for plotting taxa abundance with the ggplot2 package.
...
　The trans_beta class is developed for the beta diversity analysis.
When needed, the beta_diversity in microtable table will be invoked for transformation and ploting using trans_beta class.
...
### Key points
  ...
  + plot_group_distance(): pair_compare_filter_select parameter in plot_group_distance can be used to select the significance label shown in the plot
...
# provide the file path of SILVA123
...
t1$cal_tax4fun2(blast_tool_path = "ncbi-blast-2.5.0+/bin", path_to_reference_data = "test_ReferenceData_v2",
...
### Key points
  ...
  + blast_tool_path parameter in cal_tax4fun2: if the blast tool has been in 'environment variable' of computer, use blast_tool_path = NULL
...
data1$tidy_dataset()
# revise the species names in tax_table
...
<!--chapter:end:10-Other_examples.Rmd-->
