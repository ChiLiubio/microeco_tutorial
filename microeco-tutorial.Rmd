--- 
title: "Tutorial for R microeco package (v0.7.5)"
...
# add the rownames of tax_table as the last column of tax_table directly; very useful in some analysis, e.g. OTU/ASV biomarker finding.
ncol(test$tax_table)
...
## trans_abund class
　The trans_abund class is used to transform taxonomic abundance data to plots with the ggplot2 package.
...
Dunn's Kruskal-Wallis Multiple Comparisons for cases with groups > 2 are also available.
...
If the user only want to get the taxa with significant different abundance, both the method parameter "KW" and "KW_dunn" are available.
...
The method = "KW_dunn" represents Dunn's Kruskal-Wallis Multiple Comparisons for cases with groups > 2.
...
```{r, echo = FALSE}
pander::pander(network_node_table[1:5, c(1:4)])
```
...
t1$subset_network(node = t1$res_node_table %>% .[.$module == "M1", ] %>% rownames, rm_single = TRUE)
...
t1$plot_network(method = "ggraph", node_color = "Phylum")
...
We also introduce the third network construction approach: julia package FlashWeave[@Tackmann_Rapid_2019].
...
The fourth method that implemented in the class is beemStatic package [@Li_BEEMStatic_2021].
...
# calculate modules for the directed network
...
### Other functions
  ...
  + plot_network(): plot the network in R with igraph or ggraph package for static network, and networkD3 package for dynamic network.
...
### Key points
  ...
  + cal_rcbray(): if you just need rcbray, ignore other phylogenetic operations
 
### Other function

  + cal_Cscore(): calculates the (normalised) mean number of checkerboard combinations (C-score) using C.score
  + cal_NRI(): calculates Nearest Relative Index (NRI)
  + cal_NTI(): calculates Nearest Taxon Index (NTI)
...
```{r, echo = FALSE}
t1$cal_mantel(use_measure = "bray")
pander::pander(t1$res_mantel[, -2:3])
```
...
### Key points
  ...
  + plot_ordination(): use the parameters (e.g., env_nudge_x and taxa_nudge_x ) to adjust the text positions.
...
t2$cal_spe_func_perc(use_community = FALSE, node_type_table = network_node_table)
...
library(file2meco)
?qiime2meco
# use data files inside the package which were downloaded from (https://docs.qiime2.org/2022.2/tutorials/moving-pictures-usage/).
abund_file_path <- system.file("extdata", "dada2_table.qza", package="file2meco")
sample_file_path <- system.file("extdata", "sample-metadata.tsv", package="file2meco")
taxonomy_file_path <- system.file("extdata", "taxonomy.qza", package="file2meco")
# construct microtable object
...
t1$get_node_table()
node_type_table <- t1$res_node_table
