--- 
title: "Tutorial for R microeco package (v0.8.0)"
...
as we show in the figure. An example is the function `dataset$cal_alphadiv()` and its return result `dataset$alpha_diversity`.
...
packages <- c("MASS", "GUniFrac", "ggpubr", "randomForest", "ggdendro", "ggrepel", "agricolae", "picante", "pheatmap", "igraph", "rgexf", "ggalluvial", "ggh4x", "FSA", "gridExtra")
...
# also see the help document of microtable class about the rep_fasta parameter in microtable$new().
```
...
t1$plot_bar(use_alluvium = TRUE, clustering = TRUE, xtext_type_hor = FALSE, xtext_size = 6, color_values = RColorBrewer::brewer.pal(8, "Set2"))
```
...
From v0.8.0, the plot_alpha function add the significance label by searching the results in **object$res_diff** instead of calculating the significance again.
...
From v0.8.0, all the differential test result is stored in the object$res_diff.
...
# we show 20 taxa with the highest LDA
...
For other details of methods, please see the document with `help(trans_diff)`.
...
```{r, echo = TRUE, eval=FALSE}
# The parameter cal_cor in trans_network is used for selecting the correlation calculation method.
...
```{r, echo = TRUE, eval=FALSE}
# When the OTU number is large, using cal_cor = "WGCNA" will be faster
...
Furthermore, COR_optimization = TRUE can be used to find the optimized coefficient threshold instead of the COR_cut based on the RMT theory [@Deng_Molecular_2012].
...
```{r, echo = TRUE, eval=FALSE}
# add modules for this undirected network
...
We also add the function `plot_network` from v0.7.5 to directly plot the network in R, including the static network and dynamic network.
...
From v0.8.0, the trans_comm function can be used to transform the node classification to a new microtable object for other analysis.

```{r, echo = TRUE, eval = FALSE}
# use_col is used to select a column of t1$res_node_table
microtable_module <- t1$trans_comm(use_col = "module", abundance = TRUE)
microtable_module
```
...
See https://chiliubio.github.io/microeco_tutorial/intro.html#spieceasi for the package installation.
...
The third network construction approach: julia package FlashWeave[@Tackmann_Rapid_2019].
...
```{r, echo = TRUE, eval=FALSE}
# cal_cor select NA
t1 <- trans_network$new(dataset = dataset, cal_cor = NA, taxa_level = "OTU", filter_thres = 0)
# require Julia in the computer path, and the package FlashWeave
t1$cal_network(network_method = "FlashWeave")
# see t1$res_network
```

The fourth method implemented: beemStatic package [@Li_BEEMStatic_2021].
...
```{r, echo = TRUE, eval=FALSE}
t1 <- trans_network$new(dataset = dataset, cal_cor = NA, taxa_level = "OTU", filter_thres = 0.001)
t1$cal_network(network_method = "beemStatic")
# modules for the directed network
...
```{r, echo = FALSE}
t1$cal_diff(group = "Group", method = "wilcox")
pander::pander(t1$res_diff[, c(1, 2, 4, 6, 7)])
```
...
The functions of file2meco package are mainly developed based on a study involved in complex metagenomic analysis [@Liu_Microbial_2022].
...
```{r, echo = TRUE, eval = FALSE}
# see https://github.com/ChiLiubio/file2meco for file2meco installation
library(file2meco)
...
test <- mpa2meco(abund_table = abund_file_path, sample_data = sample_file_path, match_table = match_file_path)
test$tidy_dataset()
```
...
<!--chapter:end:11-Other_examples.Rmd-->
