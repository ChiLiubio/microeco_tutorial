...
\hypersetup{
  pdftitle={Tutorial for R microeco package (v0.7.5)},
...
\title{Tutorial for R microeco package (v0.7.5)}
...
\CommentTok{\# add the rownames of tax\_table as the last column of tax\_table directly; very useful in some analysis, e.g. OTU/ASV biomarker finding.}
...
\item
  \texttt{tidy\_dataset()}: necessary to clean files in microtable object
\item
...
\begin{verbatim}
## The names is stored in object$res_names ...
\end{verbatim}

\begin{verbatim}
## The details of each venn part is stored in object$res_content ...
\end{verbatim}

\begin{verbatim}
## The summary table used for plot is stored in object$res_summary_table ...
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# transform venn results to the sample{-}species table, here do not consider abundance, only use presence/absence.}
\NormalTok{t2 }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{trans\_venn\_com}\NormalTok{(}\AttributeTok{use\_frequency =} \ConstantTok{TRUE}\NormalTok{)}
...
Then, we test the differences among groups using Kruskal-Wallis Rank Sum Test (for groups \textgreater{} 2), Wilcoxon Rank Sum Tests (for groups = 2),
Dunn's Kruskal-Wallis Multiple Comparisons (for cases with groups \textgreater{} 2) and anova with multiple comparisons.
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.14}}@{}}
\toprule
Groups & Measure & Test\_method & p.value & p.adjust \\
\midrule
\endhead
IW vs CW & Observed & Wilcoxon Rank Sum Test & 0.03778 & 0.1511 \\
IW vs TW & Observed & Wilcoxon Rank Sum Test & 0.4598 & 0.6471 \\
CW vs TW & Observed & Wilcoxon Rank Sum Test & 0.3953 & 0.6471 \\
IW vs CW vs TW & Observed & Kruskal-Wallis Rank Sum Test & 0.155 & 0.3817 \\
IW vs CW & Chao1 & Wilcoxon Rank Sum Test & 0.002342 & 0.03075 \\
\bottomrule
\end{longtable}
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.15}}@{}}
\toprule
Measure & Comparison & Z & P.unadj & P.adj \\
\midrule
\endhead
Observed & CW - IW & -1.93 & 0.05364 & 0.1609 \\
Observed & CW - TW & -1.02 & 0.3075 & 0.615 \\
Observed & IW - TW & 0.9093 & 0.3632 & 0.3632 \\
Chao1 & CW - IW & -2.841 & 0.004491 & 0.01347 \\
Chao1 & CW - TW & -1.176 & 0.2395 & 0.2395 \\
\bottomrule
\end{longtable}
...
## Groups     2 0.04131 0.0206545 4.1682    999  0.028 *
...
In addition, Kruskal-Wallis Rank Sum Test (for groups \textgreater{} 2), Wilcoxon Rank Sum Tests (for groups = 2) and
Dunn's Kruskal-Wallis Multiple Comparisons for cases with groups \textgreater{} 2 are also available.
...
If the user only want to get the taxa with significant different abundance, both the method parameter ``KW'' and ``KW\_dunn'' are available.
The method = ``KW'' represents Kruskal-Wallis Rank Sum Test (for groups \textgreater{} 2) and Wilcoxon Rank Sum Tests (for groups = 2)
for a specific taxonomic level or all levels of microtable\$taxa\_abund.
The method = ``KW\_dunn'' represents Dunn's Kruskal-Wallis Multiple Comparisons for cases with groups \textgreater{} 2.
...
\CommentTok{\# "KW\_dunn" method depends on the FSA package}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ d1, }\AttributeTok{method =} \StringTok{"KW\_dunn"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"Genus"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
...
\item
  trans\_diff\$plot\_lefse\_cladogram: clade\_label\_size, clade\_label\_size\_add and clade\_label\_size\_log are used to control the text size in the cladogram
\end{itemize}
...
This is only intended to show some operations conveniently. Does not mean we are suggesting this approach in any case.
Please check the final part for other network construction methods.
The parameter cal\_cor in trans\_network is used for selecting the correlation calculation method.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# classify the node; return t1$res\_node\_table }
...
\NormalTok{network\_node\_table }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\NormalTok{res\_node\_table }
\end{Highlighting}
\end{Shaded}
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.19}}@{}}
\toprule
~ & name & degree & betweenness & Abundance \\
\midrule
\endhead
\textbf{OTU\_50} & OTU\_50 & 2 & 0 & 0.2301 \\
\textbf{OTU\_305} & OTU\_305 & 3 & 4 & 0.1394 \\
\textbf{OTU\_1} & OTU\_1 & 21 & 2 & 1.409 \\
\textbf{OTU\_41} & OTU\_41 & 1 & 0 & 0.1603 \\
\textbf{OTU\_59} & OTU\_59 & 3 & 2306 & 0.5588 \\
\bottomrule
\end{longtable}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_network}\NormalTok{(}\AttributeTok{method =} \StringTok{"ggraph"}\NormalTok{, }\AttributeTok{node\_color =} \StringTok{"Phylum"}\NormalTok{)}
...
We also introduce the third network construction approach: julia package FlashWeave\citep{Tackmann_Rapid_2019}.
...
The fourth method that implemented in the class is beemStatic package \citep{Li_BEEMStatic_2021}.
...
The result from this approach is a directed network.
...
\CommentTok{\# calculate modules for the directed network}
...
\item
  plot\_network(): plot the network in R with igraph or ggraph package for static network, and networkD3 package for dynamic network.
\end{itemize}
...
\item
  cal\_NRI(): calculates Nearest Relative Index (NRI)
\item
  cal\_NTI(): calculates Nearest Taxon Index (NTI)
\end{itemize}
...
Quitting from lines 1857-1859 (microeco-tutorial.Rmd)
Error in .subset(x, j) : 只有负下标里才能有零
...
\item
  plot\_ordination(): use the parameters (e.g., env\_nudge\_x and taxa\_nudge\_x ) to adjust the text positions.
\end{itemize}
...
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{cal\_spe\_func\_perc}\NormalTok{(}\AttributeTok{use\_community =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{node\_type\_table =}\NormalTok{ network\_node\_table)}
...
\CommentTok{\# use data files inside the package which were downloaded from (https://docs.qiime2.org/2022.2/tutorials/moving{-}pictures{-}usage/).}
\NormalTok{abund\_file\_path }\OtherTok{\textless{}{-}} \FunctionTok{system.file}\NormalTok{(}\StringTok{"extdata"}\NormalTok{, }\StringTok{"dada2\_table.qza"}\NormalTok{, }\AttributeTok{package=}\StringTok{"file2meco"}\NormalTok{)}
\NormalTok{sample\_file\_path }\OtherTok{\textless{}{-}} \FunctionTok{system.file}\NormalTok{(}\StringTok{"extdata"}\NormalTok{, }\StringTok{"sample{-}metadata.tsv"}\NormalTok{, }\AttributeTok{package=}\StringTok{"file2meco"}\NormalTok{)}
\NormalTok{taxonomy\_file\_path }\OtherTok{\textless{}{-}} \FunctionTok{system.file}\NormalTok{(}\StringTok{"extdata"}\NormalTok{, }\StringTok{"taxonomy.qza"}\NormalTok{, }\AttributeTok{package=}\StringTok{"file2meco"}\NormalTok{)}
\CommentTok{\# construct microtable object}
...
\NormalTok{test1}\SpecialCharTok{$}\FunctionTok{plot\_lefse\_bar}\NormalTok{(}\AttributeTok{use\_number =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{20}\NormalTok{)}
...
\NormalTok{test1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(test, }\AttributeTok{method =} \StringTok{"lefse"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{)}
\NormalTok{test1}\SpecialCharTok{$}\FunctionTok{plot\_lefse\_bar}\NormalTok{(}\AttributeTok{LDA\_score =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{node\_type\_table }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\NormalTok{res\_node\_table}
...
\includegraphics{microeco-tutorial_files/figure-latex/unnamed-chunk-242-1.pdf}
...
\includegraphics{microeco-tutorial_files/figure-latex/unnamed-chunk-243-1.pdf}
