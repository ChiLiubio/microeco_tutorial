...
\hypersetup{
  pdftitle={Tutorial for R microeco package (v0.14.1)},
...
\title{Tutorial for R microeco package (v0.14.1)}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
...
\item
  Install the packages in advance.
...
\hline
pheatmap & plot\_cor(pheatmap = TRUE) & correlation heatmap with clustering dendrogram\\
\hline
...
\hypertarget{filter-features}{%
\subsection{filter features}\label{filter-features}}

Please use filter\_taxa function to filter the features with low abundance or occurrence frequency.
...
\CommentTok{\# mean relative abundance threshold 0.0001}
...
\begin{verbatim}
## 10647 features filtered based on the abundance ...
\end{verbatim}

\begin{verbatim}
## freq smaller than 1; first convert it to an integer ...
\end{verbatim}

\begin{verbatim}
## Use converted freq integer: 9 for the following filtering ...
\end{verbatim}

\begin{verbatim}
## 6309 features filtered based on the occurrence...
\end{verbatim}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_bar}\NormalTok{(}\AttributeTok{bar\_type =} \StringTok{"notfull"}\NormalTok{, }\AttributeTok{use\_alluvium =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{clustering =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{xtext\_type\_hor =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{xtext\_size =} \DecValTok{6}\NormalTok{, }\AttributeTok{color\_values =}\NormalTok{ RColorBrewer}\SpecialCharTok{::}\FunctionTok{brewer.pal}\NormalTok{(}\DecValTok{8}\NormalTok{, }\StringTok{"Set2"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
Another way to plot the results is to use plot\_bar function, which is especially useful for a large number of samples/groups.
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_bar}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1169}}
  >{\centering\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1429}}
  >{\centering\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1299}}
  >{\centering\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1948}}@{}}
\toprule()
\begin{minipage}[b]{\linewidth}\centering
Measure
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 16\tabcolsep) * \real{0.1279}}
  >{\centering\arraybackslash}p{(\columnwidth - 16\tabcolsep) * \real{0.0930}}
  >{\centering\arraybackslash}p{(\columnwidth - 16\tabcolsep) * \real{0.0698}}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{, }\AttributeTok{order\_x\_mean =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{add\_sig\_text\_size =} \DecValTok{6}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{, }\AttributeTok{xtext\_size =} \DecValTok{15}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\item
  plot\_group\_distance(): manipulating object\$res\_group\_distance\_diff can control what statistical results are presented in the plot.
\end{itemize}

\hypertarget{model-based-class}{%
\chapter{Model-based class}\label{model-based-class}}

We group all the classes with complex models into the this section `Model-based class'.
...
It can find the significant taxa in determining the community differences across groups.
...
The method `ALDEx2\_t' and `ALDEx2\_kw' depend on the ALDEx2 package.
...
\CommentTok{\# please first install metagenomeSeq package}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# \textquotesingle{}ALDEx2\_t\textquotesingle{} and \textquotesingle{}ALDEx2\_kw\textquotesingle{} methods; use ?trans\_diff to see the description of the methods}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{method =} \StringTok{"ANCOMBC"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"Family"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{use\_number =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{20}\NormalTok{, }\AttributeTok{select\_group =} \StringTok{"CW {-} TW"}\NormalTok{)}
...
\item
  ...
  This is useful in cases where very few significant taxa are found (generally no significant taxa found after adjustment) and
  where the results need to be compared with that from Galaxy server or other LEfSe python version.
\item
  trans\_diff\(new: this class has a strict requirement on the taxonomic information, make sure `tidy_taxonomy()` function has been performed for the dataset\)tax\_table
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# The parameter cor\_method in trans\_network is used for selecting the correlation calculation method.}
...
We use Spearman correlation based on WGCNA package (use\_WGCNA\_pearson\_spearman = TRUE) to show all the following operations.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# require WGCNA package; installation command: install.packages("WGCNA", repos = BiocManager::repositories())}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# \# modularity for this undirected network with igraph cluster\_fast\_greedy function}
...
\CommentTok{\# require chorddiag package; see https://github.com/mattflor/chorddiag}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_sum\_links}\NormalTok{(}\AttributeTok{plot\_pos =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{plot\_num =} \DecValTok{10}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# we use cluster\_optimal method for the directed network}
...
please see the meconetcomp package part (\url{https://chiliubio.github.io/microeco_tutorial/meconetcomp-package.html}).
...
\NormalTok{g1 }\OtherTok{\textless{}{-}}\NormalTok{ t2}\SpecialCharTok{$}\FunctionTok{plot\_group\_distance}\NormalTok{()}
...
\begin{verbatim}
## Now randomizing by parallel computing. Begin at Sat Mar 11 13:55:41 2023. Please wait...
\end{verbatim}
...
\midrule()
\endhead
IW & 435 & 0.654 & 0.3875 & 0.364 & 9.129 \\
CW & 435 & 0.5632 & 0.2965 & 0.2867 & 14.37 \\
TW & 435 & 0.6244 & 0.416 & 0.3998 & 9.874 \\
\bottomrule()
\end{longtable}
...
If so, please add \texttt{complete\_na\ =\ TRUE} for interpolation when creating the trans\_env object.
...
The cal\_diff function is used to test the significance of variables across groups like we have shown in trans\_alpha and trans\_diff class parts.
...
\begin{center}\includegraphics[width=750px]{Images/plot_env_diff_all} \end{center}

From the v0.12.0, the trans\_env class supports the differential test of groups within each group by using the by\_group parameter in cal\_diff function.
...
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.1358}}
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.2593}}
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.3210}}
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.1235}}
...
The default \texttt{p\_adjust\_type\ =\ "Env"} means p adjustment is performed for each environmental variable separately.
If the user needs to adjust p values for all the results together, please use \texttt{p\_adjust\_type\ =\ "Type"}.
...
\includegraphics{microeco-tutorial_files/figure-latex/unnamed-chunk-289-1.pdf}
...
\includegraphics{microeco-tutorial_files/figure-latex/unnamed-chunk-290-1.pdf}
...
The class trans\_norm provide several data normalization approaches for the microtable object.
...
\CommentTok{\# Cumulative sum scaling normalization. Require metagenomeSeq package to be installed.}
...
It can be conducted with the betareg function in the betareg package \citep{Betaregression_2010} and has been implemented in the mecoturn package.
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ taxaturn}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(wheat\_16S, }\AttributeTok{taxa\_level =} \StringTok{"Phylum"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Type"}\NormalTok{, }\AttributeTok{ordered\_group =} \FunctionTok{c}\NormalTok{(}\StringTok{"S"}\NormalTok{, }\StringTok{"RS"}\NormalTok{, }\StringTok{"R"}\NormalTok{), }\AttributeTok{by\_ID =} \StringTok{"Plant\_ID"}\NormalTok{, }\AttributeTok{by\_group =} \StringTok{"Region"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
\CommentTok{\# measure can designate a taxon}
...
\CommentTok{\# require ggcor package (https://github.com/mj163163/ggcor{-}1)}
\FunctionTok{library}\NormalTok{(ggplot2)}
\FunctionTok{library}\NormalTok{(ggcor)}
\FunctionTok{set\_scale}\NormalTok{()}

\NormalTok{g1 }\OtherTok{\textless{}{-}} \FunctionTok{quickcor}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{data\_env, }\AttributeTok{type =} \StringTok{"upper"}\NormalTok{) }\SpecialCharTok{+}
