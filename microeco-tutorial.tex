...
\hypersetup{
  pdftitle={Tutorial for R microeco package (v2.0.0)},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Tutorial for R microeco package (v2.0.0)}
\author{}
\date{\vspace{-2.5em}2026-03-21}
...
Based on this background, we created the R microeco package \citep{Liu_microeco_2021, Liu_workflow_2025}.
...
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.4235}}
...
\hypertarget{add-asvotu-to-tax_table}{%
...
\hypertarget{filter-the-features-with-low-abundanceoccurrence-frequency}{%
...
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.1923}}
  >{\centering\arraybackslash}p{(\columnwidth - 12\tabcolsep) * \real{0.0641}}
...
\NormalTok{sub1 }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{subset\_network}\NormalTok{(}\AttributeTok{node =}\NormalTok{ mt}\SpecialCharTok{$}\NormalTok{otu\_table }\SpecialCharTok{\%\textgreater{}\%}\NormalTok{ .[.[, }\StringTok{"S1"}\NormalTok{] }\SpecialCharTok{!=} \DecValTok{0}\NormalTok{, ] }\SpecialCharTok{\%\textgreater{}\%}\NormalTok{ rownames, }\AttributeTok{rm\_single =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{return\_igraph =} \ConstantTok{FALSE}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# extract all sub{-}networks for each sample}
...
\begin{verbatim}
## Now randomizing by parallel computing. Begin at Sat Mar 21 21:51:48 2026. Please wait...
\end{verbatim}
...
\midrule()
\endhead
IW & 435 & 0.5548 & 0.2908 & 0.3005 & 20.02 \\
CW & 435 & 0.5886 & 0.3143 & 0.3178 & 24.46 \\
TW & 435 & 0.5822 & 0.3601 & 0.3667 & 17.3 \\
\bottomrule()
\end{longtable}
...
\hypertarget{visualize-taxonomic-abundance-with-trans_alpha-class}{%
...
\hypertarget{customized-network-input-and-analysis}{%
