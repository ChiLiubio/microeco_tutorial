...
\hypersetup{
  pdftitle={Tutorial for R microeco package (v0.9.0)},
...
\title{Tutorial for R microeco package (v0.9.0)}
...
The main goal of developing this package is to help users analyse microbial communtiy data fast.
...
This is a rough framework for users to fast understand the design of the microeco package.
...
\NormalTok{packages }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"MASS"}\NormalTok{, }\StringTok{"GUniFrac"}\NormalTok{, }\StringTok{"ggpubr"}\NormalTok{, }\StringTok{"randomForest"}\NormalTok{, }\StringTok{"ggdendro"}\NormalTok{, }\StringTok{"ggrepel"}\NormalTok{, }\StringTok{"agricolae"}\NormalTok{, }\StringTok{"picante"}\NormalTok{, }\StringTok{"pheatmap"}\NormalTok{, }\StringTok{"igraph"}\NormalTok{, }\StringTok{"rgexf"}\NormalTok{, }\StringTok{"ggalluvial"}\NormalTok{, }\StringTok{"ggh4x"}\NormalTok{, }\StringTok{"FSA"}\NormalTok{, }\StringTok{"gridExtra"}\NormalTok{)}
...
the correlation algorithm in WGCNA is very fast.
...
\CommentTok{\# also see the help document of microtable class about the rep\_fasta parameter in microtable$new().}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# metadata table}
\FunctionTok{data}\NormalTok{(sample\_info\_16S)}
\CommentTok{\# feature table}
\FunctionTok{data}\NormalTok{(otu\_table\_16S)}
\CommentTok{\# taxonomic assignment table}
...
\CommentTok{\# add the rownames of tax\_table as the last column of tax\_table directly; useful in some analysis, e.g. biomarker finding at OTU/ASV level.}
...
\CommentTok{\# sum the abundance for each taxa; very useful for taxa abundance checking and filtering}
\NormalTok{test}\SpecialCharTok{$}\FunctionTok{taxa\_sums}\NormalTok{()}

\CommentTok{\# merge samples according to a specific group of sample\_table to generate a new microtable object}
\NormalTok{test1 }\OtherTok{\textless{}{-}}\NormalTok{ test}\SpecialCharTok{$}\FunctionTok{merge\_samples}\NormalTok{(}\AttributeTok{use\_group =} \StringTok{"Group"}\NormalTok{)}
\NormalTok{test1}

\CommentTok{\# merge taxa according to a specific taxonomic rank level of tax\_table to generate a new microtable object}
\NormalTok{test1 }\OtherTok{\textless{}{-}}\NormalTok{ test}\SpecialCharTok{$}\FunctionTok{merge\_taxa}\NormalTok{(}\AttributeTok{taxa =} \StringTok{"Genus"}\NormalTok{)}
\NormalTok{test1}

\CommentTok{\# rename feature names in all the files of microtable object}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{taxrank =} \StringTok{"Genus"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{5}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_line}\NormalTok{()}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{taxrank =} \StringTok{"Genus"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{5}\NormalTok{, }\AttributeTok{group =} \StringTok{"Type"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_line}\NormalTok{(}\AttributeTok{position =} \FunctionTok{position\_dodge}\NormalTok{(}\FloatTok{0.3}\NormalTok{), }\AttributeTok{xtext\_type\_hor =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=750px]{Images/plot_line_type} \end{center}
...
\NormalTok{t3}\SpecialCharTok{$}\FunctionTok{plot\_pie}\NormalTok{(}\AttributeTok{facet\_nrow =} \DecValTok{3}\NormalTok{, }\AttributeTok{color\_values =} \FunctionTok{rev}\NormalTok{(}\FunctionTok{c}\NormalTok{(RColorBrewer}\SpecialCharTok{::}\FunctionTok{brewer.pal}\NormalTok{(}\DecValTok{8}\NormalTok{, }\StringTok{"Dark2"}\NormalTok{), }\StringTok{"grey50"}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{data\_stat[}\DecValTok{1}\SpecialCharTok{:}\DecValTok{5}\NormalTok{, ]}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{res\_diff[}\DecValTok{1}\SpecialCharTok{:}\DecValTok{5}\NormalTok{, ]}
\end{Highlighting}
\end{Shaded}
...
\toprule
Measure & Group & Comparison & Z & P.unadj & P.adj & Significance \\
\midrule
\endhead
...
\CommentTok{\# now the result t1$res\_diff is a list}
\CommentTok{\# see the help document for the usage of anova\_set}
\end{Highlighting}
\end{Shaded}

From v0.8.0, the plot\_alpha function add the significance label by searching the results in \textbf{object\$res\_diff} instead of calculating the significance again.
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{, }\AttributeTok{order\_x\_mean =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{g1 }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{, }\AttributeTok{xtext\_size =} \DecValTok{15}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# measure parameter can invoke the distance matrix in dataset$beta\_diversity}
...
Try other intresting options in the plotting.
...
With the field development, there are also some wrapped methods to better capture the important biomarkers,
...
LEfSe and RF in this class are both the wrapped method (i.e.~the combination of machine-learning and differential abundance test)
and are mainly used for the identification of biomarkers at all taxonomic levels.
The metagenomeSeq method is implemented to find significant feature between two groups at feature level (OTU/ASV) directly.

\hypertarget{example-5}{%
\subsection{Example}\label{example-5}}

From v0.8.0, all the differential test result is stored in the object\$res\_diff.
...
\CommentTok{\# we show 20 taxa with the highest LDA}
...
\begin{center}\includegraphics[width=800px]{Images/plot_rf_diff_abund} \end{center}

For v0.9.0, the significance label can also be added in the abundance plot controlled by add\_sig parameter and other related parameters.
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{use\_number =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{20}\NormalTok{, }\AttributeTok{add\_sig =}\NormalTok{ T, }\AttributeTok{add\_sig\_label =} \StringTok{"Significance"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=700px]{Images/plot_diff_abund_siglabel_wilcox} \end{center}
...
\begin{center}\includegraphics[width=700px]{Images/plot_diff_abund_siglabel_anova} \end{center}

For other details of methods, please see the document with \texttt{help(trans\_diff)}.
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{use\_number =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{20}\NormalTok{, }\AttributeTok{select\_group =} \StringTok{"CW {-} IW"}\NormalTok{, }\AttributeTok{group\_order =} \FunctionTok{c}\NormalTok{(}\StringTok{"CW"}\NormalTok{, }\StringTok{"IW"}\NormalTok{))}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{method =} \StringTok{"mseq"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  trans\_diff\(new: In trans_diff\)new, p\_adjust\_method = NULL can shut down the p value adjustment for method != ``metastat'' or ``anova''.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# The parameter cal\_cor in trans\_network is used for selecting the correlation calculation method.}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{cal\_cor =} \StringTok{"SparCC"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{,  }\AttributeTok{nThreads =} \DecValTok{4}\NormalTok{, }\AttributeTok{SparCC\_simu\_num =} \DecValTok{100}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# When the OTU number is large, using cal\_cor = "WGCNA" will be faster}
...
Furthermore, COR\_optimization = TRUE can be used to find the optimized coefficient threshold instead of the COR\_cut based on the RMT theory \citep{Deng_Molecular_2012}.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# add modules for this undirected network}
...
\CommentTok{\# open network.gexf file using Gephi(https://gephi.org/)}
...
We also add the function \texttt{plot\_network} from v0.7.5 to directly plot the network in R, including the static network and dynamic network.
...
\CommentTok{\# use ggraph package method}
...
From v0.8.0, the trans\_comm function can be used to transform the node classification to a new microtable object for other analysis.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# use\_col is used to select a column of t1$res\_node\_table}
\NormalTok{microtable\_module }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{trans\_comm}\NormalTok{(}\AttributeTok{use\_col =} \StringTok{"module"}\NormalTok{, }\AttributeTok{abundance =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{microtable\_module}
\end{Highlighting}
\end{Shaded}
...
The third network construction approach: julia package FlashWeave\citep{Tackmann_Rapid_2019}.
This is a probabilistic graph-based method to obtain the conditional independence.
It predicts ecological interactions among microbes (direct association) from large-scale compositional abundance data through statistical co-occurrence.
...
\begin{Highlighting}[]
\CommentTok{\# cal\_cor select NA}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{cal\_cor =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{filter\_thres =} \DecValTok{0}\NormalTok{)}
\CommentTok{\# require Julia in the computer path, and the package FlashWeave}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_network}\NormalTok{(}\AttributeTok{network\_method =} \StringTok{"FlashWeave"}\NormalTok{)}
\CommentTok{\# see t1$res\_network}
\end{Highlighting}
\end{Shaded}

The fourth method implemented: beemStatic package \citep{Li_BEEMStatic_2021}.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{cal\_cor =} \ConstantTok{NA}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
...
\CommentTok{\# method for the directed network}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_module}\NormalTok{(}\AttributeTok{method =} \StringTok{"cluster\_optimal"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_tNST}\NormalTok{(}\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{dist.method =} \StringTok{"bray"}\NormalTok{, }\AttributeTok{abundance.weighted =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{output.rand =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{SES =} \ConstantTok{TRUE}\NormalTok{)}
\CommentTok{\# result: t1$res\_tNST}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{res\_tNST}\SpecialCharTok{$}\NormalTok{index.grp}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
## Now randomizing by parallel computing. Begin at Tue May 24 11:28:13 2022. Please wait...
\end{verbatim}
...
\midrule
\endhead
IW & 435 & 0.6539 & 0.386 & 0.364 \\
CW & 435 & 0.5632 & 0.2956 & 0.2867 \\
TW & 435 & 0.6245 & 0.4152 & 0.3999 \\
\bottomrule
\end{longtable}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_tNST\_test}\NormalTok{(}\AttributeTok{method =} \StringTok{"nst.boot"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{d1 }\OtherTok{\textless{}{-}} \FunctionTok{qiime2meco}\NormalTok{(}\AttributeTok{ASV\_data =}\NormalTok{ abund\_file\_path, }\AttributeTok{sample\_data =}\NormalTok{ sample\_file\_path, }\AttributeTok{taxonomy\_data =}\NormalTok{ taxonomy\_file\_path)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# use Wilcoxon Rank Sum and Signed Rank Tests as an example}
...
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{res\_diff[, }\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{4}\NormalTok{, }\DecValTok{6}\NormalTok{, }\DecValTok{7}\NormalTok{)]}
\end{Highlighting}
\end{Shaded}
...
IW - TW & TOC & TW & 0.643 & ns \\
CW - TW & TOC & TW & 0.06771 & ns \\
IW - CW & NH4 & CW & 0.1842 & ns \\
IW - TW & NH4 & TW & 0.05499 & ns \\
CW - TW & NH4 & CW & 0.4776 & ns \\
IW - CW & NO3 & IW & 0.05499 & ns \\
IW - TW & NO3 & IW & 0.03071 & * \\
CW - TW & NO3 & CW & 0.8258 & ns \\
IW - CW & pH & CW & 0.5567 & ns \\
IW - TW & pH & TW & 0.05722 & ns \\
CW - TW & pH & TW & 0.0001527 & *** \\
IW - CW & Conductivity & CW & 2.649e-09 & *** \\
IW - TW & Conductivity & TW & 1.187e-05 & *** \\
CW - TW & Conductivity & CW & 3.551e-07 & *** \\
IW - CW & TN & IW & 0.05651 & ns \\
IW - TW & TN & TW & 0.9705 & ns \\
CW - TW & TN & TW & 0.04214 & * \\
\bottomrule
\end{longtable}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# require GGally package installed}
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.32}}
  >{\centering\arraybackslash}p{(\columnwidth - 8\tabcolsep) * \real{0.12}}
...
\CommentTok{\# Let\textquotesingle{}s try to use ggplot2 with clustering plot}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_cor}\NormalTok{(}\AttributeTok{cluster\_ggplot =} \StringTok{"row"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_scatterfit}\NormalTok{(}
    ...
    \AttributeTok{point\_alpha =} \FloatTok{0.1}\NormalTok{, }\AttributeTok{label.x.npc =} \StringTok{"center"}\NormalTok{, }\AttributeTok{label.y.npc =} \StringTok{"bottom"}\NormalTok{, }
...
\begin{center}\includegraphics[width=550px]{Images/plot_scatterfit_cor} \end{center}
...
\NormalTok{meco\_module\_func}\SpecialCharTok{$}\FunctionTok{plot\_spe\_func\_perc}\NormalTok{(}\AttributeTok{select\_samples =} \FunctionTok{paste0}\NormalTok{(}\StringTok{"M"}\NormalTok{, }\DecValTok{1}\SpecialCharTok{:}\DecValTok{10}\NormalTok{))}
...
Please check out \url{https://chiliubio.github.io/microeco_tutorial/intro.html\#tax4fun2} for downloading ncbi-blast and Ref99NR/Ref100NR.
...
\CommentTok{\# use read.fasta() of seqinr package or readDNAStringSet() of Biostrings package to read fasta file}
\CommentTok{\# https://chiliubio.github.io/microeco\_tutorial/intro.html\#tax4fun2  for the reading example}
...
\CommentTok{\# calculate functional gene redundancies}
...
\FunctionTok{humann2meco}\NormalTok{(}\AttributeTok{abund\_table =}\NormalTok{ abund\_file\_path, }\AttributeTok{db =} \StringTok{"MetaCyc"}\NormalTok{)}
\FunctionTok{humann2meco}\NormalTok{(}\AttributeTok{abund\_table =}\NormalTok{ abund\_file\_path, }\AttributeTok{db =} \StringTok{"MetaCyc"}\NormalTok{, }\AttributeTok{sample\_data =}\NormalTok{ sample\_file\_path, }\AttributeTok{match\_table =}\NormalTok{ match\_file\_path)}
\CommentTok{\# Let\textquotesingle{}s try more interesting usages with microeco}
\NormalTok{test }\OtherTok{\textless{}{-}} \FunctionTok{humann2meco}\NormalTok{(}\AttributeTok{abund\_table =}\NormalTok{ abund\_file\_path, }\AttributeTok{db =} \StringTok{"MetaCyc"}\NormalTok{, }\AttributeTok{sample\_data =}\NormalTok{ sample\_file\_path, }\AttributeTok{match\_table =}\NormalTok{ match\_file\_path)}
\NormalTok{test}\SpecialCharTok{$}\FunctionTok{tidy\_dataset}\NormalTok{()}
\CommentTok{\# rel = FALSE donot use relative abundance}
...
\NormalTok{test1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(test, }\AttributeTok{taxrank =} \StringTok{"Superclass1"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{10}\NormalTok{)}
\NormalTok{test1}\SpecialCharTok{$}\FunctionTok{plot\_bar}\NormalTok{(}\AttributeTok{facet =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{ylab\_title =} \StringTok{"Abundance (RPK)"}\NormalTok{)}
...
\CommentTok{\# p\_adjust\_method = NULL shut down the p value adjustment}
\NormalTok{test1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(test, }\AttributeTok{method =} \StringTok{"lefse"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{p\_adjust\_method =} \ConstantTok{NULL}\NormalTok{)}
...
\NormalTok{sample\_file\_path }\OtherTok{\textless{}{-}} \FunctionTok{system.file}\NormalTok{(}\StringTok{"extdata"}\NormalTok{, }\StringTok{"example\_metagenome\_sample\_info.tsv"}\NormalTok{, }\AttributeTok{package=}\StringTok{"file2meco"}\NormalTok{)}
\NormalTok{match\_file\_path }\OtherTok{\textless{}{-}} \FunctionTok{system.file}\NormalTok{(}\StringTok{"extdata"}\NormalTok{, }\StringTok{"example\_metagenome\_match\_table.tsv"}\NormalTok{, }\AttributeTok{package=}\StringTok{"file2meco"}\NormalTok{)}
\FunctionTok{mpa2meco}\NormalTok{(}\AttributeTok{abund\_table =}\NormalTok{ abund\_file\_path)}
\NormalTok{test }\OtherTok{\textless{}{-}} \FunctionTok{mpa2meco}\NormalTok{(}\AttributeTok{abund\_table =}\NormalTok{ abund\_file\_path, }\AttributeTok{sample\_data =}\NormalTok{ sample\_file\_path, }\AttributeTok{match\_table =}\NormalTok{ match\_file\_path)}
\NormalTok{test}\SpecialCharTok{$}\FunctionTok{tidy\_dataset}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\# Let\textquotesingle{}s try more interesting usages with microeco}
\NormalTok{test }\OtherTok{\textless{}{-}} \FunctionTok{ncyc2meco}\NormalTok{(}\AttributeTok{abund\_table =}\NormalTok{ abund\_file\_path, }\AttributeTok{sample\_data =}\NormalTok{ sample\_file\_path, }\AttributeTok{match\_table =}\NormalTok{ match\_file\_path)}
\NormalTok{test}\SpecialCharTok{$}\FunctionTok{tidy\_dataset}\NormalTok{()}
...
\NormalTok{test}\SpecialCharTok{$}\FunctionTok{cal\_abund}\NormalTok{(}\AttributeTok{select\_cols =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{3}\NormalTok{, }\AttributeTok{rel =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{test1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(test, }\AttributeTok{taxrank =} \StringTok{"Level.2"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{10}\NormalTok{)}
...
\section{clone() function}\label{clone-function}}
...
\includegraphics{microeco-tutorial_files/figure-latex/unnamed-chunk-269-1.pdf}
...
\includegraphics{microeco-tutorial_files/figure-latex/unnamed-chunk-270-1.pdf}
