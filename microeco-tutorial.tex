...
\urlstyle{same}
\hypersetup{
  pdftitle={Tutorial for R microeco package (v2.3.0)},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}

\title{Tutorial for R microeco package (v2.3.0)}
\author{}
\date{\vspace{-2.5em}2026-09-24}
...
MicrobiotaProcess \citep{Xu_MicrobiotaProcess_2023}
and so on.
...
Based on this background, we created the R microeco package \citep{Liu_microeco_2021, Liu_workflow_2026, Liu_microeco2_2026}.
It can be found in CRAN and GitHub (\url{https://github.com/ChiLiubio/microeco}).
...
For Windows system,
RTools (\url{https://cran.r-project.org/bin/windows/Rtools/}) is necessary to install some R packages from source (those that need compilation of C/C++ or Fortran code).
...
Note that some errors may come from the latest versions because of memory issue (\url{https://www.biostars.org/p/413294/}).
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(microeco)}
\CommentTok{\# load the example data; 16S rRNA gene amplicon sequencing data}
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1974}}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1842}}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3026}}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.3158}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
~
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1233}}
  >{\centering\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1507}}
  >{\centering\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1644}}
  >{\centering\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1507}}
  >{\centering\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1918}}
...
\NormalTok{mt }\OtherTok{\textless{}{-}}\NormalTok{ microtable}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{otu\_table =}\NormalTok{ otu\_table\_16S)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## No sample_table provided, automatically use column names in otu_table to create one ...
\end{verbatim}
...
\NormalTok{mt }\OtherTok{\textless{}{-}}\NormalTok{ microtable}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{otu\_table =}\NormalTok{ otu\_table\_16S, }\AttributeTok{sample\_table =}\NormalTok{ sample\_info\_16S)}
\NormalTok{mt}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mt }\OtherTok{\textless{}{-}}\NormalTok{ microtable}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{sample\_table =}\NormalTok{ sample\_info\_16S, }\AttributeTok{otu\_table =}\NormalTok{ otu\_table\_16S, }\AttributeTok{tax\_table =}\NormalTok{ taxonomy\_table\_16S, }\AttributeTok{phylo\_tree =}\NormalTok{ phylo\_tree\_16S)}
\NormalTok{mt}
\end{Highlighting}
\end{Shaded}
...
To fully back up this original dataset (R6 class), the \texttt{clone} function should be used instead of direct assignment (\url{https://chiliubio.github.io/microeco_tutorial/notes.html\#clone-function}).
To further save the dataset to a local computer file, the \texttt{save} function should be used (\url{https://chiliubio.github.io/microeco_tutorial/notes.html\#save-function}).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mt\_raw }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt)}
\end{Highlighting}
\end{Shaded}
...
HUMAnN \citep{Franzosa_Species_2018}, MetaPhlAn \citep{Blanco_MetaPhlAn_2023}, kraken2 \citep{Wood_Improved_2019}, phyloseq \citep{Mcmurdie_phyloseq_2013}, etc.
...
\NormalTok{mt}\SpecialCharTok{$}\NormalTok{tax\_table }\SpecialCharTok{\%\textless{}\textgreater{}\%}\NormalTok{ base}\SpecialCharTok{::}\FunctionTok{subset}\NormalTok{(Kingdom }\SpecialCharTok{==} \StringTok{"k\_\_Archaea"} \SpecialCharTok{|}\NormalTok{ Kingdom }\SpecialCharTok{==} \StringTok{"k\_\_Bacteria"}\NormalTok{)}
\CommentTok{\# another way with grepl function}
\NormalTok{mt}\SpecialCharTok{$}\NormalTok{tax\_table }\SpecialCharTok{\%\textless{}\textgreater{}\%}\NormalTok{ .[}\FunctionTok{grepl}\NormalTok{(}\StringTok{"Bacteria|Archaea"}\NormalTok{, .}\SpecialCharTok{$}\NormalTok{Kingdom), ]}
\NormalTok{mt}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mt}\SpecialCharTok{$}\FunctionTok{filter\_pollution}\NormalTok{(}\AttributeTok{taxa =} \FunctionTok{c}\NormalTok{(}\StringTok{"mitochondria"}\NormalTok{, }\StringTok{"chloroplast"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
To make the OTU and sample information consistent across all files in the object, we use function \texttt{tidy\_dataset} to trim the data.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mt}\SpecialCharTok{$}\FunctionTok{tidy\_dataset}\NormalTok{()}
\NormalTok{mt}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mt}\SpecialCharTok{$}\FunctionTok{save\_table}\NormalTok{(}\AttributeTok{dirpath =} \StringTok{"basic\_files"}\NormalTok{, }\AttributeTok{sep =} \StringTok{","}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Those have been shown in file2meco package part (\url{https://chiliubio.github.io/microeco_tutorial/file2meco-package.html\#humann-metagenomic-results}) with complex metagenomic data.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# default parameter (rel = TRUE) denotes relative abundance}
\NormalTok{mt}\SpecialCharTok{$}\FunctionTok{cal\_abund}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# return taxa\_abund list in the object}
\FunctionTok{class}\NormalTok{(mt}\SpecialCharTok{$}\NormalTok{taxa\_abund)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mt}\SpecialCharTok{$}\NormalTok{taxa\_abund}\SpecialCharTok{$}\NormalTok{Phylum[}\DecValTok{1}\SpecialCharTok{:}\DecValTok{5}\NormalTok{, }\DecValTok{1}\SpecialCharTok{:}\DecValTok{5}\NormalTok{]}
\end{Highlighting}
\end{Shaded}

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.4235}}
  >{\centering\arraybackslash}p{(\linewidth - 10\tabcolsep) * \real{0.1059}}
...
\bottomrule\noalign{}
\endlastfoot
**k\_\_Bacteria\textbar p\_\_Proteobacteria** & 0.2005 & 0.2055 & 0.2115 & 0.2572 & 0.1683 \\
**k\_\_Bacteria\textbar p\_\_Chloroflexi** & 0.1208 & 0.1888 & 0.1668 & 0.1494 & 0.312 \\
**k\_\_Bacteria\textbar p\_\_Bacteroidetes** & 0.1814 & 0.03274 & 0.02704 & 0.02249 & 0.02859 \\
**k\_\_Bacteria\textbar p\_\_Acidobacteria** & 0.1193 & 0.2483 & 0.2497 & 0.2644 & 0.2462 \\
**k\_\_Bacteria\textbar p\_\_Actinobacteria** & 0.1222 & 0.08405 & 0.08869 & 0.08852 & 0.07963 \\
\end{longtable}
}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mt}\SpecialCharTok{$}\FunctionTok{save\_abund}\NormalTok{(}\AttributeTok{dirpath =} \StringTok{"taxa\_abund"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mt}\SpecialCharTok{$}\FunctionTok{save\_abund}\NormalTok{(}\AttributeTok{merge\_all =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{sep =} \StringTok{"}\SpecialCharTok{\textbackslash{}t}\StringTok{"}\NormalTok{, }\AttributeTok{quote =} \ConstantTok{FALSE}\NormalTok{)}
\CommentTok{\# remove those unclassified}
\NormalTok{mt}\SpecialCharTok{$}\FunctionTok{save\_abund}\NormalTok{(}\AttributeTok{merge\_all =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{sep =} \StringTok{"}\SpecialCharTok{\textbackslash{}t}\StringTok{"}\NormalTok{, }\AttributeTok{rm\_un =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{rm\_pattern =} \StringTok{"\_\_$|Sedis$"}\NormalTok{, }\AttributeTok{quote =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
please refer to the section of the \texttt{trans\_norm} class.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# first clone the data}
\NormalTok{mt\_rarefied }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt)}
\CommentTok{\# use sample\_sums to check the sequence numbers in each sample}
\NormalTok{mt\_rarefied}\SpecialCharTok{$}\FunctionTok{sample\_sums}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}\NormalTok{ range}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mt\_rarefied}\SpecialCharTok{$}\FunctionTok{rarefy\_samples}\NormalTok{(}\AttributeTok{sample.size =} \DecValTok{10000}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mt\_rarefied}\SpecialCharTok{$}\FunctionTok{cal\_alphadiv}\NormalTok{(}\AttributeTok{PD =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# return alpha\_diversity in the object}
\FunctionTok{class}\NormalTok{(mt\_rarefied}\SpecialCharTok{$}\NormalTok{alpha\_diversity)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# save alpha\_diversity to a directory}
\NormalTok{mt\_rarefied}\SpecialCharTok{$}\FunctionTok{save\_alphadiv}\NormalTok{(}\AttributeTok{dirpath =} \StringTok{"alpha\_diversity"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mt\_rarefied}\SpecialCharTok{$}\FunctionTok{cal\_betadiv}\NormalTok{(}\AttributeTok{unifrac =} \ConstantTok{TRUE}\NormalTok{)}
\CommentTok{\# return beta\_diversity list in the object}
\FunctionTok{class}\NormalTok{(mt\_rarefied}\SpecialCharTok{$}\NormalTok{beta\_diversity)}
\CommentTok{\# save beta\_diversity to a directory}
\NormalTok{mt\_rarefied}\SpecialCharTok{$}\FunctionTok{save\_betadiv}\NormalTok{(}\AttributeTok{dirpath =} \StringTok{"beta\_diversity"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{test }\OtherTok{\textless{}{-}}\NormalTok{ mt}\SpecialCharTok{$}\FunctionTok{merge\_taxa}\NormalTok{(}\StringTok{"Genus"}\NormalTok{)}
\NormalTok{test}
\end{Highlighting}
\end{Shaded}
...
## microtable-class object:
## sample_table have 90 rows and 4 columns
## otu_table have 1267 rows and 90 columns
## tax_table have 1267 rows and 6 columns
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{test }\OtherTok{\textless{}{-}}\NormalTok{ mt}\SpecialCharTok{$}\FunctionTok{merge\_samples}\NormalTok{(}\StringTok{"Group"}\NormalTok{)}
\NormalTok{test}
\end{Highlighting}
\end{Shaded}
...
## otu_table have 13296 rows and 3 columns
## tax_table have 13296 rows and 7 columns
## phylo_tree have 13296 tips
\end{verbatim}
...
\NormalTok{group\_CW }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt)}
...
\CommentTok{\# trim all the data}
\NormalTok{group\_CW}\SpecialCharTok{$}\FunctionTok{tidy\_dataset}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## 1945 taxa with 0 abundance are removed from the otu_table ...
\end{verbatim}
...
## otu_table have 11351 rows and 30 columns
## tax_table have 11351 rows and 7 columns
## phylo_tree have 11351 tips
...
Similar with above operation, \textbf{subset of features can be achieved by manipulating the \texttt{tax\_table} or \texttt{otu\_table} in microtable object directly.}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# extracting OTU data by manipulating tax\_table}
\NormalTok{proteo }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt)}
...
## microtable-class object:
## sample_table have 90 rows and 4 columns
## otu_table have 3166 rows and 90 columns
## tax_table have 3166 rows and 7 columns
## phylo_tree have 3166 tips
## Taxa abundance: calculated for Kingdom,Phylum,Class,Order,Family,Genus,Species
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# extracting OTU data by manipulating otu\_table}
\NormalTok{test }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt)}
\NormalTok{test}\SpecialCharTok{$}\NormalTok{otu\_table }\OtherTok{\textless{}{-}}\NormalTok{ test}\SpecialCharTok{$}\NormalTok{otu\_table[}\FunctionTok{c}\NormalTok{(}\StringTok{"OTU\_32"}\NormalTok{, }\StringTok{"OTU\_50"}\NormalTok{, }\StringTok{"OTU\_1"}\NormalTok{), ]}
\NormalTok{test}\SpecialCharTok{$}\FunctionTok{tidy\_dataset}\NormalTok{()}
\NormalTok{test}
\end{Highlighting}
\end{Shaded}

\subsection{add ASV/OTU to tax\_table}\label{add-asvotu-to-tax_table}

The function \texttt{add\_rownames2taxonomy} can add the row names of \texttt{tax\_table} as the last column of \texttt{tax\_table} directly.
...
\begin{Highlighting}[]
\NormalTok{test }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt)}
\NormalTok{test}\SpecialCharTok{$}\FunctionTok{add\_rownames2taxonomy}\NormalTok{(}\StringTok{"ASV"}\NormalTok{)}
\CommentTok{\# then we can calculate the relative abundance of ASV}
\CommentTok{\# for all taxonomic levels like commonly operation}
\NormalTok{test}\SpecialCharTok{$}\FunctionTok{cal\_abund}\NormalTok{()}
\FunctionTok{View}\NormalTok{(test}\SpecialCharTok{$}\NormalTok{taxa\_abund}\SpecialCharTok{$}\NormalTok{ASV)}
\CommentTok{\# only ASV level}
\NormalTok{test}\SpecialCharTok{$}\FunctionTok{cal\_abund}\NormalTok{(}\AttributeTok{select\_cols =} \StringTok{"ASV"}\NormalTok{)}
\FunctionTok{View}\NormalTok{(test}\SpecialCharTok{$}\NormalTok{taxa\_abund}\SpecialCharTok{$}\NormalTok{ASV)}
\end{Highlighting}
\end{Shaded}

\subsection{filter the features with low abundance/occurrence frequency}\label{filter-the-features-with-low-abundanceoccurrence-frequency}
...
\NormalTok{test }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt)}
\CommentTok{\# In this example, the relative abundance threshold is 0.0001}
\CommentTok{\# occurrence frequency 0.1; 10\% samples should have the target features}
\NormalTok{test}\SpecialCharTok{$}\FunctionTok{filter\_taxa}\NormalTok{(}\AttributeTok{rel\_abund =} \FloatTok{0.0001}\NormalTok{, }\AttributeTok{freq =} \FloatTok{0.1}\NormalTok{)}
\NormalTok{test}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
## 881 taxa with 0 abundance are removed from the otu_table ...
\end{verbatim}
...
\begin{verbatim}
## 3883 taxa with 0 abundance are removed from the otu_table ...
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# clone a complete dataset}
\NormalTok{test }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt)}
...
\section{trans\_norm class}\label{trans_norm-class}
...
The \texttt{trans\_norm} class in the microeco package (\textgreater= 1.6.0) provides several data normalization approaches for the microtable object.
Current available methods include rarefaction, centered log-ratio (CLR) \citep{Aitchison_book_1986, Greenacre_Compositional_2021},
robust centered log-ratio (RCLR) \citep{Martino_Novel_2019},
total sum scaling (TSS, also known as Proportion),
median ratio of counts relative to geometric mean (DESeq2) \citep{Love_Moderated_2014},
trimmed mean of M-values (TMM) \citep{Robinson_scaling_2010}, relative log expression (RLE),
cumulative sum scaling (CSS) \citep{Paulson_Differential_2013},
geometric mean of pairwise ratios (GMPR) \citep{Chen_GMPR_2018},
and Wrench \citep{Kumar_Analysis_2018}.
The rarefaction method used here is exactly the same as the \texttt{rarefy\_samples} function in the previous \texttt{microtable} chapter,
...
The relative abundance calculated using \texttt{cal\_abund} as demonstrated in the previous chapter also falls under TSS.
...
considering compositional effects and sequencing depth issues. \citep{Swift_reviewnorm_2022}.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tmp }\OtherTok{\textless{}{-}}\NormalTok{ trans\_norm}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mt\_rarefied }\OtherTok{\textless{}{-}}\NormalTok{ tmp}\SpecialCharTok{$}\FunctionTok{norm}\NormalTok{(}\AttributeTok{method =} \StringTok{"rarefy"}\NormalTok{, }\AttributeTok{sample.size =} \DecValTok{10000}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mt\_clr }\OtherTok{\textless{}{-}}\NormalTok{ tmp}\SpecialCharTok{$}\FunctionTok{norm}\NormalTok{(}\AttributeTok{method =} \StringTok{"clr"}\NormalTok{)}
\CommentTok{\# Robust centered log{-}ratio normalization}
\NormalTok{mt\_rclr }\OtherTok{\textless{}{-}}\NormalTok{ tmp}\SpecialCharTok{$}\FunctionTok{norm}\NormalTok{(}\AttributeTok{method =} \StringTok{"rclr"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mt\_TSS }\OtherTok{\textless{}{-}}\NormalTok{ tmp}\SpecialCharTok{$}\FunctionTok{norm}\NormalTok{(}\AttributeTok{method =} \StringTok{"TSS"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mt\_GMPR }\OtherTok{\textless{}{-}}\NormalTok{ tmp}\SpecialCharTok{$}\FunctionTok{norm}\NormalTok{(}\AttributeTok{method =} \StringTok{"GMPR"}\NormalTok{)}
\CommentTok{\# Cumulative sum scaling normalization}
\NormalTok{mt\_CSS }\OtherTok{\textless{}{-}}\NormalTok{ tmp}\SpecialCharTok{$}\FunctionTok{norm}\NormalTok{(}\AttributeTok{method =} \StringTok{"CSS"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{taxrank =} \StringTok{"Phylum"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{8}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{taxrank =} \StringTok{"Genus"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{8}\NormalTok{)}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{taxrank =} \StringTok{"Phylum"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{10}\NormalTok{, }\AttributeTok{groupmean =} \StringTok{"Group"}\NormalTok{)}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{taxrank =} \StringTok{"Class"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{15}\NormalTok{)}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{taxrank =} \StringTok{"Genus"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{40}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{taxrank =} \StringTok{"Phylum"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{5}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_line}\NormalTok{()}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{taxrank =} \StringTok{"Genus"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{5}\NormalTok{, }\AttributeTok{groupmean =} \StringTok{"Type"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{taxrank =} \StringTok{"Phylum"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{6}\NormalTok{, }\AttributeTok{groupmean =} \StringTok{"Group"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{taxrank =} \StringTok{"Phylum"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{8}\NormalTok{, }\AttributeTok{groupmean =} \StringTok{"Group"}\NormalTok{)}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{taxrank =} \StringTok{"Phylum"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{8}\NormalTok{, }\AttributeTok{groupmean =} \StringTok{"Type"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_radar}\NormalTok{(}\AttributeTok{values.radar =} \FunctionTok{c}\NormalTok{(}\StringTok{"0\%"}\NormalTok{, }\StringTok{"25\%"}\NormalTok{, }\StringTok{"50\%"}\NormalTok{), }\AttributeTok{grid.min =} \DecValTok{0}\NormalTok{, }\AttributeTok{grid.mid =} \FloatTok{0.25}\NormalTok{, }\AttributeTok{grid.max =} \FloatTok{0.5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{test1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{taxrank =} \StringTok{"Class"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{10}\NormalTok{, }\AttributeTok{high\_level =} \StringTok{"Phylum"}\NormalTok{, }\AttributeTok{delete\_taxonomy\_prefix =} \ConstantTok{FALSE}\NormalTok{)}
...
\item
  input\_taxaname parameter: input\_taxaname parameter in trans\_abund\$new can be used to select interested customized taxa instead of abundance-based filtering
\item
  use\_percentage parameter: use\_percentage parameter in trans\_abund\$new - whether show the abundance percentage
\item
  order\_x parameter: order the sample (group) names in x axis in the bar plot
\end{itemize}
...
\NormalTok{tmp }\OtherTok{\textless{}{-}}\NormalTok{ mt\_rarefied}\SpecialCharTok{$}\FunctionTok{merge\_samples}\NormalTok{(}\StringTok{"Group"}\NormalTok{)}
\CommentTok{\# tmp is a new microtable object}
\CommentTok{\# create trans\_venn object}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_venn}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(tmp, }\AttributeTok{ratio =} \ConstantTok{NULL}\NormalTok{)}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_venn}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(tmp, }\AttributeTok{ratio =} \StringTok{"seqratio"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_venn}\NormalTok{()}
...
\NormalTok{tmp }\OtherTok{\textless{}{-}}\NormalTok{ mt\_rarefied}\SpecialCharTok{$}\FunctionTok{merge\_samples}\NormalTok{(}\StringTok{"Type"}\NormalTok{)}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_venn}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(tmp)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{test }\OtherTok{\textless{}{-}}\NormalTok{ mt\_rarefied}\SpecialCharTok{$}\FunctionTok{merge\_samples}\NormalTok{(}\StringTok{"Type"}\NormalTok{)}
...
\NormalTok{test }\OtherTok{\textless{}{-}}\NormalTok{ mt\_rarefied}\SpecialCharTok{$}\FunctionTok{merge\_samples}\NormalTok{(}\StringTok{"Type"}\NormalTok{)}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_venn}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(test)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_bar}\NormalTok{(}\AttributeTok{left\_bar\_fill =}\NormalTok{ RColorBrewer}\SpecialCharTok{::}\FunctionTok{brewer.pal}\NormalTok{(}\DecValTok{8}\NormalTok{, }\StringTok{"Dark2"}\NormalTok{), }\AttributeTok{left\_bar\_alpha =} \FloatTok{0.2}\NormalTok{, }\AttributeTok{bottom\_background\_fill =}\NormalTok{ RColorBrewer}\SpecialCharTok{::}\FunctionTok{brewer.pal}\NormalTok{(}\DecValTok{8}\NormalTok{, }\StringTok{"Dark2"}\NormalTok{), }\AttributeTok{bottom\_background\_alpha =} \FloatTok{0.2}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{data\_summary }\SpecialCharTok{\%\textless{}\textgreater{}\%}\NormalTok{ .[.}\SpecialCharTok{$}\NormalTok{Counts }\SpecialCharTok{\textgreater{}} \DecValTok{1}\NormalTok{, ]}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_bar}\NormalTok{(}\AttributeTok{left\_width =} \FloatTok{0.15}\NormalTok{, }\AttributeTok{left\_bar\_fill =} \StringTok{"grey50"}\NormalTok{, }\AttributeTok{up\_bar\_fill =} \StringTok{"grey50"}\NormalTok{, }\AttributeTok{up\_bar\_width =} \FloatTok{0.5}\NormalTok{, }\AttributeTok{left\_bar\_width =} \FloatTok{0.5}\NormalTok{, }\AttributeTok{bottom\_height =} \FloatTok{0.5}\NormalTok{, }\AttributeTok{bottom\_point\_color =} \StringTok{"black"}\NormalTok{, }\AttributeTok{bottom\_point\_size =} \DecValTok{8}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_alpha}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt\_rarefied, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{)}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{, }\AttributeTok{add\_sig\_text\_size =} \DecValTok{6}\NormalTok{, }\AttributeTok{add =} \StringTok{"jitter"}\NormalTok{, }\AttributeTok{order\_x\_mean =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{, }\AttributeTok{shape =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{add =} \StringTok{"jitter"}\NormalTok{, }\AttributeTok{y\_start =} \FloatTok{0.1}\NormalTok{, }\AttributeTok{y\_increase =} \FloatTok{0.1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=600px]{Images/trans_alpha_group_wilcox} \end{center}

Let's try to remove the `ns' in the label by manipulating the \texttt{object\$res\_diff}.
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{, }\AttributeTok{add =} \StringTok{"dotplot"}\NormalTok{, }\AttributeTok{xtext\_size =} \DecValTok{15}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=600px]{Images/trans_alpha_wilcox_nons} \end{center}

The trans\_alpha class supports the differential test of groups within each group using the \texttt{by\_group} parameter.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_alpha}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt\_rarefied, }\AttributeTok{group =} \StringTok{"Type"}\NormalTok{, }\AttributeTok{by\_group =} \StringTok{"Group"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_diff}\NormalTok{(}\AttributeTok{method =} \StringTok{"wilcox"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_alpha}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt\_rarefied)}
...
Note that from v1.9.0, the parameter \texttt{plot\_type} control which type of plot is employed.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_alpha}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt\_rarefied, }\AttributeTok{group =} \StringTok{"Type"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_diff}\NormalTok{(}\AttributeTok{method =} \StringTok{"KW\_dunn"}\NormalTok{, }\AttributeTok{KW\_dunn\_letter =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"ggboxplot"}\NormalTok{, }\AttributeTok{add =} \StringTok{"none"}\NormalTok{)}

\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"ggdotplot"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"ggdotplot"}\NormalTok{, }\AttributeTok{fill =} \StringTok{"Type"}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.3}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"ggdotplot"}\NormalTok{, }\AttributeTok{add =} \StringTok{"mean\_se"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"ggdotplot"}\NormalTok{, }\AttributeTok{add =} \FunctionTok{c}\NormalTok{(}\StringTok{"mean\_se"}\NormalTok{, }\StringTok{"violin"}\NormalTok{))}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"ggdotplot"}\NormalTok{, }\AttributeTok{add =} \FunctionTok{c}\NormalTok{(}\StringTok{"mean\_se"}\NormalTok{, }\StringTok{"violin"}\NormalTok{), }\AttributeTok{fill =} \StringTok{"Type"}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.2}\NormalTok{)}

\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"ggviolin"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"ggviolin"}\NormalTok{, }\AttributeTok{y\_increase =} \FloatTok{0.4}\NormalTok{, }\AttributeTok{add =} \StringTok{"mean\_se"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"ggviolin"}\NormalTok{, }\AttributeTok{fill =} \StringTok{"Type"}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.2}\NormalTok{, }\AttributeTok{y\_increase =} \FloatTok{0.4}\NormalTok{, }\AttributeTok{add =} \StringTok{"mean\_se"}\NormalTok{, }\AttributeTok{add\_sig\_text\_size =} \DecValTok{6}\NormalTok{)}

\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"ggstripchart"}\NormalTok{, }\AttributeTok{add =} \StringTok{"mean\_se"}\NormalTok{)}

\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"ggerrorplot"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

The option \texttt{"errorbar"} or \texttt{"barerrorbar"} in \texttt{plot\_alpha} will invoke the \texttt{data\_stat} instead of \texttt{data\_alpha} for the Mean±SE (or SD) plot (based on the ggplot2).
The line is optional to be added between points (Mean) for the case with a gradient.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_alpha}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt\_rarefied, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_diff}\NormalTok{(}\AttributeTok{method =} \StringTok{"KW\_dunn"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{, }\AttributeTok{KW\_dunn\_letter =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"errorbar"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"errorbar"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{, }\AttributeTok{y\_increase =} \SpecialCharTok{{-}}\FloatTok{0.2}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"errorbar"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{, }\AttributeTok{y\_increase =} \SpecialCharTok{{-}}\FloatTok{0.2}\NormalTok{, }\AttributeTok{add\_line =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{line\_type =} \DecValTok{2}\NormalTok{, }\AttributeTok{line\_alpha =} \FloatTok{0.5}\NormalTok{, }\AttributeTok{errorbar\_width =} \FloatTok{0.1}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"errorbar"}\NormalTok{, }\AttributeTok{plot\_SE =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{, }\AttributeTok{y\_increase =} \FloatTok{0.2}\NormalTok{, }\AttributeTok{add\_line =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{line\_type =} \DecValTok{2}\NormalTok{, }\AttributeTok{line\_alpha =} \FloatTok{0.5}\NormalTok{, }\AttributeTok{errorbar\_width =} \FloatTok{0.1}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"barerrorbar"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"barerrorbar"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{, }\AttributeTok{y\_increase =} \SpecialCharTok{{-}}\FloatTok{0.3}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"barerrorbar"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Chao1"}\NormalTok{, }\AttributeTok{bar\_width =} \FloatTok{0.6}\NormalTok{, }\AttributeTok{errorbar\_width =} \FloatTok{0.2}\NormalTok{, }\AttributeTok{errorbar\_size =} \DecValTok{1}\NormalTok{, }\AttributeTok{errorbar\_addpoint =} \ConstantTok{FALSE}\NormalTok{)}
...
\FunctionTok{View}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_diff)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"errorbar"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Shannon"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"errorbar"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Shannon"}\NormalTok{, }\AttributeTok{add\_line =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{line\_type =} \DecValTok{2}\NormalTok{)}
...
\CommentTok{\# For PCoA and NMDS, measure parameter must be provided.}
\CommentTok{\# measure parameter should be either one of names(mt\_rarefied$beta\_diversity) or a customized symmetric matrix}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_beta}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt\_rarefied, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"bray"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tmp }\OtherTok{\textless{}{-}}\NormalTok{ mt\_rarefied}\SpecialCharTok{$}\FunctionTok{merge\_taxa}\NormalTok{(}\AttributeTok{taxa =} \StringTok{"Genus"}\NormalTok{)}
\NormalTok{tmp}\SpecialCharTok{$}\NormalTok{tax\_table }\SpecialCharTok{\%\textless{}\textgreater{}\%}\NormalTok{ .[.}\SpecialCharTok{$}\NormalTok{Genus }\SpecialCharTok{!=} \StringTok{"g\_\_"}\NormalTok{, ]}
\NormalTok{tmp}\SpecialCharTok{$}\FunctionTok{tidy\_dataset}\NormalTok{()}
\FunctionTok{rownames}\NormalTok{(tmp}\SpecialCharTok{$}\NormalTok{otu\_table) }\OtherTok{\textless{}{-}}\NormalTok{ tmp}\SpecialCharTok{$}\NormalTok{tax\_table[}\FunctionTok{rownames}\NormalTok{(tmp}\SpecialCharTok{$}\NormalTok{otu\_table), }\StringTok{"Genus"}\NormalTok{]}
\FunctionTok{rownames}\NormalTok{(tmp}\SpecialCharTok{$}\NormalTok{tax\_table) }\OtherTok{\textless{}{-}}\NormalTok{ tmp}\SpecialCharTok{$}\NormalTok{tax\_table[, }\StringTok{"Genus"}\NormalTok{]}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_beta}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ tmp)}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_group\_distance}\NormalTok{(}\AttributeTok{add =} \StringTok{"mean"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_group\_distance\_diff}\NormalTok{(}\AttributeTok{method =} \StringTok{"wilcox"}\NormalTok{)}
\CommentTok{\# parameters in plot\_group\_distance function will be passed to the plot\_alpha function of trans\_alpha class}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_group\_distance}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"ggviolin"}\NormalTok{, }\AttributeTok{add =} \StringTok{"mean\_se"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_group\_distance}\NormalTok{(}\AttributeTok{add =} \StringTok{"mean"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{tmp }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt\_rarefied)}
\NormalTok{tmp}\SpecialCharTok{$}\NormalTok{sample\_table }\SpecialCharTok{\%\textless{}\textgreater{}\%} \FunctionTok{subset}\NormalTok{(Group }\SpecialCharTok{\%in\%} \FunctionTok{c}\NormalTok{(}\StringTok{"CW"}\NormalTok{, }\StringTok{"TW"}\NormalTok{))}
\NormalTok{tmp}\SpecialCharTok{$}\FunctionTok{tidy\_dataset}\NormalTok{()}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_beta}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ tmp, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{)}
...
\begin{verbatim}
## Use Group column for the overall test!
\end{verbatim}
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.0694}}
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1447}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1316}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1053}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1184}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1316}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1711}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1974}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
Groups
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1507}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1644}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1370}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1096}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1233}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1370}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1781}}@{}}
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\linewidth - 0\tabcolsep) * \real{0.2083}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
Significance
\end{minipage} \\
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.0641}}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Group} & 0 & -3.553e-15 & -1.135e-16 & -Inf & NA & NA \\
...
\section{trans\_rarefy class}\label{trans_rarefy-class}

The class trans\_rarefy can be used for the rarefaction and the following plotting to see whether
...
The class is a wrapper of two other classes: the rarefying is performed by the \texttt{norm} function of the
\texttt{trans\_norm} class, and the alpha diversity is calculated by the \texttt{cal\_alphadiv} function of the
\texttt{microtable} class, in which the measurements are computed by the vegan package \citep{Jari_vegan_2019}.
All the results shown in this section were checked with microeco v2.4.0.

\subsection{Example}\label{example-4}

The \texttt{alphadiv} parameter accepts one or more alpha diversity measurements, and \texttt{"Observed"} means the
observed species number, i.e.~the richness.
The \texttt{depth} parameter determines at which sequencing depths the data are rarefied.
...
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{123}\NormalTok{)}
\NormalTok{tmp }\OtherTok{\textless{}{-}}\NormalTok{ microtable}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{sample\_table =}\NormalTok{ sample\_info\_16S, }\AttributeTok{otu\_table =}\NormalTok{ otu\_table\_16S)}
\NormalTok{tmp}\SpecialCharTok{$}\FunctionTok{tidy\_dataset}\NormalTok{()}
\CommentTok{\# trans\_rarefy class: rarefy the data at the provided depths and calculate the alpha diversity}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_rarefy}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(tmp, }\AttributeTok{alphadiv =} \StringTok{"Observed"}\NormalTok{, }\AttributeTok{depth =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{, }\DecValTok{50}\NormalTok{, }\DecValTok{500}\NormalTok{, }\DecValTok{2000}\NormalTok{, }\DecValTok{6000}\NormalTok{, }\DecValTok{10000}\NormalTok{, }\DecValTok{15000}\NormalTok{, }\DecValTok{20000}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

As the \texttt{norm} function removes the features and the samples that do not survive the random subsampling,
...
The result is a data.frame stored in \texttt{t1\$res\_rarefy}, and the measurements and depths used are also
saved in the object.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# return t1$res\_rarefy; SampleID: sample name, seqnum: rarefying depth}
\FunctionTok{head}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_rarefy)}
\CommentTok{\# the measurements and depths used}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{measure}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{depth}
\CommentTok{\# the print method of the class}
\NormalTok{t1}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \texttt{depth\ =\ 0} is a special value: the data are not rarefied and all the diversity values are directly set
...
\item
  ...
  \texttt{norm} function, so it is absent at this depth and its curve stops earlier.
  In this example the numbers of retained samples are 90 at the depths 0-10000, 54 at 15000 and 25 at
  20000, so \texttt{t1\$res\_rarefy} has 709 rows in total.
\item
  The rarefying is a random subsampling, but the results are repeatable, as the \texttt{norm} function fixes the
  random seed (\texttt{rngseed\ =\ 123} by default); see the following More options section.
\end{itemize}

Then the rarefaction curve can be plotted directly with the \texttt{plot\_rarefy} function, which returns a
ggplot object.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# the color parameter can be any column name of the sample\_table}
\NormalTok{p1 }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{plot\_rarefy}\NormalTok{(}\AttributeTok{color =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{show\_point =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{add\_fitting =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{p1}
\CommentTok{\# use a fitted curve instead of the points and lines}
\NormalTok{p2 }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{plot\_rarefy}\NormalTok{(}\AttributeTok{color =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{show\_point =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{add\_fitting =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{p2}
\CommentTok{\# show the sample names at the end of the curves}
\NormalTok{p3 }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{plot\_rarefy}\NormalTok{(}\AttributeTok{show\_samplename =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{color\_values =} \FunctionTok{rep}\NormalTok{(}\StringTok{"grey70"}\NormalTok{, }\DecValTok{100}\NormalTok{), }\AttributeTok{show\_legend =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{p3}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=600px]{Images/tcr_plot_rarefy_group} \end{center}

\begin{center}\includegraphics[width=600px]{Images/tcr_plot_rarefy_fitting} \end{center}

\begin{center}\includegraphics[width=600px]{Images/tcr_plot_rarefy_samplename} \end{center}

\texttt{color} defaults to \texttt{"SampleID"}; \texttt{"Group"} is used in the first two plots just to keep the legend
readable.
In the third plot all the curves are drawn in grey and each sample is labelled at its own last available
depth.
As the label colours are taken from \texttt{color\_values}, don't forget to provide enough colours; if the colours
...
\subsubsection{Multiple measurements at the same time}\label{multiple-measurements-at-the-same-time}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_rarefy}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(tmp, }\AttributeTok{alphadiv =} \FunctionTok{c}\NormalTok{(}\StringTok{"Observed"}\NormalTok{, }\StringTok{"Shannon"}\NormalTok{), }\AttributeTok{depth =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{50}\NormalTok{, }\DecValTok{500}\NormalTok{, }\DecValTok{2000}\NormalTok{, }\DecValTok{6000}\NormalTok{, }\DecValTok{10000}\NormalTok{, }\DecValTok{15000}\NormalTok{))}
\CommentTok{\# the plot is faceted by the measurements when more than one measurement is used}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_rarefy}\NormalTok{(}\AttributeTok{color =} \StringTok{"Group"}\NormalTok{)}
\CommentTok{\# only one of the measurements can be shown with the measure parameter}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_rarefy}\NormalTok{(}\AttributeTok{color =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Shannon"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=700px]{Images/tcr_plot_rarefy_measures} \end{center}

When multiple measurements are used, each measurement is shown in its own panel with a free y axis
(\texttt{facet\_wrap(\textasciitilde{}\ measure,\ scales\ =\ "free\_y")}) and the y axis title defaults to \texttt{"Value"}.
The names of the measurements are case-insensitive, and both the long names and the short names used in
the vegan package are accepted, e.g.~\texttt{"Observed"}/\texttt{"s.obs"}, \texttt{"Chao1"}/\texttt{"s.chao1"}, \texttt{"ACE"}/\texttt{"s.ace"}.
The available measurements are Observed, Coverage, Chao1, ACE, Shannon, Simpson, InvSimpson, Fisher,
Pielou and PD.

\subsubsection{Automatic generation of the depths}\label{automatic-generation-of-the-depths}

When \texttt{depth} is not provided, a sequence of 10 depths ranging from 0 to the maximum of the sample sums is
generated automatically with a quadratic spacing.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# the depth is generated automatically when it is not provided}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_rarefy}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(tmp, }\AttributeTok{alphadiv =} \StringTok{"Observed"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{depth}
\end{Highlighting}
\end{Shaded}
...
The depths are calculated as \texttt{round(max(sample\_sums)\ *\ seq(0,\ 1,\ length.out\ =\ 10)\^{}2)}, i.e.~the intervals
...
The default measurement is \texttt{"Shannon"} when \texttt{alphadiv} is not provided.

\subsubsection{Alternative rarefying method}\label{alternative-rarefying-method}

The \texttt{method} and \texttt{rngseed} parameters are passed to the \texttt{norm} function of the \texttt{trans\_norm} class.
Besides the default \texttt{"rarefy"}, the \texttt{"SRS"} method (scaling with ranked subsampling) is available and can
be used to avoid the information loss of the classic rarefaction \citep{Beule_normalization_2020}.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# "SRS" is the scaling with ranked subsampling method}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_rarefy}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(tmp, }\AttributeTok{alphadiv =} \StringTok{"Observed"}\NormalTok{, }\AttributeTok{depth =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{2000}\NormalTok{), }\AttributeTok{method =} \StringTok{"SRS"}\NormalTok{)}
\CommentTok{\# change the random seed used by the subsampling; the rarefying is repeatable for a fixed seed}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_rarefy}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(tmp, }\AttributeTok{alphadiv =} \StringTok{"Observed"}\NormalTok{, }\AttributeTok{depth =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{2000}\NormalTok{), }\AttributeTok{rngseed =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Note that \texttt{sample.size} in \texttt{...} is ignored, as the library size is controlled by the \texttt{depth} parameter;
a message is printed when it is provided.

\subsubsection{Plot adjustments}\label{plot-adjustments}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# axes titles, legend and the size and transparency of the points}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_rarefy}\NormalTok{(}\AttributeTok{color =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{x\_axis\_title =} \StringTok{"Sequencing depth"}\NormalTok{, }\AttributeTok{y\_axis\_title =} \StringTok{"Observed richness"}\NormalTok{,}
  \AttributeTok{show\_point =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{point\_size =}\NormalTok{ .}\DecValTok{5}\NormalTok{, }\AttributeTok{point\_alpha =}\NormalTok{ .}\DecValTok{8}\NormalTok{)}
\CommentTok{\# the parameters in ... are passed to geom\_line (add\_fitting = FALSE) or geom\_smooth (add\_fitting = TRUE)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_rarefy}\NormalTok{(}\AttributeTok{linewidth =} \DecValTok{1}\NormalTok{, }\AttributeTok{alpha =}\NormalTok{ .}\DecValTok{5}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_rarefy}\NormalTok{(}\AttributeTok{color =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{show\_point =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{add\_fitting =} \ConstantTok{TRUE}\NormalTok{,}
  \AttributeTok{fitting\_method =} \StringTok{"lm"}\NormalTok{, }\AttributeTok{fitting\_formula =}\NormalTok{ y }\SpecialCharTok{\textasciitilde{}}\NormalTok{ x)}
\CommentTok{\# the returned object is a ggplot, so it can be further customized}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_rarefy}\NormalTok{(}\AttributeTok{color =} \StringTok{"Group"}\NormalTok{) }\SpecialCharTok{+}\NormalTok{ ggplot2}\SpecialCharTok{::}\FunctionTok{theme\_classic}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\texttt{add\_fitting\ =\ TRUE} fits the curve with \texttt{ggplot2::geom\_smooth}; the default \texttt{fitting\_method\ =\ "lm"} and
\texttt{fitting\_formula\ =\ y\ \textasciitilde{}\ log(x\ +\ 1)} avoid the warnings of the default \texttt{"loess"} method, as there are
usually only a few depths for each sample.
Note that \texttt{x\ +\ 1} is used instead of \texttt{x} in the default formula, because the depth 0 makes \texttt{log(x)}
invalid.
The \texttt{se} argument of \texttt{geom\_smooth} is fixed to \texttt{FALSE} inside the class, so passing \texttt{se} in \texttt{...} raises
an error when \texttt{add\_fitting\ =\ TRUE}.
...
\tightlist
\item
  The class is a wrapper: the rarefying comes from \texttt{trans\_norm\$norm} and the alpha diversity from
  \texttt{microtable\$cal\_alphadiv}; one rarefied \texttt{microtable} object is rebuilt from the original data at each
  depth, so the rarefying at different depths is independent.
\item
  \texttt{depth\ =\ 0} is kept as an artificial starting point of the curve, where all the diversity values are 0
  rather than the real values of the original data.
\item
  The \texttt{depth} values must be non-negative integers and not larger than \texttt{max(dataset\$sample\_sums())};
...
\item
  ...
  and the number of rows per depth decreases; the total number of rows of \texttt{res\_rarefy} is therefore
  smaller than \texttt{length(depth)\ *\ n\_samples}.
\item
  Rarefying is random but repeatable: \texttt{rngseed\ =\ 123} by default, and the results are identical for the
  same seed.
\item
  Very low depths should be used with care: the \texttt{"Fisher"} measurement may be unavailable, and \texttt{"Pielou"}
  returns \texttt{NaN} when only one species is present at a depth. The class prints a message listing the
  measurements and the number of rows with missing values, e.g.~\texttt{Pielou\ (90\ rows)} at depth 1 for the
  example data.
\item
  \texttt{alphadiv} is case-insensitive; the available measurements are Observed, Coverage, Chao1, ACE, Shannon,
  Simpson, InvSimpson, Fisher, Pielou and PD.
\item
  Parameters in \texttt{...} of \texttt{new} are passed to the \texttt{norm} function, such as \texttt{method}, \texttt{rngseed} and
  \texttt{replace}, except \texttt{sample.size}, which is controlled by \texttt{depth}.
\item
  \texttt{color} of \texttt{plot\_rarefy} can be any column of \texttt{sample\_table} and defaults to \texttt{"SampleID"};
  \texttt{color\_values} are expanded automatically when they are not enough.
\item
  \texttt{plot\_rarefy} returns a ggplot object, so the plot can be further customized with the ggplot2 syntax.
\end{itemize}
...
\begin{center}\includegraphics[width=8000px]{Images/trans_diff_demons} \end{center}

Note: The methods for invoking \texttt{otu\_table} are primarily designed for differential abundance analysis of ASVs/OTUs (or Genes).
These methods for invoking \texttt{taxa\_abund} list are recommended for analyses at higher taxonomic levels (such as Genus or Phylum).
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{method =} \StringTok{"lefse"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.01}\NormalTok{, }\AttributeTok{lefse\_subgroup =} \ConstantTok{NULL}\NormalTok{)}
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.6627}}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.0964}}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.0964}}
  >{\centering\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.1446}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
Taxa
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{fill =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{alpha =} \FloatTok{0.5}\NormalTok{, }\AttributeTok{add\_sig =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{group\_order =} \FunctionTok{c}\NormalTok{(}\StringTok{"CW"}\NormalTok{, }\StringTok{"IW"}\NormalTok{, }\StringTok{"TW"}\NormalTok{))}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{coord\_flip =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"errorbar"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"barerrorbar"}\NormalTok{, }\AttributeTok{coord\_flip =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"barerrorbar"}\NormalTok{, }\AttributeTok{errorbar\_addpoint =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{errorbar\_color\_black =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{plot\_SE =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"ggviolin"}\NormalTok{, }\AttributeTok{coord\_flip =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{tmp }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"c\_\_Deltaproteobacteria"}\NormalTok{, }\StringTok{"c\_\_Actinobacteria"}\NormalTok{, }\StringTok{"o\_\_Rhizobiales"}\NormalTok{, }\StringTok{"p\_\_Proteobacteria"}\NormalTok{, }\StringTok{"p\_\_Bacteroidetes"}\NormalTok{, }
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_cladogram}\NormalTok{(}\AttributeTok{use\_taxa\_num =} \DecValTok{200}\NormalTok{, }\AttributeTok{use\_feature\_num =} \DecValTok{50}\NormalTok{, }\AttributeTok{select\_show\_labels =}\NormalTok{ tmp)}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{method =} \StringTok{"rf"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"Genus"}\NormalTok{)}
...
\NormalTok{g2 }\OtherTok{\textless{}{-}}\NormalTok{ g2 }\SpecialCharTok{+} \FunctionTok{theme}\NormalTok{(}\AttributeTok{axis.text.y =} \FunctionTok{element\_blank}\NormalTok{(), }\AttributeTok{axis.ticks.y =} \FunctionTok{element\_blank}\NormalTok{(), }\AttributeTok{panel.border =} \FunctionTok{element\_blank}\NormalTok{())}
\NormalTok{p }\OtherTok{\textless{}{-}}\NormalTok{ g1 }\SpecialCharTok{\%\textgreater{}\%}\NormalTok{ aplot}\SpecialCharTok{::}\FunctionTok{insert\_right}\NormalTok{(g2)}
\NormalTok{p}
\end{Highlighting}
\end{Shaded}
...
The significance label can also be added in the abundance plot controlled by \texttt{add\_sig} parameter and other related parameters.
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{()}
\CommentTok{\# y\_start and y\_increase control the position of labels; for the details, please see the document of plot\_alpha function in trans\_alpha class}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{y\_start =} \FloatTok{0.05}\NormalTok{, }\AttributeTok{y\_increase =} \FloatTok{0.1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{coord\_flip =}\NormalTok{ F, }\AttributeTok{plot\_type =} \StringTok{"barerrorbar"}\NormalTok{, }\AttributeTok{errorbar\_addpoint =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{method =} \StringTok{"metastat"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"Genus"}\NormalTok{)}
\CommentTok{\# t1$res\_diff is the differential test result}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{use\_number =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{20}\NormalTok{, }\AttributeTok{coord\_flip =}\NormalTok{ F)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{method =} \StringTok{"metagenomeSeq"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{)}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_bar}\NormalTok{(}\AttributeTok{use\_number =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{20}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_volcano}\NormalTok{()}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_volcano}\NormalTok{(}\AttributeTok{select\_group =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# when fix\_formula is not provided (necessary in ancombc2 function of ANCOMBC package), it will be assigned automatically by using group parameter}
\CommentTok{\# Restart R if there is an error that "Error in model.matrix.formula(): data must be a data.frame"}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{method =} \StringTok{"ancombc2"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.01}\NormalTok{)}
...
\NormalTok{tmp }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt)}
\NormalTok{tmp}\SpecialCharTok{$}\NormalTok{sample\_table }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(tmp}\SpecialCharTok{$}\NormalTok{sample\_table, env\_data\_16S[}\FunctionTok{rownames}\NormalTok{(tmp}\SpecialCharTok{$}\NormalTok{sample\_table), ])}
\NormalTok{t2 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ tmp, }\AttributeTok{method =} \StringTok{"ancombc2"}\NormalTok{, }\AttributeTok{group =} \ConstantTok{NULL}\NormalTok{, }\AttributeTok{fix\_formula =} \StringTok{"pH+Group"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.005}\NormalTok{)}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{method =} \StringTok{"linda"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diff\_abund}\NormalTok{(}\AttributeTok{use\_number =} \DecValTok{1}\SpecialCharTok{:}\DecValTok{30}\NormalTok{, }\AttributeTok{group\_order =} \FunctionTok{c}\NormalTok{(}\StringTok{"TW"}\NormalTok{, }\StringTok{"CW"}\NormalTok{, }\StringTok{"IW"}\NormalTok{), }\AttributeTok{add\_sig =} \ConstantTok{TRUE}\NormalTok{)}
\FunctionTok{View}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_diff)}
...
\NormalTok{tmp }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt)}
\NormalTok{tmp}\SpecialCharTok{$}\NormalTok{sample\_table }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(tmp}\SpecialCharTok{$}\NormalTok{sample\_table, env\_data\_16S[}\FunctionTok{rownames}\NormalTok{(tmp}\SpecialCharTok{$}\NormalTok{sample\_table), ])}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ tmp, }\AttributeTok{method =} \StringTok{"linda"}\NormalTok{, }\AttributeTok{group =} \StringTok{"pH + Group"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
\NormalTok{t2 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ tmp, }\AttributeTok{method =} \StringTok{"linda"}\NormalTok{, }\AttributeTok{formula =} \StringTok{"\textasciitilde{}pH + Group"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{plot\_diff\_bar}\NormalTok{(}\AttributeTok{keep\_full\_name =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{color\_values =} \FunctionTok{c}\NormalTok{(}\StringTok{"\#053061"}\NormalTok{, }\StringTok{"white"}\NormalTok{, }\StringTok{"\#A50026"}\NormalTok{), }\AttributeTok{trans =} \StringTok{"log10"}\NormalTok{, }\AttributeTok{filter\_feature =} \StringTok{""}\NormalTok{, }\AttributeTok{ytext\_position =} \StringTok{"left"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\subsection{More options}\label{more-options-1}
...
The methods \texttt{DESeq2} and \texttt{edgeR} are also supported in the trans\_diff class and no longer demonstrated here.
...
When no random effects are present, the `glmm\_beta' method is equivalent to the `betareg' method (beta regression).
...
\NormalTok{tmp }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt)}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ tmp, }\AttributeTok{taxa\_level =} \StringTok{"Phylum"}\NormalTok{, }\AttributeTok{method =} \StringTok{"glmm\_beta"}\NormalTok{, }
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \StringTok{"spearman"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \StringTok{"spearman"}\NormalTok{, }\AttributeTok{use\_WGCNA\_pearson\_spearman =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.0001}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.3472}}
  >{\centering\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.1389}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
Property
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1772}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1266}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1139}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1772}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1519}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1139}}
...
\NormalTok{t2 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_env}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{add\_data =}\NormalTok{ env\_data\_16S[, }\DecValTok{4}\SpecialCharTok{:}\DecValTok{11}\NormalTok{])}
...
\NormalTok{sub1 }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{subset\_network}\NormalTok{(}\AttributeTok{sample\_name =} \StringTok{"S1"}\NormalTok{, }\AttributeTok{return\_igraph =} \ConstantTok{FALSE}\NormalTok{)}
\CommentTok{\# return\_igraph = FALSE means return is trans\_network object instead of igraph object}
\NormalTok{sub1}\SpecialCharTok{$}\FunctionTok{cal\_module}\NormalTok{()}
\NormalTok{sub1}\SpecialCharTok{$}\FunctionTok{save\_network}\NormalTok{(}\StringTok{"S1.gexf"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Extract all sub-networks for each sample.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{samples\_network }\OtherTok{\textless{}{-}} \FunctionTok{list}\NormalTok{()}
\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \FunctionTok{rownames}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{data\_abund))\{}
\NormalTok{    samples\_network[[i]] }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{subset\_network}\NormalTok{(}\AttributeTok{return\_igraph =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{sample\_name =}\NormalTok{ i)}
\NormalTok{\}}
\CommentTok{\# when return\_igraph = FALSE, samples\_network can be the input for functions in meconetcomp package}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \StringTok{"bray"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
\CommentTok{\# Pearson correlation}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \StringTok{"pearson"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
\CommentTok{\# Pearson correlation using WGCNA package}
\CommentTok{\# install WGCNA package}
\ControlFlowTok{if}\NormalTok{(}\SpecialCharTok{!}\FunctionTok{require}\NormalTok{(}\StringTok{"WGCNA"}\NormalTok{)) }\FunctionTok{install.packages}\NormalTok{(}\StringTok{"WGCNA"}\NormalTok{, }\AttributeTok{repos =}\NormalTok{ BiocManager}\SpecialCharTok{::}\FunctionTok{repositories}\NormalTok{())}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \StringTok{"pearson"}\NormalTok{, }\AttributeTok{use\_WGCNA\_pearson\_spearman =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
\CommentTok{\# Pearson correlation using NetCoMi package; install it from https://github.com/stefpeschel/NetCoMi}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \StringTok{"pearson"}\NormalTok{, }\AttributeTok{use\_NetCoMi\_pearson\_spearman =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
\CommentTok{\# Spearman correlation using WGCNA package}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \StringTok{"spearman"}\NormalTok{, }\AttributeTok{use\_WGCNA\_pearson\_spearman =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
\CommentTok{\# Spearman correlation using NetCoMi package}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \StringTok{"spearman"}\NormalTok{, }\AttributeTok{use\_NetCoMi\_pearson\_spearman =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
\CommentTok{\# SparCC method, from SpiecEasi package, see https://github.com/zdk123/SpiecEasi for the installation}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \StringTok{"sparcc"}\NormalTok{, }\AttributeTok{use\_sparcc\_method =} \StringTok{"SpiecEasi"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.003}\NormalTok{)}
\CommentTok{\# SparCC method, from NetCoMi package; https://github.com/stefpeschel/NetCoMi}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \StringTok{"sparcc"}\NormalTok{, }\AttributeTok{use\_sparcc\_method =} \StringTok{"NetCoMi"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
\CommentTok{\# CCLasso method based on NetCoMi package}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \StringTok{"cclasso"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
\CommentTok{\# CCREPE method based on NetCoMi package}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \StringTok{"ccrepe"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \ConstantTok{NULL}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cor\_method =} \ConstantTok{NULL}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{filter\_thres =} \DecValTok{0}\NormalTok{)}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_nullmodel}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(mt, }\AttributeTok{filter\_thres =} \FloatTok{0.0005}\NormalTok{, }\AttributeTok{add\_data =}\NormalTok{ env\_data\_16S)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mt}\SpecialCharTok{$}\NormalTok{beta\_diversity[[}\StringTok{"betaNRI"}\NormalTok{]] }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\NormalTok{res\_ses\_betampd}
\CommentTok{\# create trans\_beta class, use measure "betaNRI"}
\NormalTok{t2 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_beta}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"betaNRI"}\NormalTok{)}
...
\NormalTok{g1 }\OtherTok{\textless{}{-}}\NormalTok{ t2}\SpecialCharTok{$}\FunctionTok{plot\_group\_distance}\NormalTok{(}\AttributeTok{add =} \StringTok{"mean"}\NormalTok{)}
...
\begin{verbatim}
## Now randomizing by parallel computing. Begin at Thu Sep 24 15:12:16 2026. Please wait...
\end{verbatim}
...
\bottomrule\noalign{}
\endlastfoot
IW & 435 & 0.5551 & 0.2906 & 0.3008 & 19.99 \\
CW & 435 & 0.5885 & 0.3134 & 0.3177 & 24.61 \\
TW & 435 & 0.5825 & 0.3601 & 0.367 & 17.27 \\
\end{longtable}
}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_classifier}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ tmp, }\AttributeTok{y.response =} \StringTok{"pH"}\NormalTok{, }\AttributeTok{x.predictors =} \StringTok{"Genus"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_split}\NormalTok{(}\AttributeTok{prop.train =} \DecValTok{3}\SpecialCharTok{/}\DecValTok{4}\NormalTok{)}
\CommentTok{\# optional: preprocessing}
\CommentTok{\# t1$cal\_preProcess(method = c("center", "scale"))}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{set\_trainControl}\NormalTok{()}
...
\CommentTok{\# sort and select some taxa with high importance}
\NormalTok{tmp\_top20 }\OtherTok{\textless{}{-}} \FunctionTok{rownames}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_feature\_imp[}\FunctionTok{order}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_feature\_imp[, }\DecValTok{1}\NormalTok{], }\AttributeTok{decreasing =} \ConstantTok{TRUE}\NormalTok{), , }\AttributeTok{drop =} \ConstantTok{FALSE}\NormalTok{])[}\DecValTok{1}\SpecialCharTok{:}\DecValTok{20}\NormalTok{]}
\CommentTok{\# correlation analysis}
\NormalTok{t2 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_env}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ tmp, }\AttributeTok{add\_data =}\NormalTok{ env\_data\_16S[, }\DecValTok{4}\SpecialCharTok{:}\DecValTok{11}\NormalTok{])}
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{cal\_cor}\NormalTok{(}\AttributeTok{use\_data =} \StringTok{"other"}\NormalTok{, }\AttributeTok{other\_taxa =} \FunctionTok{gsub}\NormalTok{(}\StringTok{"\textasciigrave{}"}\NormalTok{, }\StringTok{""}\NormalTok{, tmp\_top20))}
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{plot\_cor}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
as environmental factors, microbial functions and metabolites can be generally applied to explain microbial dynamics, community structure and assembly.
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_env}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt\_rarefied, }\AttributeTok{add\_data =}\NormalTok{ env\_data\_16S[, }\DecValTok{4}\SpecialCharTok{:}\DecValTok{11}\NormalTok{])}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mt\_rarefied}\SpecialCharTok{$}\NormalTok{sample\_table }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(mt\_rarefied}\SpecialCharTok{$}\NormalTok{sample\_table, env\_data\_16S[}\FunctionTok{rownames}\NormalTok{(mt\_rarefied}\SpecialCharTok{$}\NormalTok{sample\_table), ])}
\CommentTok{\# now mt\_rarefied$sample\_table has the whole data}
\NormalTok{mt\_rarefied}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_env}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt\_rarefied, }\AttributeTok{env\_cols =} \DecValTok{8}\SpecialCharTok{:}\DecValTok{15}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\bottomrule\noalign{}
\endlastfoot
IW - CW & Temperature & CW & 3.283e-10 & NA \\
IW - TW & Temperature & TW & 0.0001958 & NA \\
CW - TW & Temperature & CW & 3.283e-10 & 2.712 \\
IW - CW & Precipitation & CW & 5.239e-09 & -1.491 \\
IW - TW & Precipitation & IW & 0.07758 & 0.2278 \\
CW - TW & Precipitation & CW & 4.002e-07 & 1.719 \\
IW - CW & TOC & IW & 0.4687 & 0.6371 \\
\end{longtable}
}
...
\begin{longtable}[]{@{}
  ...
  >{\centering\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.3250}}
  >{\centering\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1250}}
...
The parameter \texttt{partial} controls wheter conduct partial correlation.
...
Then, we can visualize the correlation results using \texttt{plot\_cor} function.
...
We can use the \texttt{filter\_feature} parameter in \texttt{plot\_cor} to filter some taxa that do not have any significance \textless{} 0.001.
...
please use \textbf{other\_taxa} parameter in \texttt{cal\_cor} function.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# first create trans\_diff object as a demonstration}
\NormalTok{t2 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt\_rarefied, }\AttributeTok{method =} \StringTok{"rf"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"Genus"}\NormalTok{)}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_cor}\NormalTok{(}\AttributeTok{add\_abund\_table =}\NormalTok{ mt\_rarefied}\SpecialCharTok{$}\NormalTok{alpha\_diversity)}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_scatterfit}\NormalTok{(}
    ...
    \AttributeTok{y =}\NormalTok{ mt\_rarefied}\SpecialCharTok{$}\NormalTok{beta\_diversity}\SpecialCharTok{$}\NormalTok{bray[}\FunctionTok{rownames}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{data\_env), }\FunctionTok{rownames}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{data\_env)], }
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_scatterfit}\NormalTok{(}
    \AttributeTok{x =}\NormalTok{ mt\_rarefied}\SpecialCharTok{$}\NormalTok{beta\_diversity}\SpecialCharTok{$}\NormalTok{bray[}\FunctionTok{rownames}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{data\_env), }\FunctionTok{rownames}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{data\_env)],}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_env}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{add\_data =}\NormalTok{ env\_data\_16S[, }\DecValTok{6}\SpecialCharTok{:}\DecValTok{11}\NormalTok{])}
...
\NormalTok{t2 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_classifier}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{x.predictors =}\NormalTok{ tmp, }\AttributeTok{y.response =} \StringTok{"Saline"}\NormalTok{)}
...
\NormalTok{tmp }\OtherTok{\textless{}{-}} \FunctionTok{unlist}\NormalTok{(mt\_rarefied}\SpecialCharTok{$}\NormalTok{taxa\_abund}\SpecialCharTok{$}\NormalTok{Genus[}\StringTok{"k\_\_Bacteria|p\_\_Actinobacteria|c\_\_Actinobacteria|o\_\_Micrococcales|f\_\_Micrococcaceae|g\_\_Arthrobacter"}\NormalTok{, ])}
...
\NormalTok{t2 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_func}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(mt)}
...
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{cal\_func}\NormalTok{(}\AttributeTok{prok\_database =} \StringTok{"FAPROTAX"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## FAPROTAX v1.2.12. Please also cite the original FAPROTAX paper: Louca et al. (2016).
\end{verbatim}
...
\begin{verbatim}
## Functional binary result is stored in object$res_func ...
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# return t2$res\_func, 1 represent trait exists, 0 represent no or cannot confirmed.}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t2}\SpecialCharTok{$}\NormalTok{res\_func[}\DecValTok{1}\SpecialCharTok{:}\DecValTok{5}\NormalTok{, }\DecValTok{1}\SpecialCharTok{:}\DecValTok{2}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
The ratio/abundance of OTUs having the same trait/function can reflect the functional redundancy (FR).
The \texttt{cal\_func\_FR} function is designed to obtain the FR for each function.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# here do not consider OTU abundances and taxonomic dispersion of traits}
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{cal\_func\_FR}\NormalTok{(}\AttributeTok{abundance\_weighted =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{perc =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{adj\_tax =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## The result table is stored in object$res_func_FR ...
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# t2$res\_func\_FR[1:5, 1:2]}
\end{Highlighting}
\end{Shaded}
...
The \texttt{trans\_func\_FR} function is implemented to get the long-format table for more flexible manipulation, e.g., filtering and grouping.
The return \texttt{res\_func\_FR\_trans} in the object is the table for the following visualization.
Note that this step is not necessary as \texttt{plot\_func\_FR} function can automatically invoke this function if \texttt{res\_func\_FR\_trans} is not found.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{trans\_func\_FR}\NormalTok{()}
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{plot\_func\_FR}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

The function \texttt{cal\_func\_FR\_comm} can be used to calculate overall FR of a community based on the the result of each function from \texttt{cal\_func\_FR} function.
The result is the geometric mean of FR of all functions in a community.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{cal\_func\_FR\_comm}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

For the differential test of the FR across groups,
...
Then we take another example to show the FR for each trait in network modules.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# construct a network for the example}
\NormalTok{network }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{cal\_cor =} \StringTok{"base"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.0001}\NormalTok{, }\AttributeTok{cor\_method =} \StringTok{"spearman"}\NormalTok{)}
...
\NormalTok{meco\_module\_func}\SpecialCharTok{$}\FunctionTok{cal\_func}\NormalTok{(}\AttributeTok{prok\_database =} \StringTok{"FAPROTAX"}\NormalTok{)}
\NormalTok{meco\_module\_func}\SpecialCharTok{$}\FunctionTok{cal\_func\_FR}\NormalTok{(}\AttributeTok{abundance\_weighted =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{perc =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{meco\_module\_func}\SpecialCharTok{$}\FunctionTok{plot\_func\_FR}\NormalTok{(}\AttributeTok{order\_x =} \FunctionTok{paste0}\NormalTok{(}\StringTok{"M"}\NormalTok{, }\DecValTok{1}\SpecialCharTok{:}\DecValTok{10}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
Then we try to correlate the FR data in \texttt{res\_func\_FR} to environmental variables.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t3 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_env}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{add\_data =}\NormalTok{ env\_data\_16S[, }\DecValTok{4}\SpecialCharTok{:}\DecValTok{11}\NormalTok{])}
\NormalTok{t3}\SpecialCharTok{$}\FunctionTok{cal\_cor}\NormalTok{(}\AttributeTok{add\_abund\_table =}\NormalTok{ t2}\SpecialCharTok{$}\NormalTok{res\_func\_FR, }\AttributeTok{cor\_method =} \StringTok{"spearman"}\NormalTok{)}
\NormalTok{t3}\SpecialCharTok{$}\FunctionTok{plot\_cor}\NormalTok{(}\AttributeTok{cluster\_ggplot =} \StringTok{"both"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\subsection{Key points}\label{key-points-11}
...
\section{trans\_metab class}\label{trans_metab-class}

This class is a wrapper for a series of metabolite analysis, including origin inference \citep{Wang_TidyMass2_2026} and pathway enrichment.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# use data inside the microeco package}
\NormalTok{tmp }\OtherTok{\textless{}{-}}\NormalTok{ trans\_metab}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{metab =}\NormalTok{ soil\_metab, }\AttributeTok{microb =}\NormalTok{ soil\_microb)}

\CommentTok{\# If the tax\_table in the input metab data already contains detailed annotations (such as KEGG, HMDB columns),}
\CommentTok{\# user can skip the following step. This step performs text{-}based matching to align the input metabolite names with those in the database.}
\CommentTok{\# please download the database from https://zenodo.org/records/18618912; uncompress the zip file}
\CommentTok{\# Fuzzy matching of metabolite names against those in the database}
\NormalTok{tmp}\SpecialCharTok{$}\FunctionTok{cal\_match}\NormalTok{(}\AttributeTok{database\_path =} \StringTok{"./metorigindb\_split\_202602"}\NormalTok{)}
\CommentTok{\# View(test$res\_match)}

\CommentTok{\# origin inference based on the database of TidyMass2}
\CommentTok{\# The parameter match\_col works by determining metabolite assignment based on the provided "names" (row names) or other columns (e.g., "KEGG\_ID"). }
\CommentTok{\# For "names", the function first checks whether res\_match (from the last step) exists; if it does, metabolite identification is performed according to the distance threshold.}
\NormalTok{tmp}\SpecialCharTok{$}\FunctionTok{cal\_origin}\NormalTok{(}\AttributeTok{database\_path =} \StringTok{"./metorigindb\_split\_202602"}\NormalTok{, }\AttributeTok{match\_col =} \FunctionTok{c}\NormalTok{(}\StringTok{"names"}\NormalTok{, }\StringTok{"HMDB\_ID"}\NormalTok{, }\StringTok{"KEGG\_ID"}\NormalTok{))}
\CommentTok{\# tmp$res\_origin\_rawtable and tmp$res\_origin\_list}

\CommentTok{\# create the metabolites{-}bacteria network according to the result from the last step}
\NormalTok{res\_network }\OtherTok{\textless{}{-}}\NormalTok{ tmp}\SpecialCharTok{$}\FunctionTok{cal\_origin\_network}\NormalTok{()}

\CommentTok{\# use trans\_network class to visualize the result}
\NormalTok{test }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{()}
\NormalTok{test}\SpecialCharTok{$}\NormalTok{res\_network }\OtherTok{\textless{}{-}}\NormalTok{ res\_network}
\NormalTok{test}\SpecialCharTok{$}\FunctionTok{cal\_module}\NormalTok{()}
\NormalTok{test}\SpecialCharTok{$}\FunctionTok{get\_node\_table}\NormalTok{()}
\FunctionTok{head}\NormalTok{(test}\SpecialCharTok{$}\NormalTok{res\_node\_table)}

\CommentTok{\# test$plot\_network(method = "networkD3", node\_color = "type")}



\DocumentationTok{\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#}
\DocumentationTok{\#\#\#\#\#\#\#\#\#\# Pathway analysis}
\CommentTok{\# Map metabolites to pathways based on a database}
\NormalTok{tmp}\SpecialCharTok{$}\FunctionTok{cal\_pathway}\NormalTok{(}\AttributeTok{db\_type =} \StringTok{"kegg"}\NormalTok{)}

\CommentTok{\# Perform pathway enrichment analysis for target metabolites using Fisher\textquotesingle{}s exact test}
\NormalTok{target }\OtherTok{\textless{}{-}} \FunctionTok{rownames}\NormalTok{(soil\_metab}\SpecialCharTok{$}\NormalTok{data\_metab}\SpecialCharTok{$}\NormalTok{otu\_table)[}\DecValTok{1}\SpecialCharTok{:}\DecValTok{50}\NormalTok{]}
\NormalTok{tmp}\SpecialCharTok{$}\FunctionTok{cal\_pathway\_enrich}\NormalTok{(}\AttributeTok{target\_metabs =}\NormalTok{ target, }\AttributeTok{p\_adjust\_method =} \StringTok{"BH"}\NormalTok{, }\AttributeTok{p\_cutoff =} \FloatTok{0.05}\NormalTok{, }\AttributeTok{min\_metab\_count =} \DecValTok{3}\NormalTok{)}

\NormalTok{tmp}\SpecialCharTok{$}\FunctionTok{plot\_pathway\_enrich}\NormalTok{()}


\DocumentationTok{\#\#\#\# Following steps can be used to get the metabolite{-}pathway network}
\CommentTok{\# res\_net \textless{}{-} tmp$cal\_pathway\_network()}

\CommentTok{\# test \textless{}{-} trans\_network$new()}
\CommentTok{\# test$res\_network \textless{}{-} res\_net}
\CommentTok{\# test$cal\_module()}
\CommentTok{\# test$get\_node\_table()}
\CommentTok{\# head(test$res\_node\_table)}
\end{Highlighting}
\end{Shaded}

\chapter{trans\_niche and trans\_phylo}\label{trans_niche-and-trans_phylo}

Starting from version 2.3.0, two new classes have been added: trans\_niche and trans\_phylo.
The trans\_niche class is used to calculate niche breadth and niche overlap.
The trans\_phylo class relies on the ggtree package to visualize phylogenetic trees of ASVs/OTUs/Species and annotate attributes on the outer layers of trees.
...
\NormalTok{mt }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(dataset)}
\NormalTok{mt200 }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt)}

\CommentTok{\# Subset to top 200 OTUs}
\NormalTok{top\_otus }\OtherTok{\textless{}{-}} \FunctionTok{names}\NormalTok{(}\FunctionTok{sort}\NormalTok{(mt200}\SpecialCharTok{$}\FunctionTok{taxa\_sums}\NormalTok{(), }\AttributeTok{decreasing =} \ConstantTok{TRUE}\NormalTok{))[}\DecValTok{1}\SpecialCharTok{:}\DecValTok{200}\NormalTok{]}
\NormalTok{mt200}\SpecialCharTok{$}\NormalTok{otu\_table }\SpecialCharTok{\%\textless{}\textgreater{}\%}\NormalTok{ .[top\_otus, ]}
\NormalTok{mt200}\SpecialCharTok{$}\FunctionTok{tidy\_dataset}\NormalTok{()}

\CommentTok{\# ========================================================================}
\CommentTok{\# Build rich ring annotation data}
\CommentTok{\# ========================================================================}

\NormalTok{otu }\OtherTok{\textless{}{-}}\NormalTok{ mt200}\SpecialCharTok{$}\NormalTok{otu\_table}
\NormalTok{tax }\OtherTok{\textless{}{-}}\NormalTok{ mt200}\SpecialCharTok{$}\NormalTok{tax\_table}
\NormalTok{si  }\OtherTok{\textless{}{-}}\NormalTok{ mt200}\SpecialCharTok{$}\NormalTok{sample\_table}
\NormalTok{n\_otus }\OtherTok{\textless{}{-}} \FunctionTok{nrow}\NormalTok{(otu)}

\CommentTok{\# Relative abundance (mean \%)}
\NormalTok{total\_rel\_abund }\OtherTok{\textless{}{-}}\NormalTok{ trans\_norm}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(mt)}\SpecialCharTok{$}\FunctionTok{norm}\NormalTok{(}\AttributeTok{method =} \StringTok{"TSS"}\NormalTok{)}\SpecialCharTok{$}\NormalTok{otu\_table}
\NormalTok{rel\_abund   }\OtherTok{\textless{}{-}}\NormalTok{ total\_rel\_abund[}\FunctionTok{rownames}\NormalTok{(otu), ]}
\NormalTok{mean\_abund  }\OtherTok{\textless{}{-}} \FunctionTok{rowMeans}\NormalTok{(rel\_abund) }\SpecialCharTok{*} \DecValTok{100}

\CommentTok{\# Prevalence (\% of samples where detected)}
\NormalTok{prevalence }\OtherTok{\textless{}{-}} \FunctionTok{rowMeans}\NormalTok{(otu }\SpecialCharTok{\textgreater{}} \DecValTok{0}\NormalTok{) }\SpecialCharTok{*} \DecValTok{100}

\CommentTok{\# Log10 abundance}
\NormalTok{log\_abund }\OtherTok{\textless{}{-}} \FunctionTok{log10}\NormalTok{(}\FunctionTok{rowMeans}\NormalTok{(otu) }\SpecialCharTok{+} \DecValTok{1}\NormalTok{)}

\CommentTok{\# {-}{-}{-}{-} Simulated ecological indicators {-}{-}{-}{-}}
\CommentTok{\# Importance index: composite of abundance and prevalence}
\CommentTok{\# (mimics indicators like indicator species analysis or random forest importance)}
\NormalTok{importance\_raw }\OtherTok{\textless{}{-}}\NormalTok{ mean\_abund }\SpecialCharTok{*} \FunctionTok{sqrt}\NormalTok{(prevalence }\SpecialCharTok{/} \DecValTok{100}\NormalTok{)}
\NormalTok{importance }\OtherTok{\textless{}{-}} \FunctionTok{round}\NormalTok{(importance\_raw }\SpecialCharTok{/} \FunctionTok{max}\NormalTok{(importance\_raw, }\AttributeTok{na.rm =} \ConstantTok{TRUE}\NormalTok{) }\SpecialCharTok{*} \DecValTok{100}\NormalTok{, }\DecValTok{2}\NormalTok{)}

\CommentTok{\# Specificity: how concentrated is an OTU in its preferred habitat; Use niche breadth value from trans\_niche class}
\CommentTok{\# (low = generalist, high = specialist)}
\NormalTok{niche }\OtherTok{\textless{}{-}}\NormalTok{ trans\_niche}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(mt)}\SpecialCharTok{$}\FunctionTok{cal\_niche\_breadth}\NormalTok{()}
\NormalTok{specificity }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ niche}\SpecialCharTok{$}\NormalTok{res\_niche\_breadth[}\FunctionTok{rownames}\NormalTok{(otu), }\StringTok{"Levins\_Standardized"}\NormalTok{]}

\CommentTok{\# network metrics with trans\_network class example}
\NormalTok{net }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(mt, }\AttributeTok{cor\_method =} \StringTok{"pearson"}\NormalTok{)}\SpecialCharTok{$}\FunctionTok{cal\_network}\NormalTok{()}\SpecialCharTok{$}\FunctionTok{get\_node\_table}\NormalTok{()}
\NormalTok{centrality }\OtherTok{\textless{}{-}}\NormalTok{ net}\SpecialCharTok{$}\NormalTok{res\_node\_table[}\FunctionTok{rownames}\NormalTok{(otu), }\StringTok{"closeness\_centrality"}\NormalTok{]}
\NormalTok{connectivity }\OtherTok{\textless{}{-}}\NormalTok{ net}\SpecialCharTok{$}\NormalTok{res\_node\_table[}\FunctionTok{rownames}\NormalTok{(otu), }\StringTok{"z"}\NormalTok{]}

\CommentTok{\# Habitat}
\NormalTok{mt}\SpecialCharTok{$}\FunctionTok{add\_rownames2tax}\NormalTok{(}\StringTok{"OTU"}\NormalTok{)}
\NormalTok{mt}\SpecialCharTok{$}\FunctionTok{cal\_abund}\NormalTok{()}
\NormalTok{mt\_diff }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{method =} \StringTok{"rf"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{alpha =} \DecValTok{1}\NormalTok{)}
\NormalTok{mt\_diff\_res }\OtherTok{\textless{}{-}}\NormalTok{ mt\_diff}\SpecialCharTok{$}\NormalTok{res\_diff }
\FunctionTok{rownames}\NormalTok{(mt\_diff\_res) }\SpecialCharTok{\%\textless{}\textgreater{}\%} \FunctionTok{gsub}\NormalTok{(}\StringTok{".*}\SpecialCharTok{\textbackslash{}\textbackslash{}}\StringTok{|"}\NormalTok{, }\StringTok{""}\NormalTok{, .)}
\NormalTok{dominant\_habitat }\OtherTok{\textless{}{-}}\NormalTok{ mt\_diff\_res[}\FunctionTok{rownames}\NormalTok{(otu), }\StringTok{"Group"}\NormalTok{]}

\CommentTok{\# Salinity}
\NormalTok{mt\_diff }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt, }\AttributeTok{method =} \StringTok{"rf"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{group =} \StringTok{"Saline"}\NormalTok{, }\AttributeTok{alpha =} \DecValTok{1}\NormalTok{)}
\NormalTok{mt\_diff\_res }\OtherTok{\textless{}{-}}\NormalTok{ mt\_diff}\SpecialCharTok{$}\NormalTok{res\_diff }
\FunctionTok{rownames}\NormalTok{(mt\_diff\_res) }\SpecialCharTok{\%\textless{}\textgreater{}\%} \FunctionTok{gsub}\NormalTok{(}\StringTok{".*}\SpecialCharTok{\textbackslash{}\textbackslash{}}\StringTok{|"}\NormalTok{, }\StringTok{""}\NormalTok{, .)}
\NormalTok{dominant\_salinity }\OtherTok{\textless{}{-}}\NormalTok{ mt\_diff\_res[}\FunctionTok{rownames}\NormalTok{(otu), }\StringTok{"Group"}\NormalTok{]}


\CommentTok{\# Class taxonomy}
\NormalTok{class\_col }\OtherTok{\textless{}{-}} \ControlFlowTok{if}\NormalTok{ (}\StringTok{"Class"} \SpecialCharTok{\%in\%} \FunctionTok{colnames}\NormalTok{(tax)) tax}\SpecialCharTok{$}\NormalTok{Class }\ControlFlowTok{else} \FunctionTok{rep}\NormalTok{(}\StringTok{"Unknown"}\NormalTok{, n\_otus)}


\CommentTok{\# Assemble ring data}
\NormalTok{ring\_df }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}
  \AttributeTok{label        =} \FunctionTok{rownames}\NormalTok{(otu),}
  \AttributeTok{RelAbund     =} \FunctionTok{round}\NormalTok{(mean\_abund, }\DecValTok{4}\NormalTok{),}
  \AttributeTok{Prevalence   =} \FunctionTok{round}\NormalTok{(prevalence, }\DecValTok{1}\NormalTok{),}
  \AttributeTok{LogAbund     =} \FunctionTok{round}\NormalTok{(log\_abund, }\DecValTok{3}\NormalTok{),}
  \AttributeTok{Importance   =}\NormalTok{ importance,}
  \AttributeTok{Specificity  =}\NormalTok{ specificity,}
  \AttributeTok{Connectivity =}\NormalTok{ connectivity,}
  \AttributeTok{Habitat      =}\NormalTok{ dominant\_habitat,}
  \AttributeTok{Salinity     =}\NormalTok{ dominant\_salinity,}
  \AttributeTok{Class        =}\NormalTok{ class\_col,}
  \AttributeTok{stringsAsFactors =} \ConstantTok{FALSE}
\NormalTok{)}

\CommentTok{\# Clean Class column}
\NormalTok{ring\_df}\SpecialCharTok{$}\NormalTok{Class }\OtherTok{\textless{}{-}} \FunctionTok{gsub}\NormalTok{(}\StringTok{"\^{}c\_\_"}\NormalTok{, }\StringTok{""}\NormalTok{, ring\_df}\SpecialCharTok{$}\NormalTok{Class)}
\NormalTok{ring\_df}\SpecialCharTok{$}\NormalTok{Class[ring\_df}\SpecialCharTok{$}\NormalTok{Class }\SpecialCharTok{==} \StringTok{""} \SpecialCharTok{|} \FunctionTok{is.na}\NormalTok{(ring\_df}\SpecialCharTok{$}\NormalTok{Class)] }\OtherTok{\textless{}{-}} \StringTok{"Unclassified"}



\CommentTok{\# ============================================================================}
\CommentTok{\# Create trans\_phylo object and plot tree}
\CommentTok{\# ============================================================================}

\NormalTok{OPEN\_ANGLE }\OtherTok{\textless{}{-}} \DecValTok{18}   \CommentTok{\# degrees — slightly wider opening for ring label text}

\NormalTok{pviz }\OtherTok{\textless{}{-}}\NormalTok{ trans\_phylo}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}
  \AttributeTok{dataset             =}\NormalTok{ mt200,}
  \AttributeTok{group\_col           =} \StringTok{"Phylum"}\NormalTok{,}
  \AttributeTok{group\_prefix\_remove =} \StringTok{"p\_\_"}\NormalTok{,}
  \AttributeTok{ring\_data           =}\NormalTok{ ring\_df,}
  \AttributeTok{color\_palette       =} \ConstantTok{NULL}\NormalTok{,}
  \AttributeTok{clade\_coloring      =} \ConstantTok{TRUE}
\NormalTok{)}

\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{plot\_tree}\NormalTok{(}
  \AttributeTok{open\_angle       =}\NormalTok{ OPEN\_ANGLE,}
  \AttributeTok{branch\_size      =} \FloatTok{0.15}\NormalTok{,}
  \AttributeTok{tip\_point\_size   =} \FloatTok{1.2}\NormalTok{,}
  \AttributeTok{tip\_point\_shape  =} \DecValTok{21}\NormalTok{,}
  \AttributeTok{tip\_point\_stroke =} \FloatTok{0.1}\NormalTok{,}
  \AttributeTok{tip\_point\_alpha  =} \FloatTok{0.80}\NormalTok{,}
  \AttributeTok{legend\_title     =} \StringTok{"Phylum"}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\# ============================================================================}
\CommentTok{\# Add ring layers}
\CommentTok{\# ============================================================================}
\CommentTok{\# Two approaches are shown below:}
\CommentTok{\#   (A) Individual add\_ring() calls — maximum per{-}ring control}
\CommentTok{\#   (B) Batch add\_rings\_batch() with vector parameters — concise alternative}
\CommentTok{\# ============================================================================}

\CommentTok{\# {-}{-}{-}{-}{-}{-}{-}{-}{-}{-} Approach A: Individual add\_ring() calls {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}

\CommentTok{\# {-}{-}{-}{-}{-}{-}{-}{-}{-}{-} Numeric rings {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}

\CommentTok{\# Ring 1: Log10 Abundance — viridis{-}style purple{-}to{-}yellow}
\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{add\_ring}\NormalTok{(}
  \AttributeTok{col\_name            =} \StringTok{"LogAbund"}\NormalTok{,}
  \AttributeTok{geom                =} \StringTok{"bar"}\NormalTok{,}
  \AttributeTok{ring\_width          =} \FloatTok{0.038}\NormalTok{,}
  \AttributeTok{ring\_offset         =} \FloatTok{0.012}\NormalTok{,}
  \AttributeTok{color\_low           =} \StringTok{"\#440154"}\NormalTok{,}
  \AttributeTok{color\_high          =} \StringTok{"\#FDE725"}\NormalTok{,}
  \AttributeTok{na\_fill             =} \StringTok{"grey95"}\NormalTok{,}
  \AttributeTok{legend\_title\_custom =} \StringTok{"Log10}\SpecialCharTok{\textbackslash{}n}\StringTok{Abund."}\NormalTok{,}
  \AttributeTok{ring\_label          =} \StringTok{"Log10 Abund."}
\NormalTok{)}

\CommentTok{\# Ring 2: Prevalence (\%) — green gradient}
\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{add\_ring}\NormalTok{(}
  \AttributeTok{col\_name            =} \StringTok{"Prevalence"}\NormalTok{,}
  \AttributeTok{geom                =} \StringTok{"bar"}\NormalTok{,}
  \AttributeTok{color\_low           =} \StringTok{"\#EDF8FB"}\NormalTok{,}
  \AttributeTok{color\_high          =} \StringTok{"\#006D2C"}\NormalTok{,}
  \AttributeTok{limits              =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{100}\NormalTok{),}
  \AttributeTok{na\_fill             =} \StringTok{"grey95"}\NormalTok{,}
  \AttributeTok{legend\_title\_custom =} \StringTok{"Prevalence}\SpecialCharTok{\textbackslash{}n}\StringTok{(\%)"}\NormalTok{,}
  \AttributeTok{ring\_label          =} \StringTok{"Prevalence"}
\NormalTok{)}

\CommentTok{\# Ring 3: Ecological Importance — orange{-}red gradient}
\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{add\_ring}\NormalTok{(}
  \AttributeTok{col\_name            =} \StringTok{"Importance"}\NormalTok{,}
  \AttributeTok{geom                =} \StringTok{"bar"}\NormalTok{,}
  \AttributeTok{color\_low           =} \StringTok{"\#FFF5EB"}\NormalTok{,}
  \AttributeTok{color\_high          =} \StringTok{"\#D94801"}\NormalTok{,}
  \AttributeTok{limits              =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{100}\NormalTok{),}
  \AttributeTok{na\_fill             =} \StringTok{"grey95"}\NormalTok{,}
  \AttributeTok{legend\_title\_custom =} \StringTok{"Importance}\SpecialCharTok{\textbackslash{}n}\StringTok{Index"}\NormalTok{,}
  \AttributeTok{ring\_label          =} \StringTok{"Importance"}
\NormalTok{)}

\CommentTok{\# Ring 4: Habitat Specificity — blue gradient}
\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{add\_ring}\NormalTok{(}
  \AttributeTok{col\_name            =} \StringTok{"Specificity"}\NormalTok{,}
  \AttributeTok{geom                =} \StringTok{"bar"}\NormalTok{,}
  \AttributeTok{color\_low           =} \StringTok{"\#F7FBFF"}\NormalTok{,}
  \AttributeTok{color\_high          =} \StringTok{"\#08306B"}\NormalTok{,}
  \AttributeTok{limits              =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{100}\NormalTok{),}
  \AttributeTok{na\_fill             =} \StringTok{"grey95"}\NormalTok{,}
  \AttributeTok{legend\_title\_custom =} \StringTok{"Specificity"}
\NormalTok{)}

\CommentTok{\# Ring 5: Network Connectivity — warm gradient}
\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{add\_ring}\NormalTok{(}
  \AttributeTok{col\_name            =} \StringTok{"Connectivity"}\NormalTok{,}
  \AttributeTok{geom                =} \StringTok{"bar"}\NormalTok{,}
  \AttributeTok{color\_low           =} \StringTok{"\#FFF7EC"}\NormalTok{,}
  \AttributeTok{color\_high          =} \StringTok{"\#7F2704"}\NormalTok{,}
  \AttributeTok{limits              =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{100}\NormalTok{),}
  \AttributeTok{na\_fill             =} \StringTok{"grey95"}\NormalTok{,}
  \AttributeTok{legend\_title\_custom =} \StringTok{"Connectivity"}
\NormalTok{)}

\CommentTok{\# {-}{-}{-}{-}{-}{-}{-}{-}{-}{-} Categorical rings {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}

\CommentTok{\# Ring 6: Habitat preference}
\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{add\_ring}\NormalTok{(}
  \AttributeTok{col\_name            =} \StringTok{"Habitat"}\NormalTok{,}
  \AttributeTok{na\_fill             =} \StringTok{"grey95"}\NormalTok{,}
  \AttributeTok{legend\_title\_custom =} \StringTok{"Habitat"}\NormalTok{,}
  \AttributeTok{categorical\_colors  =} \FunctionTok{c}\NormalTok{(}\StringTok{"IW"} \OtherTok{=} \StringTok{"\#E64B35"}\NormalTok{, }\StringTok{"CW"} \OtherTok{=} \StringTok{"\#4DBBD5"}\NormalTok{, }\StringTok{"TW"} \OtherTok{=} \StringTok{"\#00A087"}\NormalTok{)}
\NormalTok{)}

\CommentTok{\# Ring 7: Salinity preference}
\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{add\_ring}\NormalTok{(}
  \AttributeTok{col\_name            =} \StringTok{"Salinity"}\NormalTok{,}
  \AttributeTok{ring\_offset         =} \FloatTok{0.1}\NormalTok{,}
  \AttributeTok{na\_fill             =} \StringTok{"grey95"}\NormalTok{,}
  \AttributeTok{legend\_title\_custom =} \StringTok{"Salinity"}\NormalTok{,}
  \AttributeTok{categorical\_colors  =} \FunctionTok{c}\NormalTok{(}
    \StringTok{"Non{-}saline soil"} \OtherTok{=} \StringTok{"\#F39B7F"}\NormalTok{,}
    \StringTok{"Saline soil"}     \OtherTok{=} \StringTok{"\#3C5488"}
\NormalTok{  )}
\NormalTok{)}

\CommentTok{\# Ring 8: Class taxonomy}
\NormalTok{class\_counts  }\OtherTok{\textless{}{-}} \FunctionTok{sort}\NormalTok{(}\FunctionTok{table}\NormalTok{(ring\_df}\SpecialCharTok{$}\NormalTok{Class), }\AttributeTok{decreasing =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{top\_classes   }\OtherTok{\textless{}{-}} \FunctionTok{names}\NormalTok{(class\_counts)[}\DecValTok{1}\SpecialCharTok{:}\FunctionTok{min}\NormalTok{(}\DecValTok{8}\NormalTok{, }\FunctionTok{length}\NormalTok{(class\_counts))]}
\NormalTok{other\_classes }\OtherTok{\textless{}{-}} \FunctionTok{setdiff}\NormalTok{(}\FunctionTok{names}\NormalTok{(class\_counts), top\_classes)}

\NormalTok{n\_top     }\OtherTok{\textless{}{-}} \FunctionTok{length}\NormalTok{(top\_classes)}
\NormalTok{class\_pal }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}
  \FunctionTok{brewer.pal}\NormalTok{(}\FunctionTok{max}\NormalTok{(}\DecValTok{3}\NormalTok{, n\_top), }\StringTok{"Set2"}\NormalTok{)[}\DecValTok{1}\SpecialCharTok{:}\NormalTok{n\_top],}
  \FunctionTok{rep}\NormalTok{(}\StringTok{"grey80"}\NormalTok{, }\FunctionTok{length}\NormalTok{(other\_classes))}
\NormalTok{)}
\FunctionTok{names}\NormalTok{(class\_pal) }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(top\_classes, other\_classes)}

\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{add\_ring}\NormalTok{(}
  \AttributeTok{col\_name            =} \StringTok{"Class"}\NormalTok{,}
  \AttributeTok{ring\_width          =} \FloatTok{0.06}\NormalTok{,}
  \AttributeTok{ring\_offset         =} \FloatTok{0.2}\NormalTok{,}
  \AttributeTok{na\_fill             =} \StringTok{"grey95"}\NormalTok{,}
  \AttributeTok{legend\_title\_custom =} \StringTok{"Class"}\NormalTok{,}
  \AttributeTok{categorical\_colors  =}\NormalTok{ class\_pal}
\NormalTok{)}


\CommentTok{\# ============================================================================}
\CommentTok{\# Auto{-}label all rings at the fan opening}
\CommentTok{\# ============================================================================}
\CommentTok{\# Now fully automated via add\_ring\_labels() — no manual geometry table needed.}
\CommentTok{\# The ring geometry (offset, width) is automatically tracked by add\_ring().}

\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{add\_ring\_labels}\NormalTok{(}\AttributeTok{size =} \FloatTok{1.8}\NormalTok{, }\AttributeTok{color =} \StringTok{"grey25"}\NormalTok{)}


\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{add\_scale\_bar}\NormalTok{(}\AttributeTok{x =} \FloatTok{0.2}\NormalTok{, }\AttributeTok{y =} \DecValTok{0}\NormalTok{, }\AttributeTok{label =} \StringTok{"0.05"}\NormalTok{)}


\CommentTok{\# ============================================================================}
\CommentTok{\# Apply publication theme and save}
\CommentTok{\# ============================================================================}

\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{theme\_publication}\NormalTok{(}\AttributeTok{base\_size =} \DecValTok{7}\NormalTok{)}

\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{save\_plot}\NormalTok{(}
  \StringTok{"Phylotree\_addring.png"}\NormalTok{,}
...
\begin{center}\includegraphics[width=800px]{Images/Phylotree_addring} \end{center}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# {-}{-}{-}{-}{-}{-}{-}{-}{-}{-} Approach B: Equivalent batch call with vector parameters {-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\CommentTok{\# The code below produces the same 5 numeric rings as Approach A above,}
\CommentTok{\# but in a single add\_rings\_batch() call with per{-}ring vector parameters.}
\CommentTok{\#}
...
\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{add\_rings\_batch}\NormalTok{(}
  \AttributeTok{col\_names    =} \FunctionTok{c}\NormalTok{(}\StringTok{"LogAbund"}\NormalTok{, }\StringTok{"Prevalence"}\NormalTok{, }\StringTok{"Importance"}\NormalTok{, }\StringTok{"Specificity"}\NormalTok{, }\StringTok{"Connectivity"}\NormalTok{, }\StringTok{"Habitat"}\NormalTok{, }\StringTok{"Salinity"}\NormalTok{),}
  \AttributeTok{numeric\_geom =} \StringTok{"bar"}\NormalTok{,}
  \AttributeTok{ring\_offset  =} \FloatTok{0.05}\NormalTok{,}
  \AttributeTok{color\_low    =} \FunctionTok{c}\NormalTok{(}\StringTok{"\#440154"}\NormalTok{, }\StringTok{"\#EDF8FB"}\NormalTok{, }\StringTok{"\#FFF5EB"}\NormalTok{, }\StringTok{"\#F7FBFF"}\NormalTok{, }\StringTok{"\#FFF7EC"}\NormalTok{),}
  \AttributeTok{color\_high   =} \FunctionTok{c}\NormalTok{(}\StringTok{"\#FDE725"}\NormalTok{, }\StringTok{"\#006D2C"}\NormalTok{, }\StringTok{"\#D94801"}\NormalTok{, }\StringTok{"\#08306B"}\NormalTok{, }\StringTok{"\#7F2704"}\NormalTok{),}
  \AttributeTok{categorical\_palette =} \FunctionTok{list}\NormalTok{(}\AttributeTok{Habitat =} \FunctionTok{c}\NormalTok{(}\StringTok{"IW"} \OtherTok{=} \StringTok{"red"}\NormalTok{, }\StringTok{"CW"} \OtherTok{=} \StringTok{"blue"}\NormalTok{, }\StringTok{"TW"} \OtherTok{=} \StringTok{"green"}\NormalTok{), }\AttributeTok{Salinity =} \FunctionTok{c}\NormalTok{(}\StringTok{"Non{-}saline soil"} \OtherTok{=} \StringTok{"\#F39B7F"}\NormalTok{, }\StringTok{"Saline soil"} \OtherTok{=} \StringTok{"\#3C5488"}\NormalTok{)),}
  \AttributeTok{show\_legend  =} \ConstantTok{TRUE}
\NormalTok{)}
...
\NormalTok{pviz}\SpecialCharTok{$}\FunctionTok{save\_plot}\NormalTok{(}
  \StringTok{"Phylotree\_addringbatch.png"}\NormalTok{,}
  \AttributeTok{width  =} \DecValTok{16}\NormalTok{,}
  \AttributeTok{height =} \DecValTok{16}\NormalTok{,}
  \AttributeTok{dpi    =} \DecValTok{300}
\NormalTok{)}
...
\begin{center}\includegraphics[width=800px]{Images/Phylotree_addringbatch} \end{center}

\chapter{trans\_multiomics and trans\_mst class}\label{trans_multiomics-and-trans_mst-class}

\section{trans\_multiomics class}\label{trans_multiomics-class}

The \texttt{trans\_multiomics} class integrates two or more omics data sets (typically a microbiome \texttt{microtable} plus one or more other \texttt{microtable} objects such as metabolome or transcriptome) and provides a complete supervised/unsupervised multi-block workflow powered by the \texttt{mixOmics} package \citep{Rohart_mixOmics_2017}.
It wraps the most common \texttt{mixOmics} models (\texttt{block.splsda}, \texttt{block.spls}, \texttt{spls}) and adds several analysis layers that are useful in microbiome studies: cross-omics feature extraction and stability, a trans-kingdom correlation network (TkNA-style) with a bridge to the \texttt{trans\_network} class, mediation analysis (bridging to the \texttt{multimedia} package), and exporters for knowledge-driven mechanism tools (MIMOSA2 / AMON / anansi).

\begin{center}\includegraphics[width=750px]{Images/trans_multiomics_overview} \end{center}

As with the other classes of \texttt{microeco}, each method returns the object itself (invisibly), so the steps can be chained with \texttt{\$}, and every intermediate result is kept in the object, for example \texttt{data\_list}, \texttt{model}, \texttt{perf\_result}, \texttt{res\_features}, \texttt{res\_stability}, \texttt{res\_transkingdom} and \texttt{res\_mediation}.
The example below follows the whole pipeline on the two omics data sets shipped with \texttt{microeco}: prokaryotic amplicon sequencing and untargeted metabolomics from the same agricultural field experiment.
All the results shown in this section were checked with microeco 2.4.0 and mixOmics 6.36.0.

\subsection{Prerequisites}\label{prerequisites}

Install the required packages first.
\texttt{mixOmics} is on Bioconductor; \texttt{multimedia} is only needed for the mediation analysis section and can be skipped if you do not use it.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{install.packages}\NormalTok{(}\StringTok{"microeco"}\NormalTok{)}
...
\CommentTok{\# optional, only for mediation analysis}
\FunctionTok{install.packages}\NormalTok{(}\StringTok{"multimedia"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\subsection{Prepare the example data}\label{prepare-the-example-data-1}

The example uses two data sets shipped with \texttt{microeco}: \texttt{soil\_microb} (prokaryotic amplicon sequencing, 1245 taxa x 90 samples) and \texttt{soil\_metab} (untargeted metabolomics, 160 metabolites x 60 samples).
Both come from the same agricultural field experiment with crop rotation and fertilization treatments and share 60 samples across six treatment groups \citep{Liu_microeco_2021, Liu_workflow_2026}.

Each omics layer must be a \texttt{microtable} object.
The \texttt{sample\_table} must contain a grouping column (e.g.~\texttt{Group}) that will be used as the supervised response \texttt{Y} in DIABLO.
The names of the list become the block names in \texttt{mixOmics}.
For your own data, create one \texttt{microtable} per omics layer (see the microtable class section) and pass them as a named list.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(microeco)}
\FunctionTok{library}\NormalTok{(mixOmics)}
\FunctionTok{data}\NormalTok{(soil\_microb)}
\FunctionTok{data}\NormalTok{(soil\_metab)}
\CommentTok{\# soil\_microb: 1245 taxa x 90 samples; soil\_metab: 160 metabolites x 60 samples}
\CommentTok{\# both share 60 samples from the same field experiment}
\end{Highlighting}
\end{Shaded}

\subsection{Create the trans\_multiomics object}\label{create-the-trans_multiomics-object}

\texttt{trans\_multiomics\$new} takes a named list of \texttt{microtable} objects, aligns samples by intersection, and extracts the response \texttt{Y} from \texttt{group\_col} of the first \texttt{sample\_table}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_multiomics}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}
  \AttributeTok{microtables =} \FunctionTok{list}\NormalTok{(}\AttributeTok{microb =}\NormalTok{ soil\_microb, }\AttributeTok{metab =}\NormalTok{ soil\_metab),}
  \AttributeTok{group\_col   =} \StringTok{"Group"}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  Samples are aligned by \textbf{intersection} across all blocks.
  The number in the message is the total number of sample rows dropped over all blocks, so here it is 30: the 30 samples occurring only in \texttt{soil\_microb} are removed while \texttt{soil\_metab} keeps its 60 samples.
\item
  \texttt{Y} must contain at least two levels.
  You can also pass a named factor/character vector via the \texttt{group} argument to override \texttt{group\_col}.
...
\item
  The original \texttt{microtable} objects are deep-cloned inside the class so later changes to the user's data do not affect the model.
  All preprocessing therefore happens on the cloned data.
\end{itemize}

\subsection{Preprocess the data}\label{preprocess-the-data}

\texttt{preprocess\_data} reuses the \texttt{trans\_norm} machinery to apply a normalization method to each block.
For compositional microbiome/metabolome data, \textbf{CLR (centered log-ratio) is recommended} because it removes the compositional constraint and produces values whose sum is zero within each sample.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{preprocess\_data}\NormalTok{(}
  \AttributeTok{method      =} \StringTok{"clr"}\NormalTok{,      }\CommentTok{\# CLR transformation}
  \AttributeTok{pseudocount =} \DecValTok{1}\NormalTok{,          }\CommentTok{\# added before log for zero values}
  \AttributeTok{filter\_freq =} \FloatTok{0.2}         \CommentTok{\# keep features present in at least 20\% of samples}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}

The feature filtering is delegated to \texttt{microtable\$filter\_taxa}, so the usual filtering messages of the microtable class are printed first.
...
The processed matrices (rows are samples, columns are features) are stored in \texttt{t1\$data\_list} and will be passed to \texttt{mixOmics}.

The argument \texttt{method} accepts any value supported by \texttt{trans\_norm\$norm} (e.g.~\texttt{"clr"}, \texttt{"rclr"}, \texttt{"css"}, \texttt{"tss"}, \texttt{"log"}).
For \texttt{filter\_freq\ \textless{}\ 1}, the threshold is computed as \texttt{round(n\_samples\ *\ filter\_freq)} (here \texttt{0.2\ *\ 60\ =\ 12}); features observed in fewer samples than this threshold are removed.
Pass an integer \textgreater= 1 if you prefer an absolute count threshold.
\texttt{filter\_thres} adds a relative abundance threshold on the same call.
Setting \texttt{filter\_freq\ =\ 1} and \texttt{filter\_thres\ =\ NULL} keeps all features.

\subsection{Set the design matrix}\label{set-the-design-matrix}

The design matrix specifies how strongly two blocks should be correlated.
A small off-diagonal weight (e.g.~\texttt{0.1}) tends to favour classification accuracy, whereas \texttt{1} maximises the covariance between blocks and is more useful for exploratory association analysis \citep{Singh_DIABLO_2019}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{set\_design}\NormalTok{(}\AttributeTok{weight =} \FloatTok{0.1}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{design}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{design\_full }\OtherTok{\textless{}{-}} \FunctionTok{matrix}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{0}\NormalTok{), }\AttributeTok{nrow =} \DecValTok{2}\NormalTok{,}
  \AttributeTok{dimnames =} \FunctionTok{list}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"microb"}\NormalTok{, }\StringTok{"metab"}\NormalTok{), }\FunctionTok{c}\NormalTok{(}\StringTok{"microb"}\NormalTok{, }\StringTok{"metab"}\NormalTok{)))}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{set\_design}\NormalTok{(}\AttributeTok{design =}\NormalTok{ design\_full)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{set\_design}\NormalTok{(}\AttributeTok{weight =} \FloatTok{0.1}\NormalTok{)  }\CommentTok{\# reset to the default}
\end{Highlighting}
\end{Shaded}

\subsection{Supervised integration (block.splsda)}\label{supervised-integration-block.splsda}

DIABLO \citep{Singh_DIABLO_2019} extends sparse PLS-DA to multiple data sets and selects correlated features across blocks that best discriminate the groups in \texttt{Y}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{run\_diablo}\NormalTok{(}
  \AttributeTok{ncomp  =} \DecValTok{2}\NormalTok{,}
  \AttributeTok{keepX  =} \FunctionTok{list}\NormalTok{(}\AttributeTok{microb =} \FunctionTok{c}\NormalTok{(}\DecValTok{10}\NormalTok{, }\DecValTok{10}\NormalTok{), }\AttributeTok{metab =} \FunctionTok{c}\NormalTok{(}\DecValTok{15}\NormalTok{, }\DecValTok{15}\NormalTok{))}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}

The fitted model is stored in \texttt{t1\$model}.
Its class is \texttt{block.splsda} and the latent components live on \texttt{comp1} and \texttt{comp2}.
Choosing \texttt{keepX} requires balancing interpretability (small \texttt{keepX} -\textgreater{} sparse biomarker panels) and classification performance (larger \texttt{keepX} retains more signal).
The rule of thumb ``5-20 features per block per component'' works well for tens of samples per group.
The tuning section below shows how to choose \texttt{keepX} automatically.

\subsection{Performance evaluation}\label{performance-evaluation}

\texttt{eval\_performance} runs \texttt{mixOmics::perf} to estimate the classification error rate via cross-validation.
\texttt{perf} aggregates the per-block predictions with four strategies: \texttt{AveragedPredict} and \texttt{WeightedPredict} average the predicted classes over the blocks, while \texttt{MajorityVote} and \texttt{WeightedVote} take the majority (or weighted majority) class over the blocks.
The error rate of the two voting strategies is computed for each of the three distance metrics (\texttt{max.dist}, \texttt{centroids.dist}, \texttt{mahalanobis.dist}), so \texttt{MajorityVote.error.rate} and \texttt{WeightedVote.error.rate} are lists containing one matrix per distance metric, whereas \texttt{AveragedPredict.error.rate} and \texttt{WeightedPredict.error.rate} are single matrices (no distance is involved).
Every one of those matrices has one row per predicted class plus an \texttt{Overall.ER} and an \texttt{Overall.BER} row, and one column per component.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{eval\_performance}\NormalTok{(}\AttributeTok{method =} \StringTok{"Mfold"}\NormalTok{, }\AttributeTok{folds =} \DecValTok{5}\NormalTok{, }\AttributeTok{nrepeat =} \DecValTok{1}\NormalTok{, }\AttributeTok{seed =} \DecValTok{123}\NormalTok{)}
\CommentTok{\# overall balanced error rate (BER) across prediction strategies}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{perf\_result}\SpecialCharTok{$}\NormalTok{MajorityVote.error.rate}\SpecialCharTok{$}\NormalTok{centroids.dist[}\StringTok{"Overall.BER"}\NormalTok{, ]}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{perf\_result}\SpecialCharTok{$}\NormalTok{WeightedVote.error.rate}\SpecialCharTok{$}\NormalTok{centroids.dist[}\StringTok{"Overall.BER"}\NormalTok{, ]}
\CommentTok{\# per{-}block error rate (which block is more informative?)}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{perf\_result}\SpecialCharTok{$}\NormalTok{error.rate}\SpecialCharTok{$}\NormalTok{microb}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{perf\_result}\SpecialCharTok{$}\NormalTok{error.rate}\SpecialCharTok{$}\NormalTok{metab}
\end{Highlighting}
\end{Shaded}

With our example data (\texttt{folds\ =\ 5,\ nrepeat\ =\ 1,\ seed\ =\ 123}) the WeightedVote BER drops from \texttt{0.533} (comp1) to \texttt{0.400} (comp2, centroids.dist) and further to \texttt{0.333} (mahalanobis.dist), while MajorityVote remains high because the majority vote is conservative on a six-class problem.
BER values are cross-validated estimates, so they change with \texttt{folds}, \texttt{nrepeat} and \texttt{seed}; interpret the trend rather than the exact numbers.
In practice, \textbf{WeightedVote or AveragedPredict} are usually more informative than MajorityVote when the class sizes are balanced.

\texttt{error.rate} is a list with one matrix per block, each giving the error rate of that single block (rows: components, columns: distance metrics).
It complements the four aggregated strategies above and helps to decide which block carries the classification signal.
The cross-validation takes about 20 s for \texttt{folds\ =\ 5,\ nrepeat\ =\ 10} on 60 samples.

Always set \texttt{seed} for reproducible cross-validation; the value is forwarded to \texttt{mixOmics} when supported (\textgreater= 6.3.2) and otherwise applied via \texttt{set.seed()}.

\subsection{Feature selection stability}\label{feature-selection-stability}

The features selected by a single DIABLO fit are subject to sampling noise.
\texttt{get\_stability} averages the selection frequency across all folds and repeats of the cross-validation performed by \texttt{eval\_performance}, returning a tidy \texttt{data.frame} with columns \texttt{Feature}, \texttt{Block}, \texttt{Comp}, and \texttt{Stability} in \texttt{{[}0,\ 1{]}}.
A stability close to \texttt{1} means the feature is robust across resamples; low stability suggests the feature may be an artefact of a particular data split \citep{Liquet_stability_2012}.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# eval\_performance must be called first with folds/nrepeat \textgreater{} 1 for a meaningful stability}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{eval\_performance}\NormalTok{(}\AttributeTok{folds =} \DecValTok{5}\NormalTok{, }\AttributeTok{nrepeat =} \DecValTok{10}\NormalTok{, }\AttributeTok{seed =} \DecValTok{123}\NormalTok{)}
\NormalTok{stab }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{get\_stability}\NormalTok{()}
\FunctionTok{head}\NormalTok{(stab, }\DecValTok{10}\NormalTok{)}
\CommentTok{\# robust biomarker candidates on the first component}
\FunctionTok{subset}\NormalTok{(stab, Comp }\SpecialCharTok{==} \DecValTok{1} \SpecialCharTok{\&}\NormalTok{ Stability }\SpecialCharTok{\textgreater{}=} \FloatTok{0.8}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
With this example (\texttt{folds\ =\ 5,\ nrepeat\ =\ 10,\ seed\ =\ 123}), the subset on \texttt{Comp\ ==\ 1\ \&\ Stability\ \textgreater{}=\ 0.8} keeps 15 features on comp1.
Increase \texttt{nrepeat} (e.g.~\texttt{50}) for publication-grade stability; \texttt{nrepeat\ =\ 1} only shows whether the feature survives all folds of a single split.

Note that \texttt{get\_stability} needs the selection frequency table that \texttt{perf} stores in \texttt{perf\_result\$features\$stable}, so it requires a sparse model fitted with \texttt{keepX} and evaluated with \texttt{eval\_performance()}.

\subsection{Hyperparameter tuning (keepX)}\label{hyperparameter-tuning-keepx}

\texttt{mixOmics::tune.block.splsda} searches an optimal \texttt{keepX} per block per component via cross-validation.
The class provides a thin wrapper \texttt{tune\_model}.
When \texttt{test\_keepX} is not supplied, an adaptive grid of 5\%, 10\% and 20\% of the number of features per block is used.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{tune\_model}\NormalTok{(}
  \AttributeTok{ncomp       =} \DecValTok{2}\NormalTok{,}
  \AttributeTok{test\_keepX  =} \FunctionTok{list}\NormalTok{(}\AttributeTok{microb =} \FunctionTok{c}\NormalTok{(}\DecValTok{5}\NormalTok{, }\DecValTok{10}\NormalTok{, }\DecValTok{20}\NormalTok{), }\AttributeTok{metab =} \FunctionTok{c}\NormalTok{(}\DecValTok{5}\NormalTok{, }\DecValTok{10}\NormalTok{, }\DecValTok{20}\NormalTok{)),}
  \AttributeTok{folds       =} \DecValTok{5}\NormalTok{,}
  \AttributeTok{nrepeat     =} \DecValTok{1}\NormalTok{,}
  \AttributeTok{seed        =} \DecValTok{123}
\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{best\_keepX}
\end{Highlighting}
\end{Shaded}
...
The optimal \texttt{keepX} is stored in \texttt{t1\$best\_keepX} and can be reused to refit the model.
As a refit replaces every panel computed from the object, the refitted model is built in a separate copy here, so that the results in the following sections remain reproducible with the initial \texttt{keepX} of the supervised integration section.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# refit with the tuned keepX in a copy of the object}
\NormalTok{t1\_tuned }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{clone}\NormalTok{(}\AttributeTok{deep =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{t1\_tuned}\SpecialCharTok{$}\FunctionTok{run\_diablo}\NormalTok{(}\AttributeTok{ncomp =} \DecValTok{2}\NormalTok{, }\AttributeTok{keepX =}\NormalTok{ t1}\SpecialCharTok{$}\NormalTok{best\_keepX)}
\NormalTok{t1\_tuned}\SpecialCharTok{$}\FunctionTok{get\_features}\NormalTok{(}\AttributeTok{comp =} \DecValTok{1}\NormalTok{)   }\CommentTok{\# 40 features on comp1 (20 + 20) instead of 25}
\end{Highlighting}
\end{Shaded}

Run the same refit directly on \texttt{t1} (i.e.~without the copy) if you want the tuned model to become the working model; the feature panels and the correlation matrix shown below will then change accordingly.
...
\tightlist
\item
  \texttt{mixOmics} does not provide a tune function for the unsupervised \texttt{block.spls}, so \texttt{method} must be \texttt{"block.splsda"} (which requires \texttt{Y}).
\item
  \texttt{folds} must not exceed the smallest class size; with ten samples per group \texttt{folds\ \textless{}=\ 10}.
\item
  The grid search cost grows multiplicatively with the number of candidate values per block.
  A 3 x 3 grid with \texttt{folds\ =\ 5}, \texttt{nrepeat\ =\ 1} and 60 samples takes \textasciitilde10 s on a laptop; increase \texttt{nrepeat} only when you really need it.
\end{itemize}

\subsection{Feature extraction and cross-block correlation}\label{feature-extraction-and-cross-block-correlation}

After a DIABLO fit, \texttt{get\_features} extracts the selected variables on each component and returns a tidy \texttt{data.frame} sorted by the absolute loading.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{features }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{get\_features}\NormalTok{(}\AttributeTok{comp =} \DecValTok{1}\NormalTok{)}
\FunctionTok{head}\NormalTok{(features)}
\FunctionTok{table}\NormalTok{(features}\SpecialCharTok{$}\NormalTok{Block)}
\CommentTok{\# comp = 2}
\NormalTok{features2 }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{get\_features}\NormalTok{(}\AttributeTok{comp =} \DecValTok{2}\NormalTok{)}
\FunctionTok{head}\NormalTok{(features2, }\DecValTok{6}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\texttt{get\_correlation} returns the cross-block similarity matrix of the selected features on a component, thresholded by an absolute cutoff.
The result is stored in \texttt{t1\$res\_correlation} and can be exported for downstream network or pathway analyses.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cm }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{get\_correlation}\NormalTok{(}\AttributeTok{comp =} \DecValTok{1}\NormalTok{, }\AttributeTok{cutoff =} \FloatTok{0.7}\NormalTok{)}
\FunctionTok{dim}\NormalTok{(cm)   }\CommentTok{\# 10 rows x 15 cols in this example (microb x metab)}
\FunctionTok{range}\NormalTok{(cm)}
\end{Highlighting}
\end{Shaded}

Note: in \texttt{mixOmics} \textgreater= 6.x the returned similarity matrix has \texttt{colnames} but \texttt{rownames} may be \texttt{NULL}; the rows correspond to the selected features of the first block and the order follows \texttt{mixOmics::selectVar}.
The column names carry the block name as a suffix (e.g.~\texttt{palmitoleic\ acid\_metab}), because \texttt{mixOmics::network} labels the variables by block.
Restore the names manually if you need them:

\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{if}\NormalTok{ (}\FunctionTok{is.null}\NormalTok{(}\FunctionTok{rownames}\NormalTok{(cm))) \{}
\NormalTok{  sel }\OtherTok{\textless{}{-}}\NormalTok{ mixOmics}\SpecialCharTok{::}\FunctionTok{selectVar}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{model, }\AttributeTok{comp =} \DecValTok{1}\NormalTok{, }\AttributeTok{block =} \StringTok{"microb"}\NormalTok{)}
  \FunctionTok{rownames}\NormalTok{(cm) }\OtherTok{\textless{}{-}}\NormalTok{ sel}\SpecialCharTok{$}\NormalTok{microb}\SpecialCharTok{$}\NormalTok{name}
\NormalTok{\}}
\CommentTok{\# remove the "\_\textless{}block\textgreater{}" suffix from the column names}
\FunctionTok{colnames}\NormalTok{(cm) }\OtherTok{\textless{}{-}} \FunctionTok{sub}\NormalTok{(}\StringTok{"\_metab$"}\NormalTok{, }\StringTok{""}\NormalTok{, }\FunctionTok{colnames}\NormalTok{(cm))}
\end{Highlighting}
\end{Shaded}

\texttt{mixOmics::network} opens a graphics device internally even though no plot is requested, so a stray \texttt{Rplots.pdf} may appear in the working directory; it can simply be deleted.

\subsection{Visualizations}\label{visualizations}

\subsubsection{Sample projection}\label{sample-projection}

\texttt{plot\_samples} calls \texttt{mixOmics::plotIndiv} with \texttt{style\ =\ "ggplot2"} and returns a \texttt{ggplot} object that can be customised with \texttt{+\ theme\_xx()}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p1 }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{plot\_samples}\NormalTok{(}\AttributeTok{comp =} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{), }\AttributeTok{blocks =} \StringTok{"average"}\NormalTok{, }\AttributeTok{ellipse =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{p1}
\NormalTok{p1b }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{plot\_samples}\NormalTok{(}\AttributeTok{comp =} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{), }\AttributeTok{blocks =} \StringTok{"microb"}\NormalTok{, }\AttributeTok{ellipse =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{p1b}
\end{Highlighting}
\end{Shaded}

\texttt{blocks\ =\ "average"} plots the consensus components (weighted average across all blocks), which is the standard view to check whether the blocks separate the groups jointly.
Pick a single block name/index to inspect the within-block projection.

\begin{center}\includegraphics[width=650px]{Images/tmo_plot_samples} \end{center}

\begin{center}\includegraphics[width=650px]{Images/tmo_plot_samples_microb} \end{center}

\subsubsection{Variable loadings}\label{variable-loadings}

\texttt{plot\_loadings} calls \texttt{mixOmics::plotLoadings} and draws a bar plot of the loadings on the current graphics device.
The function returns the loading values as a \texttt{list} (one element per block, each a \texttt{data.frame} with the columns \texttt{importance} and \texttt{names}), \textbf{not} a \texttt{ggplot} object, so use it directly without \texttt{ggsave()}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_loadings}\NormalTok{(}\AttributeTok{comp =} \DecValTok{1}\NormalTok{, }\AttributeTok{block =} \StringTok{"microb"}\NormalTok{)}
\NormalTok{loadings }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{plot\_loadings}\NormalTok{(}\AttributeTok{comp =} \DecValTok{1}\NormalTok{, }\AttributeTok{block =} \StringTok{"metab"}\NormalTok{)}
\FunctionTok{head}\NormalTok{(loadings}\SpecialCharTok{$}\NormalTok{metab)  }\CommentTok{\# the loading values}
\end{Highlighting}
\end{Shaded}

The \texttt{block} argument must always be given for the multi-block models; the default \texttt{NULL} is not resolved to the first block and raises an error.

\begin{center}\includegraphics[width=650px]{Images/tmo_plot_loadings} \end{center}

\subsubsection{DIABLO diagnostic plot}\label{diablo-diagnostic-plot}

\texttt{plot\_diablo} shows the correlation between the latent components of each block on the specified \texttt{ncomp}.
The lower panel gives the Pearson correlation coefficient and the upper panel the corresponding scatter plot, so the plot is used to check whether DIABLO succeeded in maximising the correlation between the components of the different blocks \citep{Singh_DIABLO_2019}.
Note that the design weight is a link weight and not the expected correlation value: the paired components of the two blocks are correlated by \texttt{0.75} and \texttt{0.61} here with \texttt{weight\ =\ 0.1}, i.e.~well above the design value.
Raising the weight to \texttt{1} enforces the link more strongly and usually increases the correlation of the paired components.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_diablo}\NormalTok{(}\AttributeTok{ncomp =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=650px]{Images/tmo_plot_diablo} \end{center}

\subsubsection{Relevance network and circos plot}\label{relevance-network-and-circos-plot}

\texttt{plot\_cor\_network} displays the cross-block correlation network of the selected features, thresholded by \texttt{cutoff}.
\texttt{plot\_circos} shows the same information in a circos layout.
Both rely on the underlying \texttt{mixOmics::network} and \texttt{mixOmics::circosPlot}; pass additional arguments through \texttt{...} when needed.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_cor\_network}\NormalTok{(}\AttributeTok{comp =} \DecValTok{1}\NormalTok{, }\AttributeTok{cutoff =} \FloatTok{0.7}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_circos}\NormalTok{(}\AttributeTok{comp =} \DecValTok{1}\NormalTok{, }\AttributeTok{cutoff =} \FloatTok{0.7}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

The default \texttt{cutoff\ =\ 0.7} requires that at least some of the selected features are correlated above \texttt{0.7}; \texttt{mixOmics} stops when the cutoff exceeds the largest value of the similarity matrix.
Check \texttt{range(t1\$get\_correlation(comp\ =\ 1))} (here \texttt{-0.85} to \texttt{0.78}) and lower the cutoff when the panel is only weakly correlated.

\begin{center}\includegraphics[width=650px]{Images/tmo_plot_cor_network} \end{center}

\begin{center}\includegraphics[width=650px]{Images/tmo_plot_circos} \end{center}

\subsubsection{Clustered image map}\label{clustered-image-map}

\texttt{plot\_heatmap} calls \texttt{mixOmics::cimDiablo} to draw a clustered image map (CIM) of the selected features across samples.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_heatmap}\NormalTok{(}\AttributeTok{comp =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=650px]{Images/tmo_plot_heatmap} \end{center}

\subsubsection{Per-class AUC}\label{per-class-auc}

\texttt{cal\_auroc} runs \texttt{mixOmics::auroc} for each block and each component, returning per-class AUCs and a \texttt{ggplot} ROC plot.
The function detects the correct \texttt{roc.comp} vs \texttt{comp} argument name across \texttt{mixOmics} versions, so it is safe to use with both 6.3.x and 6.4.x.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{au }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{cal\_auroc}\NormalTok{(}\AttributeTok{comp =} \DecValTok{1}\NormalTok{)}
\NormalTok{au}\SpecialCharTok{$}\NormalTok{metab  }\CommentTok{\# AUC for the metabolome block on each class (vs all others)}
\end{Highlighting}
\end{Shaded}
...
The AUC tables of all blocks are the list elements named after the blocks (\texttt{au\$microb}, \texttt{au\$metab}), one table per component.
The ROC plot is stored separately as \texttt{au\$graph.\textless{}block\textgreater{}} (a list with one \texttt{ggplot} per component).
\texttt{mixOmics::auroc} only builds the plot of the first block by default, so use \texttt{au\$graph.microb}; pass \texttt{roc.block\ =\ 2} to \texttt{cal\_auroc} to get the plot of the second block instead.

\subsection{Unsupervised alternatives}\label{unsupervised-alternatives}

When no grouping is available, or when the goal is to explore the covariance structure between two data sets rather than to classify samples, use the unsupervised \texttt{block.spls} or \texttt{spls} engines.

\texttt{run\_spls} performs multi-block sparse PLS.
Because \texttt{mixOmics::block.spls} requires a numeric response matrix, one block must be designated as the response via \texttt{Y\_block} (passed internally as \texttt{indY}).
In that case \texttt{keepX} must be supplied for \textbf{all} blocks (including the response block) because \texttt{mixOmics} ignores \texttt{keepY} when \texttt{indY} is used.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t2 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_multiomics}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}
  \AttributeTok{microtables =} \FunctionTok{list}\NormalTok{(}\AttributeTok{microb =}\NormalTok{ soil\_microb, }\AttributeTok{metab =}\NormalTok{ soil\_metab),}
  \AttributeTok{group       =} \ConstantTok{NULL}\NormalTok{,}
  \AttributeTok{group\_col   =} \ConstantTok{NULL}
\NormalTok{)}
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{preprocess\_data}\NormalTok{(}\AttributeTok{method =} \StringTok{"clr"}\NormalTok{, }\AttributeTok{filter\_freq =} \FloatTok{0.2}\NormalTok{)}
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{run\_spls}\NormalTok{(}
  \AttributeTok{ncomp   =} \DecValTok{2}\NormalTok{,}
  \AttributeTok{Y\_block =} \StringTok{"metab"}\NormalTok{,}
  \AttributeTok{mode    =} \StringTok{"regression"}\NormalTok{,}
  \AttributeTok{keepX   =} \FunctionTok{list}\NormalTok{(}\AttributeTok{microb =} \FunctionTok{c}\NormalTok{(}\DecValTok{10}\NormalTok{, }\DecValTok{10}\NormalTok{), }\AttributeTok{metab =} \FunctionTok{c}\NormalTok{(}\DecValTok{10}\NormalTok{, }\DecValTok{10}\NormalTok{))}
\NormalTok{)}
\NormalTok{p }\OtherTok{\textless{}{-}}\NormalTok{ t2}\SpecialCharTok{$}\FunctionTok{plot\_samples}\NormalTok{(}\AttributeTok{comp =} \FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{2}\NormalTok{))  }\CommentTok{\# blocks = "average" may fall back to a single block on block.spls}
\FunctionTok{print}\NormalTok{(p)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=650px]{Images/tmo_plot_samples_spls} \end{center}

For a strictly two-block association, use \texttt{run\_spls\_single} which wraps \texttt{mixOmics::spls}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t3 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_multiomics}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}
  \AttributeTok{microtables =} \FunctionTok{list}\NormalTok{(}\AttributeTok{microb =}\NormalTok{ soil\_microb, }\AttributeTok{metab =}\NormalTok{ soil\_metab),}
  \AttributeTok{group\_col   =} \StringTok{"Group"}
\NormalTok{)}
\NormalTok{t3}\SpecialCharTok{$}\FunctionTok{preprocess\_data}\NormalTok{(}\AttributeTok{method =} \StringTok{"clr"}\NormalTok{, }\AttributeTok{filter\_freq =} \FloatTok{0.2}\NormalTok{)}
\NormalTok{t3}\SpecialCharTok{$}\FunctionTok{run\_spls\_single}\NormalTok{(}
  \AttributeTok{X\_block =} \StringTok{"microb"}\NormalTok{, }\AttributeTok{Y\_block =} \StringTok{"metab"}\NormalTok{,}
  \AttributeTok{ncomp   =} \DecValTok{2}\NormalTok{,}
  \AttributeTok{keepX   =} \FunctionTok{c}\NormalTok{(}\DecValTok{10}\NormalTok{, }\DecValTok{10}\NormalTok{), }\AttributeTok{keepY =} \FunctionTok{c}\NormalTok{(}\DecValTok{10}\NormalTok{, }\DecValTok{10}\NormalTok{)}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}

On these unsupervised models, \texttt{get\_features}, \texttt{get\_correlation}, \texttt{plot\_samples} and \texttt{plot\_cor\_network} work as described above (on the two-block \texttt{spls} model, \texttt{plot\_samples} warns that \texttt{blocks} is ignored because the model is not multiblock).
\texttt{plot\_loadings} also works: on a \texttt{block.spls} object use the original block names (\texttt{microb}, \texttt{metab}), but on the two-block \texttt{spls} model \texttt{mixOmics} internally renames the blocks to \texttt{X} and \texttt{Y}, so you must pass \texttt{block\ =\ "X"} or \texttt{block\ =\ "Y"}.
However, \texttt{plot\_circos}, \texttt{plot\_heatmap} and \texttt{plot\_diablo} only support the supervised \texttt{block.splsda} model: their underlying \texttt{mixOmics} functions (\texttt{circosPlot}, \texttt{cimDiablo}, \texttt{plotDiablo}) expect a supervised object and fail on unsupervised models.

\subsection{Trans-kingdom network (TkNA)}\label{trans-kingdom-network-tkna}

Beyond block-level integration, \texttt{trans\_multiomics} can build a \textbf{bipartite} cross-omics association network following the Transkingdom Network Analysis (TkNA) framework.
The pipeline has three steps \citep{Newman_TkNA_2024}:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Differential feature filtering} within each block (optional).
  Features whose adjusted p-value is above \texttt{diff\_p} (Kruskal-Wallis, ANOVA or Wilcoxon, depending on \texttt{diff\_test}) are dropped.
  This step removes uninformative background noise before computing associations.
\item
  \textbf{Cross-block correlation} between every remaining feature of one block and every remaining feature of the other block(s).
  Spearman is recommended for CLR-transformed compositional data.
  Pearson is also available.
  A \texttt{covariates} \texttt{data.frame} (with rownames matching the sample names) is regressed out from every feature before the correlation, mirroring the MaAsLin2 confounder handling \citep{Mallick_MaAsLin2_2021}.
\item
  \textbf{Bipartite Betweenness Centrality (BiBC)} quantifies how often a node lies on the shortest path between two nodes of different blocks.
  Features with high BiBC are flagged as \textbf{cross-omics hubs} (top \texttt{hub\_quantile} among nodes with BiBC \textgreater{} 0).
\end{enumerate}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_transkingdom}\NormalTok{(}
  \AttributeTok{corr\_method =} \StringTok{"spearman"}\NormalTok{,}
  \AttributeTok{corr\_thres  =} \FloatTok{0.6}\NormalTok{,}
  \AttributeTok{p\_thres     =} \FloatTok{0.05}\NormalTok{,}
  \AttributeTok{p\_adjust    =} \StringTok{"BH"}\NormalTok{,}
  \AttributeTok{diff\_test   =} \StringTok{"kruskal"}\NormalTok{,}
  \AttributeTok{diff\_p      =} \FloatTok{0.05}
\NormalTok{)}
\CommentTok{\# node table: Feature, Block, Diff\_p, BiBC, Hub}
\FunctionTok{head}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_transkingdom}\SpecialCharTok{$}\NormalTok{nodes)}
\CommentTok{\# edge table: From, To, Corr, P, P\_adj, weight}
\FunctionTok{head}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_transkingdom}\SpecialCharTok{$}\NormalTok{edges)}
\CommentTok{\# cross{-}omics hub candidates}
\FunctionTok{subset}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_transkingdom}\SpecialCharTok{$}\NormalTok{nodes, Hub)}
\end{Highlighting}
\end{Shaded}
...
The \texttt{weight} column of the edge table is the distance-like weight \texttt{1\ -\ \textbar{}Corr\textbar{}} used internally for the shortest paths of the BiBC computation; it is not the association strength (use \texttt{Corr} for that).

\texttt{plot\_transkingdom} draws the bipartite network with \texttt{ggplot2} (Fruchterman-Reingold layout weighted by \texttt{\textbar{}correlation\textbar{}}, or a circle layout):

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_transkingdom}\NormalTok{(}\AttributeTok{label =} \StringTok{"hub"}\NormalTok{)         }\CommentTok{\# only label hubs}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_transkingdom}\NormalTok{(}\AttributeTok{label =} \StringTok{"all"}\NormalTok{, }\AttributeTok{layout =} \StringTok{"circle"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=700px]{Images/tmo_transkingdom} \end{center}

Regressing out confounders (e.g.~sequencing batch, host age) reduces spurious cross-layer edges.
The \texttt{covariates} argument must be a \texttt{data.frame} whose rownames match the sample names:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cov\_df }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}
  \AttributeTok{batch =} \FunctionTok{factor}\NormalTok{(}\FunctionTok{rep}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"B1"}\NormalTok{, }\StringTok{"B2"}\NormalTok{), }\AttributeTok{length.out =} \FunctionTok{nrow}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{data\_list}\SpecialCharTok{$}\NormalTok{microb))),}
  \AttributeTok{row.names =} \FunctionTok{rownames}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{data\_list}\SpecialCharTok{$}\NormalTok{microb)}
\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_transkingdom}\NormalTok{(}\AttributeTok{covariates =}\NormalTok{ cov\_df, }\AttributeTok{corr\_thres =} \FloatTok{0.5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  The network is a \textbf{data-driven association network}, not a causal network.
  Prioritised hubs/edges should be validated with independent cohorts or perturbation experiments.
\item
  If \texttt{diff\_test} removes \textbf{all} features from a block, the block is automatically retained with a warning.
  Lower \texttt{diff\_p} or change the test to keep enough candidates.
\item
  If no edge passes the \texttt{corr\_thres} / \texttt{p\_thres} thresholds, a warning is issued and an empty graph is returned.
\item
  \texttt{diff\_test\ =\ "wilcox"} requires exactly two groups in \texttt{Y}; use \texttt{"kruskal"} or \texttt{"aov"} for multi-group data.
\end{itemize}

\subsection{Bridge to the trans\_network class}\label{bridge-to-the-trans_network-class}

\texttt{convert\_transkingdom} converts the trans-kingdom network into a \texttt{trans\_network} object, so that the whole downstream network toolkit of the trans\_network class becomes available: module detection (\texttt{cal\_module}), node/edge property tables (\texttt{get\_node\_table} / \texttt{get\_edge\_table}), network topological attributes (\texttt{cal\_network\_attr}), sub-network extraction (\texttt{subset\_network}), eigengene analysis (\texttt{cal\_eigen}), random network comparison (\texttt{random\_network}) and network visualization (\texttt{plot\_network}).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{net1 }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{convert\_transkingdom}\NormalTok{()}
\NormalTok{net1}\SpecialCharTok{$}\FunctionTok{cal\_module}\NormalTok{()}
\NormalTok{net1}\SpecialCharTok{$}\FunctionTok{get\_node\_table}\NormalTok{(}\AttributeTok{node\_roles =} \ConstantTok{TRUE}\NormalTok{)}
\FunctionTok{head}\NormalTok{(net1}\SpecialCharTok{$}\NormalTok{res\_node\_table)}
\end{Highlighting}
\end{Shaded}

The conversion follows the conventions of the correlation networks created by \texttt{trans\_network\$cal\_network}: the edge weight is re-defined as the absolute correlation and the edge label as the correlation sign (``+''/``-''), while the node attributes \texttt{Block}, \texttt{Diff\_p}, \texttt{BiBC} and \texttt{Hub} are kept, so they appear in the node table together with the usual network attributes (degree, betweenness, module, z, p, \ldots).
The original (non-transformed) abundances of the network features are taken from \texttt{dataset\_list} to fill \texttt{data\_abund}, \texttt{data\_relabund} and \texttt{sample\_table}, and the \texttt{tax\_table} slots of all blocks are merged, which is convenient for a microbe-metabolite network.
The returned object can then be plotted with the trans\_network visualizations, for example:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{net1}\SpecialCharTok{$}\FunctionTok{cal\_network\_attr}\NormalTok{()}
\NormalTok{net1}\SpecialCharTok{$}\NormalTok{res\_network\_attr}
\NormalTok{net1}\SpecialCharTok{$}\FunctionTok{plot\_network}\NormalTok{(}\AttributeTok{method =} \StringTok{"ggraph"}\NormalTok{, }\AttributeTok{node\_color =} \StringTok{"module"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Note that the taxonomy-dependent functions of \texttt{trans\_network}, e.g.~\texttt{cal\_sum\_links} and \texttt{plot\_sum\_links}, need a taxonomic column shared by all the involved blocks; for a bipartite cross-omics network (e.g.~microbe vs metabolite) such a column usually does not exist, so those functions are not applicable unless the blocks use the same feature annotation.
...
\subsection{Mediation analysis (multimedia bridge)}\label{mediation-analysis-multimedia-bridge}

High-dimensional mediation analysis asks whether the effect of an \textbf{exposure} on an \textbf{outcome} is transmitted through a set of \textbf{mediators}.
The \texttt{multimedia} package implements a sparse, high-dimensional mediation workflow \citep{Jiang_multimedia_2024} that fits naturally on top of \texttt{trans\_multiomics}.
A typical microbiome question is ``do specific metabolites mediate the effect of a microbial community on a host phenotype?''.

The \texttt{run\_mediation} method:

\begin{itemize}
\tightlist
\item
  takes the exposure, mediator and outcome as either \texttt{microtable} blocks (named in \texttt{data\_list\_names}) or low-dimensional named vectors/factors,
\item
  reuses the CLR-transformed matrices from \texttt{t1\$data\_list},
\item
  optionally restricts the analysis to features previously selected by \texttt{get\_features} via \texttt{features},
\item
  caps the number of features per block with \texttt{max\_features} to keep the regression tractable,
\item
  supports three model engines: \texttt{"glmnet"} (default, recommended for high-dimensional blocks), \texttt{"lm"} (low-dimensional exposure only) and \texttt{"rf"}.
\end{itemize}

\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{if}\NormalTok{ (}\SpecialCharTok{!}\FunctionTok{requireNamespace}\NormalTok{(}\StringTok{"multimedia"}\NormalTok{, }\AttributeTok{quietly =} \ConstantTok{TRUE}\NormalTok{)) \{}
  \FunctionTok{install.packages}\NormalTok{(}\StringTok{"multimedia"}\NormalTok{)}
\NormalTok{\}}
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{123}\NormalTok{)}
\CommentTok{\# simulate a continuous phenotype (replace with your real phenotype vector)}
\NormalTok{pheno }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\NormalTok{data\_list}\SpecialCharTok{$}\NormalTok{metab[, }\DecValTok{1}\NormalTok{] }\SpecialCharTok{+} \FunctionTok{rnorm}\NormalTok{(}\FunctionTok{nrow}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{data\_list}\SpecialCharTok{$}\NormalTok{metab), }\AttributeTok{sd =} \FloatTok{0.5}\NormalTok{)}
\FunctionTok{names}\NormalTok{(pheno) }\OtherTok{\textless{}{-}} \FunctionTok{rownames}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{data\_list}\SpecialCharTok{$}\NormalTok{metab)}

\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{run\_mediation}\NormalTok{(}
  \AttributeTok{exposure     =} \StringTok{"microb"}\NormalTok{,        }\CommentTok{\# microbial block (high{-}dimensional)}
  \AttributeTok{mediator     =} \StringTok{"metab"}\NormalTok{,         }\CommentTok{\# metabolite block}
  \AttributeTok{outcome      =}\NormalTok{ pheno,           }\CommentTok{\# named vector matching the samples}
  \AttributeTok{model        =} \StringTok{"glmnet"}\NormalTok{,}
  \AttributeTok{max\_features =} \DecValTok{50}\NormalTok{,             }\CommentTok{\# cap each block at the top 50 most variable features}
  \AttributeTok{n\_boot       =} \DecValTok{0}               \CommentTok{\# 0 = skip bootstrap (use 1000 for final results)}
\NormalTok{)}
\CommentTok{\# total effect decomposed}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{res\_mediation}\SpecialCharTok{$}\NormalTok{direct\_effect       }\CommentTok{\# exposure {-}\textgreater{} outcome bypassing the mediator}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{res\_mediation}\SpecialCharTok{$}\NormalTok{indirect\_overall    }\CommentTok{\# total indirect effect through mediators}
\FunctionTok{head}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_mediation}\SpecialCharTok{$}\NormalTok{indirect\_pathwise)  }\CommentTok{\# per{-}mediator pathway effect}
\end{Highlighting}
\end{Shaded}

To focus the analysis on DIABLO-prioritised features, pass them via \texttt{features}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{features }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{get\_features}\NormalTok{(}\AttributeTok{comp =} \DecValTok{1}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{run\_mediation}\NormalTok{(}
  \AttributeTok{exposure =} \StringTok{"microb"}\NormalTok{, }\AttributeTok{mediator =} \StringTok{"metab"}\NormalTok{, }\AttributeTok{outcome =}\NormalTok{ pheno,}
  \AttributeTok{features =} \FunctionTok{list}\NormalTok{(}
    \AttributeTok{microb =} \FunctionTok{unique}\NormalTok{(features}\SpecialCharTok{$}\NormalTok{Feature[features}\SpecialCharTok{$}\NormalTok{Block }\SpecialCharTok{==} \StringTok{"microb"}\NormalTok{]),}
    \AttributeTok{metab  =} \FunctionTok{unique}\NormalTok{(features}\SpecialCharTok{$}\NormalTok{Feature[features}\SpecialCharTok{$}\NormalTok{Block  }\SpecialCharTok{==} \StringTok{"metab"}\NormalTok{])}
\NormalTok{  )}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\tightlist
\item
  \texttt{multimedia} builds R formulas from the variable names, so metabolite names like \texttt{"trehalose+d-Glucoheptose\ 1"} would crash the formula parser.
  \texttt{run\_mediation} transparently replaces the names with syntactically valid synthetic names (\texttt{V1}, \texttt{V2}, \ldots) and restores the original names in the returned effect tables.
  The mapping is preserved in \texttt{t1\$res\_mediation\$name\_map}.
\item
  The engine chosen by \texttt{model} is passed to \texttt{multimedia} as the \textbf{outcome} estimator; the mediator model keeps the default \texttt{lm\_model()} of \texttt{multimedia}, i.e.~a plain linear model of each mediator on the exposure variables.
  Keep \texttt{max\_features} (or \texttt{features}) small when the exposure is high-dimensional, otherwise that inner model becomes over-parameterised.
\item
  \texttt{n\_boot\ \textgreater{}\ 0} adds \texttt{res\_mediation\$bootstrap} with the bootstrap distributions of the direct, overall indirect and pathwise indirect effects.
\item
...
\item
  For binary factors in \texttt{exposure} or \texttt{outcome}, pass a 2-level factor or a 0/1 numeric vector.
\end{itemize}

\subsection{Export feature tables for knowledge tools}\label{export-feature-tables-for-knowledge-tools}

DIABLO and TkNA produce data-driven candidate associations, but they say nothing about the \textbf{biochemical plausibility} of a microbe -\textgreater{} metabolite link.
Knowledge-driven tools fill this gap by constraining associations to known reactions.
\texttt{export\_bridge} writes the abundance tables of two blocks into the input formats expected by three popular tools:

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.0710}}
  >{\raggedright\arraybackslash}p{(\linewidth - 2\tabcolsep) * \real{0.9290}}@{}}
\toprule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{mimosa2} & two tab-separated abundance tables (\texttt{\textless{}file\textgreater{}\_ko.txt}, \texttt{\textless{}file\textgreater{}\_metabolites.txt}), features as rows and samples as columns \\
\texttt{amon} & two plain-text ID lists (\texttt{\textless{}file\textgreater{}\_ko\_ids.txt}, \texttt{\textless{}file\textgreater{}\_metabolite\_ids.txt}), one KEGG ID per line \\
\texttt{anansi} & the same two tab-separated tables as \texttt{mimosa2} \\
\end{longtable}
}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{export\_bridge}\NormalTok{(}
  \AttributeTok{target      =} \StringTok{"mimosa2"}\NormalTok{,}
  \AttributeTok{file        =} \StringTok{"./bridge/mimosa\_input"}\NormalTok{,}
  \AttributeTok{ko\_block    =} \StringTok{"microb"}\NormalTok{,}
  \AttributeTok{metab\_block =} \StringTok{"metab"}
\NormalTok{)}
\CommentTok{\# export only the DIABLO{-}prioritised features}
\NormalTok{features }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{get\_features}\NormalTok{(}\AttributeTok{comp =} \DecValTok{1}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{export\_bridge}\NormalTok{(}
  \AttributeTok{target      =} \StringTok{"mimosa2"}\NormalTok{,}
  \AttributeTok{file        =} \StringTok{"./bridge/mimosa\_selected"}\NormalTok{,}
  \AttributeTok{ko\_block    =} \StringTok{"microb"}\NormalTok{,}
  \AttributeTok{metab\_block =} \StringTok{"metab"}\NormalTok{,}
  \AttributeTok{features    =} \FunctionTok{list}\NormalTok{(}
    \AttributeTok{microb =} \FunctionTok{unique}\NormalTok{(features}\SpecialCharTok{$}\NormalTok{Feature[features}\SpecialCharTok{$}\NormalTok{Block }\SpecialCharTok{==} \StringTok{"microb"}\NormalTok{]),}
    \AttributeTok{metab  =} \FunctionTok{unique}\NormalTok{(features}\SpecialCharTok{$}\NormalTok{Feature[features}\SpecialCharTok{$}\NormalTok{Block  }\SpecialCharTok{==} \StringTok{"metab"}\NormalTok{])}
\NormalTok{  )}
...
For the metabolite names, \texttt{metab\_id\_table} (a two-column \texttt{data.frame}: original name + KEGG ID) converts them to KEGG compound IDs, which is what all three tools expect.
Without it, the original names are kept and a message reminds you that KEGG IDs are expected.
When \texttt{ko\_block} / \texttt{metab\_block} are not given, the first block and the block whose name contains ``metab'' are used.

Note that the exported tables come from \texttt{t1\$dataset\_list} (the \textbf{original} abundances), not from the CLR-transformed \texttt{t1\$data\_list}, because these tools expect raw counts or relative abundances.

\subsection{Suggested workflow}\label{suggested-workflow}
...
\tightlist
\item
  \textbf{Load and align} (\texttt{trans\_multiomics\$new}): make sure each \texttt{microtable} carries the same sample IDs and a clean grouping column.
\item
  \textbf{Preprocess} with \texttt{preprocess\_data(method\ =\ "clr",\ filter\_freq\ =\ 0.2)} for compositional blocks; consider additional \texttt{filter\_thres} for very sparse features.
\item
  \textbf{Run DIABLO} with a small initial \texttt{keepX} (e.g.~\texttt{c(10,\ 10)} per block per component) to get a first sanity-check plot (\texttt{plot\_samples}).
\item
  \textbf{Evaluate performance} with \texttt{eval\_performance(folds\ =\ 5,\ nrepeat\ =\ 10,\ seed\ =\ ...)}; compare WeightedVote vs AveragedPredict to choose the best \texttt{ncomp}.
\item
  \textbf{Tune} \texttt{keepX} with \texttt{tune\_model} (start with a coarse grid; refine around the optimum).
\item
  \textbf{Refit} with \texttt{run\_diablo(keepX\ =\ t1\$best\_keepX)} and extract the final \texttt{get\_features} panel.
\item
  \textbf{Stability-check} with \texttt{get\_stability} and keep only the features with \texttt{Stability\ \textgreater{}=\ 0.8} for downstream interpretation.
\item
  \textbf{Visualise} with \texttt{plot\_samples}, \texttt{plot\_loadings}, \texttt{plot\_diablo}, \texttt{plot\_circos} (or \texttt{plot\_cor\_network} for an explicit graph).
\item
  \textbf{Cross-omics associations} with \texttt{cal\_transkingdom}; explore with \texttt{plot\_transkingdom} and follow up on the \texttt{Hub} features, then hand the network over to the trans\_network class with \texttt{convert\_transkingdom}.
\item
  \textbf{Mediation} with \texttt{run\_mediation} (requires \texttt{multimedia}) to quantify exposure -\textgreater{} mediator -\textgreater{} outcome pathways, optionally restricted to the DIABLO / TkNA-prioritised features.
\item
  \textbf{Mechanism filtering} with \texttt{export\_bridge} to feed the candidates into MIMOSA2 / AMON / anansi and retain only biochemically plausible links.
\end{enumerate}

\subsection{Key points}\label{key-points-12}

\begin{itemize}
\tightlist
\item
  \textbf{Block sizes:} \texttt{mixOmics::block.splsda} requires at least 2 levels of \texttt{Y}; \texttt{block.spls} does not use \texttt{Y} and needs \texttt{indY} (handled by \texttt{Y\_block} in \texttt{run\_spls}).
\item
  \textbf{Small class sizes:} \texttt{folds} in \texttt{eval\_performance} and \texttt{tune\_model} must not exceed the smallest class size in \texttt{Y}.
  With 10 samples per group use \texttt{folds\ \textless{}=\ 10}.
\item
  \textbf{Reproducibility:} always pass \texttt{seed} to \texttt{tune\_model} and \texttt{eval\_performance}.
  The seed is forwarded to \texttt{mixOmics} when supported, otherwise applied via \texttt{set.seed}.
\item
  \textbf{Plot types:} \texttt{plot\_samples} returns a \texttt{ggplot}; \texttt{plot\_loadings} draws on the current device and returns a \texttt{list} of loading \texttt{data.frame}s, \textbf{not} a \texttt{ggplot} -- do not pipe it to \texttt{ggsave()}.
\item
  \textbf{\texttt{plot\_loadings} block argument:} it must be supplied explicitly for multi-block models; for the two-block \texttt{spls} model use \texttt{block\ =\ "X"} or \texttt{"Y"}.
\item
  \textbf{\texttt{get\_correlation} names:} in \texttt{mixOmics} \textgreater= 6.x the returned similarity matrix has column names with a \texttt{\_\textless{}block\textgreater{}} suffix, while the row names may be \texttt{NULL}; restore the rows from \texttt{mixOmics::selectVar} if needed.
\item
  \textbf{\texttt{cutoff} of the correlation network:} \texttt{mixOmics} stops when \texttt{cutoff} exceeds the largest value of the similarity matrix, so lower it for weakly correlated blocks (check \texttt{range(t1\$get\_correlation(comp\ =\ 1))}).
\item
  \textbf{More than two blocks:} \texttt{mixOmics::network} returns one similarity matrix per block pair, so \texttt{get\_correlation} needs exactly two blocks selected via \texttt{block\ =\ c(...)} (or a model with two blocks).
\item
  \textbf{Large feature panels:} DIABLO with thousands of features per block is slow.
  Apply \texttt{filter\_freq} and/or \texttt{filter\_thres} in \texttt{preprocess\_data} before the tuning step.
\item
  \textbf{Refitting:} a new \texttt{run\_diablo} replaces the model and invalidates the panels and tables computed from the previous one; refit a copy of the object (\texttt{t1\$clone(deep\ =\ TRUE)}) if both models must be kept.
\item
  \textbf{Unsupervised models:} \texttt{plot\_circos}, \texttt{plot\_heatmap} and \texttt{plot\_diablo} only work on the supervised \texttt{block.splsda} model.
\end{itemize}

\section{trans\_mst class}\label{trans_mst-class}

Microbial source tracking (MST) \citep{Shenhav_FEAST_2019} is used to answer a simple but important question:
for a given community, where do its members come from?
In such an analysis, the community of interest is called the \textbf{sink},
and the candidate communities that may contribute to it are called \textbf{sources}.
...
(e.g., bulk soil -\textgreater{} rhizosphere soil -\textgreater{} root endophyte), detecting contamination in low-biomass samples,
and tracking the formation of developing microbial communities.

Starting from version 2.4.0, the microeco package provides the \texttt{trans\_mst} class for microbial source tracking.
It is a wrapper of \textbf{FEAST} (Fast Expectation-maximization for microbial Source Tracking) \citep{Shenhav_FEAST_2019},
...
directly on a \texttt{microtable} object, so that the preprocessing power of the microeco package
(e.g., taxonomic filtering, sample subsetting and data transformation) is fully reused.

The main functions of the \texttt{trans\_mst} class are:

\begin{itemize}
\tightlist
\item
  \texttt{trans\_mst\$new}: create the object from a microtable object.
\item
  \texttt{cal\_mst}: run the source tracking for one transmission step.
\item
  \texttt{cal\_mst\_chain}: run a chain of transmission steps in one call.
\item
  \texttt{plot\_mst}: plot the contribution proportions as a stacked barplot with ggplot2.
\end{itemize}
...
\NormalTok{remotes}\SpecialCharTok{::}\FunctionTok{install\_github}\NormalTok{(}\StringTok{"cozygene/FEAST"}\NormalTok{)}

\FunctionTok{library}\NormalTok{(microeco)}
\CommentTok{\# for the pipe operator \%\textgreater{}\%}
\FunctionTok{library}\NormalTok{(magrittr)}
\FunctionTok{library}\NormalTok{(ggplot2)}
\end{Highlighting}
\end{Shaded}

\subsection{Prepare the example data}\label{prepare-the-example-data-2}

The example data is \texttt{soil\_microb}, a microtable object of prokaryotic amplicon sequencing data
from an agricultural field experiment with crop rotation and fertilization treatments.
The \texttt{Compartment} column in \texttt{sample\_table} represents the sampling compartments,
i.e., `Bulk soil', `Rhizosphere' and `Endophyte' (root endophyte), with 30 samples for each compartment.
The \texttt{Plant\_ID} column denotes the unique plant individual that each sample belongs to,
so that the three samples sharing the same \texttt{Plant\_ID} are from the same plant
and can be treated as a paired source-sink set.
The \texttt{Cropping} and \texttt{Fertilization} columns are the two experimental factors and
the \texttt{Group} column is the combination of them.

Note that FEAST requires \textbf{raw counts (integers)}, not a relative abundance or rarefied table,
because the estimation relies on the multinomial sampling of reads.
If non-integer values are detected in the \texttt{otu\_table}, they are automatically converted by \texttt{ceiling()} with a message.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{data}\NormalTok{(soil\_microb)}
\NormalTok{soil\_microb}
\CommentTok{\# number of samples in each compartment}
\FunctionTok{table}\NormalTok{(soil\_microb}\SpecialCharTok{$}\NormalTok{sample\_table}\SpecialCharTok{$}\NormalTok{Compartment)}
\CommentTok{\# check that samples with the same Plant\_ID form a paired set across compartments}
\FunctionTok{table}\NormalTok{(soil\_microb}\SpecialCharTok{$}\NormalTok{sample\_table}\SpecialCharTok{$}\NormalTok{Plant\_ID, soil\_microb}\SpecialCharTok{$}\NormalTok{sample\_table}\SpecialCharTok{$}\NormalTok{Compartment)}
\end{Highlighting}
\end{Shaded}

\subsection{Create the object}\label{create-the-object}

There are three parameters that can be used when creating a \texttt{trans\_mst} object:
\texttt{dataset} is the microtable object;
\texttt{env\_col} is a column name in \texttt{sample\_table} used to describe the environment of each sample,
i.e., the `Env' information required by FEAST;
\texttt{filter\_thres} is the relative abundance threshold used to filter low-abundance taxa
(delegated to the \texttt{filter\_taxa} function of the microtable class;
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_mst}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ soil\_microb, }\AttributeTok{env\_col =} \StringTok{"Compartment"}\NormalTok{, }\AttributeTok{filter\_thres =} \DecValTok{0}\NormalTok{)}
\NormalTok{t1}
\end{Highlighting}
\end{Shaded}

\subsection{Source tracking with a shared source pool}\label{source-tracking-with-a-shared-source-pool}
...
Let's use the \texttt{sources} and \texttt{sinks} parameters to assign the environments,
which are matched against the \texttt{env\_col} column of \texttt{sample\_table}.
The results are always stored in the object, so multiple runs can be performed with the same object by
assigning a different \texttt{label} to each run.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_mst}\NormalTok{(}\AttributeTok{sources =} \StringTok{"Bulk soil"}\NormalTok{, }\AttributeTok{sinks =} \StringTok{"Rhizosphere"}\NormalTok{, }\AttributeTok{EM\_iterations =} \DecValTok{1000}\NormalTok{, }\AttributeTok{label =} \StringTok{"bulk2rhizo"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Two key parameters of the FEAST algorithm deserve attention.
\texttt{EM\_iterations} is the number of expectation-maximization iterations.
...
\texttt{COVERAGE} is the rarefaction depth used internally by FEAST.
...
\tightlist
\item
  \texttt{res\_mst{[}{[}label{]}{]}}: Sink x Source matrix at the \textbf{sample level}. Each row sums to 1.
\item
  \texttt{res\_mst\_env{[}{[}label{]}{]}}: Sink x Source matrix at the \textbf{environment (source pooled) level}. Each row sums to 1.
\item
  \texttt{res\_mst\_param{[}{[}label{]}{]}}: the input parameters and the involved sample information of the run.
\end{itemize}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# sample level result: rows are sinks, columns are sources}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{res\_mst[[}\StringTok{"bulk2rhizo"}\NormalTok{]][}\DecValTok{1}\SpecialCharTok{:}\DecValTok{5}\NormalTok{, }\DecValTok{1}\SpecialCharTok{:}\DecValTok{5}\NormalTok{]}
\CommentTok{\# environment level result: contributions of the source environments}
\FunctionTok{head}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_mst\_env[[}\StringTok{"bulk2rhizo"}\NormalTok{]])}
\CommentTok{\# check the row sum}
\FunctionTok{rowSums}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_mst\_env[[}\StringTok{"bulk2rhizo"}\NormalTok{]])}
\CommentTok{\# mean contribution of each source environment}
\FunctionTok{colMeans}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_mst\_env[[}\StringTok{"bulk2rhizo"}\NormalTok{]])}
\CommentTok{\# parameters and sample information of the run}
\NormalTok{t1}\SpecialCharTok{$}\NormalTok{res\_mst\_param[[}\StringTok{"bulk2rhizo"}\NormalTok{]]}
\end{Highlighting}
\end{Shaded}

\subsection{Source tracking with different source sets}\label{source-tracking-with-different-source-sets}
...
\textbf{the same plant individual}, instead of a general pool containing all the plants.
This is realized by \texttt{different\_sources\_flag\ =\ 1} together with the \texttt{id\_col} parameter.
Samples sharing the same value of \texttt{id\_col} (here \texttt{Plant\_ID}) are considered as one paired group,
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_mst}\NormalTok{(}\AttributeTok{sources =} \FunctionTok{c}\NormalTok{(}\StringTok{"Bulk soil"}\NormalTok{, }\StringTok{"Rhizosphere"}\NormalTok{), }\AttributeTok{sinks =} \StringTok{"Endophyte"}\NormalTok{,}
    \AttributeTok{different\_sources\_flag =} \DecValTok{1}\NormalTok{, }\AttributeTok{id\_col =} \StringTok{"Plant\_ID"}\NormalTok{, }\AttributeTok{EM\_iterations =} \DecValTok{1000}\NormalTok{, }\AttributeTok{label =} \StringTok{"soil2root"}\NormalTok{)}
\CommentTok{\# now there are two runs stored in the object}
\NormalTok{t1}
\FunctionTok{head}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_mst\_env[[}\StringTok{"soil2root"}\NormalTok{]])}
\end{Highlighting}
\end{Shaded}

In the different-sources mode, the sources that do not belong to the source set of a given sink are \texttt{NA} in \texttt{res\_mst}.
These \texttt{NA} values are automatically removed in the plot, and they are also skipped when aggregating
the contributions by source environment in \texttt{res\_mst\_env}.
For each sink, the non-\texttt{NA} contributions (including `Unknown') still sum to 1.

\subsection{Run sequential steps in one call}\label{run-sequential-steps-in-one-call}

The transmission of microbes is often a chain rather than a single step.
The \texttt{cal\_mst\_chain} function is a convenient wrapper of \texttt{cal\_mst},
where each element of the \texttt{steps} argument is a list of the parameters passed to \texttt{cal\_mst}.
The results are stored with the labels `step1', `step2', \ldots{} unless a custom \texttt{label} is provided.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_mst}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ soil\_microb, }\AttributeTok{env\_col =} \StringTok{"Compartment"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_mst\_chain}\NormalTok{(}\AttributeTok{steps =} \FunctionTok{list}\NormalTok{(}
    \FunctionTok{list}\NormalTok{(}\AttributeTok{sources =} \StringTok{"Bulk soil"}\NormalTok{, }\AttributeTok{sinks =} \StringTok{"Rhizosphere"}\NormalTok{, }\AttributeTok{EM\_iterations =} \DecValTok{1000}\NormalTok{, }\AttributeTok{label =} \StringTok{"bulk2rhizo"}\NormalTok{),}
    \FunctionTok{list}\NormalTok{(}\AttributeTok{sources =} \FunctionTok{c}\NormalTok{(}\StringTok{"Bulk soil"}\NormalTok{, }\StringTok{"Rhizosphere"}\NormalTok{), }\AttributeTok{sinks =} \StringTok{"Endophyte"}\NormalTok{,}
        \AttributeTok{different\_sources\_flag =} \DecValTok{1}\NormalTok{, }\AttributeTok{id\_col =} \StringTok{"Plant\_ID"}\NormalTok{, }\AttributeTok{EM\_iterations =} \DecValTok{1000}\NormalTok{, }\AttributeTok{label =} \StringTok{"soil2root"}\NormalTok{)}
\NormalTok{))}
\CommentTok{\# all the stored runs}
\FunctionTok{names}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_mst)}
\end{Highlighting}
\end{Shaded}

\subsection{Specify sources and sinks by sample names}\label{specify-sources-and-sinks-by-sample-names}

Instead of using environment values, the sources and sinks can also be assigned by explicit sample names
with the \texttt{source\_samples} and \texttt{sink\_samples} parameters.
This is useful when the source/sink assignment has no correspondence with any existing column of \texttt{sample\_table}.
Note that \texttt{sources} and \texttt{source\_samples} cannot be provided at the same time (the same for \texttt{sinks} and \texttt{sink\_samples}),
and no sample can be assigned to both sources and sinks.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# pick some bulk soil samples as sources and some rhizosphere samples as sinks}
\NormalTok{st\_tmp }\OtherTok{\textless{}{-}}\NormalTok{ soil\_microb}\SpecialCharTok{$}\NormalTok{sample\_table}
\NormalTok{source\_samples }\OtherTok{\textless{}{-}} \FunctionTok{rownames}\NormalTok{(st\_tmp)[st\_tmp}\SpecialCharTok{$}\NormalTok{Compartment }\SpecialCharTok{==} \StringTok{"Bulk soil"}\NormalTok{][}\DecValTok{1}\SpecialCharTok{:}\DecValTok{10}\NormalTok{]}
\NormalTok{sink\_samples }\OtherTok{\textless{}{-}} \FunctionTok{rownames}\NormalTok{(st\_tmp)[st\_tmp}\SpecialCharTok{$}\NormalTok{Compartment }\SpecialCharTok{==} \StringTok{"Rhizosphere"}\NormalTok{][}\DecValTok{1}\SpecialCharTok{:}\DecValTok{10}\NormalTok{]}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_mst}\NormalTok{(}\AttributeTok{source\_samples =}\NormalTok{ source\_samples, }\AttributeTok{sink\_samples =}\NormalTok{ sink\_samples, }\AttributeTok{EM\_iterations =} \DecValTok{1000}\NormalTok{, }\AttributeTok{label =} \StringTok{"custom"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\subsection{Visualization}\label{visualization}

The \texttt{plot\_mst} function returns a ggplot2 object, so that it can be further modified before saving.
By default, the environment-level result (\texttt{res\_mst\_env}) of the first run is plotted.
The \texttt{group\_sink} parameter is used to group (and facet) the sinks according to a column of \texttt{sample\_table},
which is very useful to compare the contribution patterns among treatments.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# the first run with the environment{-}level result}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_mst}\NormalTok{(}\AttributeTok{use\_run =} \StringTok{"bulk2rhizo"}\NormalTok{, }\AttributeTok{group\_sink =} \StringTok{"Group"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=900px]{Images/trans_mst_bulk2rhizo} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_mst}\NormalTok{(}\AttributeTok{use\_run =} \StringTok{"soil2root"}\NormalTok{, }\AttributeTok{group\_sink =} \StringTok{"Cropping"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=900px]{Images/trans_mst_soil2root} \end{center}
...
The \texttt{use\_env\ =\ FALSE} parameter switches to the sample-level result (\texttt{res\_mst}),
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_mst}\NormalTok{(}\AttributeTok{use\_run =} \StringTok{"custom"}\NormalTok{, }\AttributeTok{use\_env =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{center}\includegraphics[width=900px]{Images/trans_mst_sample_level} \end{center}

More parameters are available for the details of the plot, e.g., \texttt{show\_unknown} (whether to show the `Unknown' proportion),
\texttt{colors}, \texttt{unknown\_color}, \texttt{bar\_width}, \texttt{xtext\_size}, \texttt{xtext\_angle},
\texttt{legend\_title} (the legend title) and \texttt{facet\_scales} (the scales argument of \texttt{facet\_wrap}).
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# hide the Unknown proportion and customize the colors}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_mst}\NormalTok{(}\AttributeTok{use\_run =} \StringTok{"soil2root"}\NormalTok{, }\AttributeTok{group\_sink =} \StringTok{"Cropping"}\NormalTok{, }\AttributeTok{show\_unknown =} \ConstantTok{FALSE}\NormalTok{,}
    \AttributeTok{colors =} \FunctionTok{c}\NormalTok{(}\StringTok{"\#E64B35FF"}\NormalTok{, }\StringTok{"\#4DBBD5FF"}\NormalTok{), }\AttributeTok{xtext\_size =} \DecValTok{8}\NormalTok{, }\AttributeTok{xtext\_angle =} \DecValTok{90}\NormalTok{)}
\CommentTok{\# save the plot}
\FunctionTok{ggsave}\NormalTok{(}\StringTok{"MST\_soil2root.pdf"}\NormalTok{, t1}\SpecialCharTok{$}\FunctionTok{plot\_mst}\NormalTok{(}\AttributeTok{use\_run =} \StringTok{"soil2root"}\NormalTok{, }\AttributeTok{group\_sink =} \StringTok{"Cropping"}\NormalTok{), }\AttributeTok{width =} \DecValTok{8}\NormalTok{, }\AttributeTok{height =} \DecValTok{4}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\subsection{Downstream analysis of the results}\label{downstream-analysis-of-the-results}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# take the environment{-}level result of the soil {-}\textgreater{} root step}
\NormalTok{contrib }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\NormalTok{res\_mst\_env[[}\StringTok{"soil2root"}\NormalTok{]]}
\CommentTok{\# add the group information of the sinks}
\NormalTok{contrib\_df }\OtherTok{\textless{}{-}} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{SampleID =} \FunctionTok{rownames}\NormalTok{(contrib), contrib, }\AttributeTok{check.names =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{contrib\_df}\SpecialCharTok{$}\NormalTok{Fertilization }\OtherTok{\textless{}{-}}\NormalTok{ soil\_microb}\SpecialCharTok{$}\NormalTok{sample\_table[contrib\_df}\SpecialCharTok{$}\NormalTok{SampleID, }\StringTok{"Fertilization"}\NormalTok{]}
\CommentTok{\# make a long table for plotting}
\NormalTok{plot\_df }\OtherTok{\textless{}{-}}\NormalTok{ reshape2}\SpecialCharTok{::}\FunctionTok{melt}\NormalTok{(contrib\_df, }\AttributeTok{measure.vars =} \FunctionTok{c}\NormalTok{(}\StringTok{"Bulk soil"}\NormalTok{, }\StringTok{"Rhizosphere"}\NormalTok{, }\StringTok{"Unknown"}\NormalTok{),}
    \AttributeTok{variable.name =} \StringTok{"Source"}\NormalTok{, }\AttributeTok{value.name =} \StringTok{"Contribution"}\NormalTok{)}
\FunctionTok{ggplot}\NormalTok{(plot\_df, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ Fertilization, }\AttributeTok{y =}\NormalTok{ Contribution, }\AttributeTok{fill =}\NormalTok{ Source)) }\SpecialCharTok{+}
    \FunctionTok{geom\_boxplot}\NormalTok{(}\AttributeTok{width =} \FloatTok{0.6}\NormalTok{, }\AttributeTok{outlier.alpha =} \DecValTok{0}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{stat\_summary}\NormalTok{(}\AttributeTok{fun =}\NormalTok{ mean, }\AttributeTok{geom =} \StringTok{"point"}\NormalTok{, }\AttributeTok{position =} \FunctionTok{position\_dodge}\NormalTok{(}\AttributeTok{width =} \FloatTok{0.6}\NormalTok{)) }\SpecialCharTok{+}
    ...
    \FunctionTok{labs}\NormalTok{(}\AttributeTok{x =} \ConstantTok{NULL}\NormalTok{, }\AttributeTok{y =} \StringTok{"Contribution proportion"}\NormalTok{) }\SpecialCharTok{+}
    \FunctionTok{theme\_bw}\NormalTok{()}
\CommentTok{\# save the tables for further use}
\FunctionTok{write.csv}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_mst[[}\StringTok{"soil2root"}\NormalTok{]], }\StringTok{"MST\_soil2root\_sample\_level.csv"}\NormalTok{)}
\FunctionTok{write.csv}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_mst\_env[[}\StringTok{"soil2root"}\NormalTok{]], }\StringTok{"MST\_soil2root\_env\_level.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# install file2meco package (\textgreater{}= 0.9.0)}
...
\NormalTok{test1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(test, }\AttributeTok{taxrank =} \StringTok{"Phylum"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{10}\NormalTok{, }\AttributeTok{delete\_taxonomy\_lineage =} \ConstantTok{FALSE}\NormalTok{)}
...
\CommentTok{\# taxonomic biomarker}
...
MetaPhlAn is an software used for metagenomic taxonomic profiling \citep{Blanco_MetaPhlAn_2023}.
...
\NormalTok{mt\_fungi }\OtherTok{\textless{}{-}}\NormalTok{ microtable}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{sample\_table =}\NormalTok{ sample\_info\_ITS, }\AttributeTok{otu\_table =}\NormalTok{ otu\_table\_ITS, }\AttributeTok{tax\_table =}\NormalTok{ taxonomy\_table\_ITS)}
\CommentTok{\# remove the taxa not assigned in the Kingdom "k\_\_Fungi"}
\NormalTok{mt\_fungi}\SpecialCharTok{$}\NormalTok{tax\_table }\SpecialCharTok{\%\textless{}\textgreater{}\%}\NormalTok{ base}\SpecialCharTok{::}\FunctionTok{subset}\NormalTok{(Kingdom }\SpecialCharTok{==} \StringTok{"k\_\_Fungi"}\NormalTok{)}
\CommentTok{\# use tidy\_dataset() to make OTUs and samples information consistent across files}
\NormalTok{mt\_fungi}\SpecialCharTok{$}\FunctionTok{tidy\_dataset}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_func}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(mt\_fungi)}
\CommentTok{\# Both FungalTraits and FUNGuild are available}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_func}\NormalTok{(}\AttributeTok{fungi\_database =} \StringTok{"FungalTraits"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_func\_FR}\NormalTok{(}\AttributeTok{abundance\_weighted =} \ConstantTok{TRUE}\NormalTok{)}
\CommentTok{\# return: t1$res\_func\_FR}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# clone mt\_fungi for differential test}
\NormalTok{tmp\_mt }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(mt\_fungi)}
\CommentTok{\# we need assign the table to taxa\_abund manually}
\NormalTok{tmp\_mt}\SpecialCharTok{$}\NormalTok{taxa\_abund}\SpecialCharTok{$}\NormalTok{func }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(}\FunctionTok{t}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_func\_FR), }\AttributeTok{check.names =} \ConstantTok{FALSE}\NormalTok{)}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ mt\_fungi, }\AttributeTok{cal\_cor =} \StringTok{"WGCNA"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"OTU"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.000001}\NormalTok{, }\AttributeTok{cor\_method =} \StringTok{"spearman"}\NormalTok{)}
...
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{cal\_func}\NormalTok{(}\AttributeTok{fungi\_database =} \StringTok{"FUNGuild"}\NormalTok{)}
\CommentTok{\# calculate abundance{-}unweighted functional redundancy of each trait for each network module}
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{cal\_func\_FR}\NormalTok{(}\AttributeTok{abundance\_weighted =} \ConstantTok{FALSE}\NormalTok{)}
\CommentTok{\# plot the functional redundancy of network modules}
\NormalTok{t2}\SpecialCharTok{$}\FunctionTok{plot\_func\_FR}\NormalTok{(}\AttributeTok{order\_x =} \FunctionTok{paste0}\NormalTok{(}\StringTok{"M"}\NormalTok{, }\DecValTok{1}\SpecialCharTok{:}\DecValTok{10}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{taxonomy\_file\_path }\OtherTok{\textless{}{-}} \FunctionTok{system.file}\NormalTok{(}\StringTok{"extdata"}\NormalTok{, }\StringTok{"taxonomy.qza"}\NormalTok{, }\AttributeTok{package=}\StringTok{"file2meco"}\NormalTok{)}
\CommentTok{\# construct microtable object}
\NormalTok{tmp }\OtherTok{\textless{}{-}} \FunctionTok{qiime2meco}\NormalTok{(abund\_file\_path, }\AttributeTok{sample\_table =}\NormalTok{ sample\_file\_path, }\AttributeTok{taxonomy\_table =}\NormalTok{ taxonomy\_file\_path)}
\NormalTok{tmp}\SpecialCharTok{$}\FunctionTok{tidy\_dataset}\NormalTok{()}
\CommentTok{\# revise the species names in tax\_table as the information in the example file is not standard}
\NormalTok{select\_rows }\OtherTok{\textless{}{-}}\NormalTok{ tmp}\SpecialCharTok{$}\NormalTok{tax\_table}\SpecialCharTok{$}\NormalTok{Species }\SpecialCharTok{!=} \StringTok{"s\_\_"}
\NormalTok{tmp}\SpecialCharTok{$}\NormalTok{tax\_table}\SpecialCharTok{$}\NormalTok{Species[select\_rows] }\OtherTok{\textless{}{-}} \FunctionTok{paste0}\NormalTok{(}\StringTok{"s\_\_"}\NormalTok{, }\FunctionTok{gsub}\NormalTok{(}\StringTok{"g\_\_"}\NormalTok{, }\StringTok{""}\NormalTok{, tmp}\SpecialCharTok{$}\NormalTok{tax\_table}\SpecialCharTok{$}\NormalTok{Genus[select\_rows]), }\StringTok{" "}\NormalTok{, }\FunctionTok{gsub}\NormalTok{(}\StringTok{"s\_\_"}\NormalTok{, }\StringTok{""}\NormalTok{, tmp}\SpecialCharTok{$}\NormalTok{tax\_table}\SpecialCharTok{$}\NormalTok{Species[select\_rows]))}
\CommentTok{\# taxonomic abundance}
\NormalTok{tmp}\SpecialCharTok{$}\FunctionTok{cal\_abund}\NormalTok{()}

\CommentTok{\# create object of trans\_func}
\NormalTok{data2 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_func}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(tmp)}
\CommentTok{\# Select NJC19 database}
\NormalTok{data2}\SpecialCharTok{$}\FunctionTok{cal\_func}\NormalTok{(}\AttributeTok{prok\_database =} \StringTok{"NJC19"}\NormalTok{)}
\CommentTok{\# get the FR data using weighted abundance method}
\NormalTok{data2}\SpecialCharTok{$}\FunctionTok{cal\_func\_FR}\NormalTok{(}\AttributeTok{abundance\_weighted =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{perc =} \ConstantTok{TRUE}\NormalTok{)}
\CommentTok{\# inset the FR result into taxa\_abund of microtable object}
\NormalTok{tmp}\SpecialCharTok{$}\NormalTok{taxa\_abund}\SpecialCharTok{$}\NormalTok{Trait }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(}\FunctionTok{t}\NormalTok{(data2}\SpecialCharTok{$}\NormalTok{res\_func\_FR))}
\CommentTok{\# use trans\_abund to plot}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_abund}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ tmp, }\AttributeTok{taxrank =} \StringTok{"Trait"}\NormalTok{, }\AttributeTok{ntaxa =} \DecValTok{10}\NormalTok{, }\AttributeTok{use\_percentage =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_box}\NormalTok{(}\AttributeTok{group =} \StringTok{"donor\_status"}\NormalTok{, }\AttributeTok{xtext\_angle =} \DecValTok{30}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"Relative abundance (\%)"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{theme}\NormalTok{(}\AttributeTok{axis.text.x =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{size =} \DecValTok{13}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_diff}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ tmp, }\AttributeTok{method =} \StringTok{"rf"}\NormalTok{, }\AttributeTok{group =} \StringTok{"donor\_status"}\NormalTok{, }\AttributeTok{taxa\_level =} \StringTok{"Trait"}\NormalTok{)}
...
It is convenient to save the analyzed data (i.e.~object) to local computer with \texttt{save} function for back-up and communication with others.
This is also the recommended way to attach data for the test in `Github Issues'.
...
\NormalTok{g1 }\OtherTok{\textless{}{-}}\NormalTok{ tmp}\SpecialCharTok{$}\FunctionTok{plot}\NormalTok{(}\AttributeTok{add =} \StringTok{"none"}\NormalTok{, }\AttributeTok{add\_sig =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{add\_sig\_text\_size =} \DecValTok{5}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{ylab}\NormalTok{(}\StringTok{"Phylogenetic distance"}\NormalTok{)}
...
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_diff}\NormalTok{(}\AttributeTok{measure =} \StringTok{"r\_pos"}\NormalTok{, }\AttributeTok{method =} \StringTok{"anova"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot}\NormalTok{()}

\CommentTok{\# without statistical result}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ cohesionclass}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(soil\_amp\_network)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot}\NormalTok{(}\AttributeTok{measure =} \StringTok{"c\_neg"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_rarefy}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(tmp, }\AttributeTok{alphadiv =} \StringTok{"Observed"}\NormalTok{, }\AttributeTok{depth =} \FunctionTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{10}\NormalTok{, }\DecValTok{50}\NormalTok{, }\DecValTok{500}\NormalTok{, }\DecValTok{2000}\NormalTok{, }\DecValTok{6000}\NormalTok{, }\DecValTok{10000}\NormalTok{, }\DecValTok{15000}\NormalTok{, }\DecValTok{20000}\NormalTok{))}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_rarefy}\NormalTok{(}\AttributeTok{color =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{show\_point =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{add\_fitting =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_rarefy}\NormalTok{(}\AttributeTok{show\_point =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{add\_fitting =} \ConstantTok{TRUE}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_rarefy}\NormalTok{(}\AttributeTok{show\_samplename =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{color\_values =} \FunctionTok{rep}\NormalTok{(}\StringTok{"grey70"}\NormalTok{, }\DecValTok{100}\NormalTok{), }\AttributeTok{show\_legend =} \ConstantTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
The mecoturn package \citep{Liu_mecoturn_2025} is designed to study microbial turnover along plant compartments originally (\url{https://github.com/ChiLiubio/mecoturn}).
Generally speaking, it can be used in any case associated with a gradient.
To cite mecoturn in publications, please use:

Chi Liu, Jiahui Luo, Chunmiao Lu, Tianlong Sheng, Raymond Jianxiong Zeng, Xiangzhen Li, Minjie Yao. mecoturn: An R package for deciphering microbial turnover patterns along gradients. Soil Ecology Letters. 2025, 7(4): 250355. \url{https://doi.org/10.1007/s42832-025-0355-6}
...
Please refer to the solution in this topic (\url{https://github.com/ChiLiubio/microeco/issues/165}).
...
Now, we go ahead and use \texttt{trans\_diff} class to demonstrate the differential test of the abundances of OTUs with specific traits across groups.
...
\FunctionTok{data}\NormalTok{(dataset)}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_func}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(dataset)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_func}\NormalTok{(}\AttributeTok{prok\_database =} \StringTok{"FAPROTAX"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_func\_FR}\NormalTok{(}\AttributeTok{abundance\_weighted =} \ConstantTok{TRUE}\NormalTok{)}
...
\NormalTok{tmp }\OtherTok{\textless{}{-}} \FunctionTok{as.data.frame}\NormalTok{(}\FunctionTok{t}\NormalTok{(t1}\SpecialCharTok{$}\NormalTok{res\_func\_FR), }\AttributeTok{check.names =} \ConstantTok{FALSE}\NormalTok{)}
...
\section{Visualize taxonomic abundance with trans\_alpha class}\label{visualize-taxonomic-abundance-with-trans_alpha-class}

When it is necessary to perform differential analysis and visualization between groups within different categories,
the trans\_diff class currently struggles to achieve this for multiple taxa.
At this time, we can use the trans\_alpha class to analyze each taxon and merge the figures into one.
...
\FunctionTok{library}\NormalTok{(magrittr)}
\FunctionTok{library}\NormalTok{(mecoturn)}
\FunctionTok{data}\NormalTok{(}\StringTok{"wheat\_16S"}\NormalTok{)}
\CommentTok{\# first clone the data}
\NormalTok{d1 }\OtherTok{\textless{}{-}} \FunctionTok{clone}\NormalTok{(wheat\_16S)}
\CommentTok{\# tidy the abundance table and view it as alpha diversity}
\NormalTok{d1}\SpecialCharTok{$}\NormalTok{alpha\_diversity }\OtherTok{\textless{}{-}}\NormalTok{ d1}\SpecialCharTok{$}\NormalTok{taxa\_abund}\SpecialCharTok{$}\NormalTok{Genus }\SpecialCharTok{\%\textgreater{}\%}\NormalTok{ .[}\SpecialCharTok{!}\FunctionTok{grepl}\NormalTok{(}\StringTok{"\_\_$"}\NormalTok{, }\FunctionTok{rownames}\NormalTok{(.), }\AttributeTok{ignore.case =} \ConstantTok{TRUE}\NormalTok{), ] }\SpecialCharTok{\%\textgreater{}\%}\NormalTok{ t }\SpecialCharTok{\%\textgreater{}\%}\NormalTok{ as.data.frame}
\CommentTok{\# extract a part of taxa as an example}
\NormalTok{d1}\SpecialCharTok{$}\NormalTok{alpha\_diversity }\SpecialCharTok{\%\textless{}\textgreater{}\%}\NormalTok{ .[, }\DecValTok{1}\SpecialCharTok{:}\DecValTok{10}\NormalTok{]}
\CommentTok{\# simplify the taxa names}
\FunctionTok{colnames}\NormalTok{(d1}\SpecialCharTok{$}\NormalTok{alpha\_diversity) }\SpecialCharTok{\%\textless{}\textgreater{}\%} \FunctionTok{gsub}\NormalTok{(}\StringTok{".*g\_\_"}\NormalTok{, }\StringTok{""}\NormalTok{, .)}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_alpha}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(d1, }\AttributeTok{group =} \StringTok{"Type"}\NormalTok{, }\AttributeTok{by\_group =} \StringTok{"Region"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_diff}\NormalTok{(}\AttributeTok{method =} \StringTok{"anova"}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{measure =} \StringTok{"Pseudomonas"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\section{Customized network input and analysis}\label{customized-network-input-and-analysis}

When users need to use the \texttt{trans\_network} class to calculate some properties of their customized network,
they can refer to the following method.
Only the network in igraph format is required, and no other data is necessary.
It should be noted that the nodes of the provided network must have the \texttt{name} attribute.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(igraph)}
\CommentTok{\# first generate an igraph{-}format network as the input of the example}
\NormalTok{g1 }\OtherTok{\textless{}{-}} \FunctionTok{make\_graph}\NormalTok{(}\FunctionTok{rep}\NormalTok{(LETTERS, }\AttributeTok{each =} \DecValTok{2}\NormalTok{)[}\SpecialCharTok{{-}}\FunctionTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{, }\DecValTok{52}\NormalTok{)], }\AttributeTok{directed =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{g1}

\FunctionTok{library}\NormalTok{(microeco)}
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =} \ConstantTok{NULL}\NormalTok{)}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_network}\NormalTok{(}\AttributeTok{network\_method =} \ConstantTok{NULL}\NormalTok{)}

\NormalTok{t1}\SpecialCharTok{$}\NormalTok{res\_network }\OtherTok{\textless{}{-}}\NormalTok{ g1}

\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_module}\NormalTok{()}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_network\_attr}\NormalTok{()}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{get\_node\_table}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
Based on the characteristics of R6 class objects, pipelining operations can be performed using the \texttt{\$} operator.
...
\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_alpha}\SpecialCharTok{$}
    \FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{group =} \StringTok{"Type"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_diff}\NormalTok{(}\AttributeTok{method =} \StringTok{"anova"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Shannon"}\NormalTok{)}

\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"errorbar"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Shannon"}\NormalTok{, }\AttributeTok{add\_sig =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

When the main purpose is to create a plot and there is no need to retain the object of a class, the user can write the entire operation chain.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{trans\_alpha}\SpecialCharTok{$}
    \FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{group =} \StringTok{"Type"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_diff}\NormalTok{(}\AttributeTok{method =} \StringTok{"anova"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Shannon"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"errorbar"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Shannon"}\NormalTok{, }\AttributeTok{add\_sig =} \ConstantTok{TRUE}\NormalTok{)}


\NormalTok{g1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_alpha}\SpecialCharTok{$}
    \FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{group =} \StringTok{"Type"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_diff}\NormalTok{(}\AttributeTok{method =} \StringTok{"anova"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Shannon"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{plot\_alpha}\NormalTok{(}\AttributeTok{plot\_type =} \StringTok{"errorbar"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"Shannon"}\NormalTok{, }\AttributeTok{add\_sig =} \ConstantTok{TRUE}\NormalTok{)}
...
\NormalTok{test }\OtherTok{\textless{}{-}}\NormalTok{ microtable}\SpecialCharTok{$}
    \FunctionTok{new}\NormalTok{(}\AttributeTok{sample\_table =}\NormalTok{ sample\_info\_16S, }\AttributeTok{otu\_table =}\NormalTok{ otu\_table\_16S, }\AttributeTok{tax\_table =}\NormalTok{ taxonomy\_table\_16S, }\AttributeTok{phylo\_tree =}\NormalTok{ phylo\_tree\_16S)}\SpecialCharTok{$}
    \FunctionTok{filter\_pollution}\NormalTok{(}\AttributeTok{taxa =} \FunctionTok{c}\NormalTok{(}\StringTok{"mitochondria"}\NormalTok{, }\StringTok{"chloroplast"}\NormalTok{))}\SpecialCharTok{$}
    \FunctionTok{tidy\_dataset}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{filter\_taxa}\NormalTok{()}

\NormalTok{test}\SpecialCharTok{$}\FunctionTok{sample\_sums}\NormalTok{() }\SpecialCharTok{\%\textgreater{}\%}\NormalTok{ range}

\NormalTok{test}\SpecialCharTok{$}
    \FunctionTok{rarefy\_samples}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{cal\_abund}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{save\_abund}\NormalTok{(}\FunctionTok{file.path}\NormalTok{(test\_dir, }\StringTok{"taxa\_abund"}\NormalTok{))}\SpecialCharTok{$}
    \FunctionTok{cal\_alphadiv}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{save\_alphadiv}\NormalTok{(}\FunctionTok{file.path}\NormalTok{(test\_dir, }\StringTok{"alpha\_diversity"}\NormalTok{))}\SpecialCharTok{$}
    \FunctionTok{cal\_betadiv}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{save\_betadiv}\NormalTok{(}\FunctionTok{file.path}\NormalTok{(test\_dir, }\StringTok{"beta\_diversity"}\NormalTok{))}



\CommentTok{\# trans\_beta}

\FunctionTok{library}\NormalTok{(magrittr)}
\FunctionTok{library}\NormalTok{(microeco)}
\FunctionTok{data}\NormalTok{(dataset)}

\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_beta}\SpecialCharTok{$}
    \FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"bray"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_ordination}\NormalTok{(}\AttributeTok{method =} \StringTok{"PCoA"}\NormalTok{, }\AttributeTok{ncomp =} \DecValTok{5}\NormalTok{)}

\NormalTok{g1 }\OtherTok{\textless{}{-}}\NormalTok{ t1}\SpecialCharTok{$}\FunctionTok{plot\_ordination}\NormalTok{(}\AttributeTok{plot\_color =} \StringTok{"Group"}\NormalTok{)}
\NormalTok{g1}


\NormalTok{trans\_beta}\SpecialCharTok{$}
    \FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{group =} \StringTok{"Type"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"bray"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_group\_distance}\NormalTok{(}\AttributeTok{within\_group =} \ConstantTok{TRUE}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_group\_distance\_diff}\NormalTok{(}\AttributeTok{method =} \StringTok{"wilcox"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{plot\_group\_distance}\NormalTok{()}

\NormalTok{trans\_beta}\SpecialCharTok{$}
    \FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{measure =} \StringTok{"bray"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_manova}\NormalTok{(}\AttributeTok{manova\_all =} \ConstantTok{FALSE}\NormalTok{, }\AttributeTok{group =} \StringTok{"Type"}\NormalTok{, }\AttributeTok{by\_group =} \StringTok{"Group"}\NormalTok{)}\SpecialCharTok{$}
\NormalTok{    res\_manova}

\NormalTok{trans\_beta}\SpecialCharTok{$}
    \FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{measure =} \StringTok{"bray"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_anosim}\NormalTok{()}\SpecialCharTok{$}
\NormalTok{    res\_anosim}



\CommentTok{\# trans\_network}

\FunctionTok{library}\NormalTok{(microeco)}
\FunctionTok{data}\NormalTok{(dataset)}

\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_network}\SpecialCharTok{$}\FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{cor\_method =} \StringTok{"pearson"}\NormalTok{, }\AttributeTok{filter\_thres =} \FloatTok{0.001}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_network}\NormalTok{(}\AttributeTok{COR\_p\_thres =} \FloatTok{0.05}\NormalTok{, }\AttributeTok{COR\_cut =} \FloatTok{0.6}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_module}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{cal\_network\_attr}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{get\_node\_table}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{get\_edge\_table}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{cal\_sum\_links}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{save\_network}\NormalTok{()}

\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_network}\NormalTok{()}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_sum\_links}\NormalTok{(}\AttributeTok{method =} \StringTok{"circlize"}\NormalTok{, }\AttributeTok{transparency =} \FloatTok{0.2}\NormalTok{, }\AttributeTok{annotationTrackHeight =}\NormalTok{ circlize}\SpecialCharTok{::}\FunctionTok{mm\_h}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\DecValTok{5}\NormalTok{, }\DecValTok{5}\NormalTok{)))}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_taxa\_roles}\NormalTok{(}\AttributeTok{use\_type =} \DecValTok{1}\NormalTok{)}



\CommentTok{\# trans\_classifier}

\FunctionTok{library}\NormalTok{(magrittr)}
\FunctionTok{library}\NormalTok{(microeco)}
\FunctionTok{data}\NormalTok{(dataset)}

\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_classifier}\SpecialCharTok{$}
    \FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{y.response =} \StringTok{"Saline"}\NormalTok{, }\AttributeTok{x.predictors =} \StringTok{"Genus"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_split}\NormalTok{(}\AttributeTok{prop.train =} \DecValTok{3}\SpecialCharTok{/}\DecValTok{4}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_feature\_sel}\NormalTok{(}\AttributeTok{boruta.maxRuns =} \DecValTok{300}\NormalTok{, }\AttributeTok{boruta.pValue =} \FloatTok{0.01}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{set\_trainControl}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{cal\_train}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{cal\_predict}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{cal\_ROC}\NormalTok{()}

\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{plot\_ROC}\NormalTok{()}


\NormalTok{t1}\SpecialCharTok{$}
    \FunctionTok{cal\_caretList}\NormalTok{(}\AttributeTok{methodList =} \FunctionTok{c}\NormalTok{(}\StringTok{\textquotesingle{}rf\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}svmRadial\textquotesingle{}}\NormalTok{))}\SpecialCharTok{$}
    \FunctionTok{cal\_caretList\_resamples}\NormalTok{()}\SpecialCharTok{$}
    \FunctionTok{plot\_caretList\_resamples}\NormalTok{() }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{(}\AttributeTok{position =} \StringTok{"jitter"}\NormalTok{) }\SpecialCharTok{+} \FunctionTok{theme\_bw}\NormalTok{()}


\CommentTok{\# trans\_env}

\FunctionTok{library}\NormalTok{(microeco)}
\FunctionTok{data}\NormalTok{(dataset)}
\FunctionTok{data}\NormalTok{(env\_data\_16S)}

\NormalTok{trans\_env}\SpecialCharTok{$}
    \FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{add\_data =}\NormalTok{ env\_data\_16S[, }\DecValTok{4}\SpecialCharTok{:}\DecValTok{10}\NormalTok{])}\SpecialCharTok{$}
    \FunctionTok{cal\_diff}\NormalTok{(}\AttributeTok{group =} \StringTok{"Group"}\NormalTok{, }\AttributeTok{method =} \StringTok{"KW\_dunn"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{plot\_diff}\NormalTok{(}\AttributeTok{measure =} \StringTok{"pH"}\NormalTok{, }\AttributeTok{add\_sig =}\NormalTok{ T)}


\NormalTok{t1 }\OtherTok{\textless{}{-}}\NormalTok{ trans\_env}\SpecialCharTok{$}
    \FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{add\_data =}\NormalTok{ env\_data\_16S[, }\DecValTok{4}\SpecialCharTok{:}\DecValTok{11}\NormalTok{])}\SpecialCharTok{$}
    \FunctionTok{cal\_ordination}\NormalTok{(}\AttributeTok{method =} \StringTok{"dbRDA"}\NormalTok{, }\AttributeTok{use\_measure =} \StringTok{"bray"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{trans\_ordination}\NormalTok{(}\AttributeTok{adjust\_arrow\_length =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{max\_perc\_env =} \FloatTok{1.5}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_ordination\_anova}\NormalTok{()}
...
\NormalTok{trans\_env}\SpecialCharTok{$}
    \FunctionTok{new}\NormalTok{(}\AttributeTok{dataset =}\NormalTok{ dataset, }\AttributeTok{add\_data =}\NormalTok{ env\_data\_16S[, }\DecValTok{4}\SpecialCharTok{:}\DecValTok{11}\NormalTok{])}\SpecialCharTok{$}
    \FunctionTok{cal\_cor}\NormalTok{(}\AttributeTok{use\_data =} \StringTok{"Genus"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{plot\_cor}\NormalTok{()}


\CommentTok{\# trans\_func}

\FunctionTok{library}\NormalTok{(magrittr)}
\FunctionTok{library}\NormalTok{(microeco)}
\FunctionTok{data}\NormalTok{(dataset)}

\NormalTok{trans\_func}\SpecialCharTok{$}
    \FunctionTok{new}\NormalTok{(dataset)}\SpecialCharTok{$}
    \FunctionTok{cal\_spe\_func}\NormalTok{(}\AttributeTok{prok\_database =} \StringTok{"FAPROTAX"}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{cal\_spe\_func\_perc}\NormalTok{(}\AttributeTok{abundance\_weighted =} \ConstantTok{FALSE}\NormalTok{)}\SpecialCharTok{$}
    \FunctionTok{plot\_spe\_func\_perc}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
