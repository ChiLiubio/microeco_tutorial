...
\hypersetup{
  pdftitle={Tutorial for R microeco package (v0.6.0)},
  pdfauthor={Chi Liu, Felipe R. P. Mansoldo, Umer Zeeshan Ijaz, Chenhao Li, Yang Cao, Jarrod J. Scott, Yaoming Cui, Alane B. Vermelho, Xiangzhen Li, Minjie Yao},
  hidelinks,
...
\title{Tutorial for R microeco package (v0.6.0)}
\author{Chi Liu, Felipe R. P. Mansoldo, Umer Zeeshan Ijaz, Chenhao Li, Yang Cao, Jarrod J. Scott, Yaoming Cui, Alane B. Vermelho, Xiangzhen Li, Minjie Yao}
\date{2021-12-09}
...
Besides, we also develop the file2meco package (\url{https://github.com/ChiLiubio/file2meco}) for the data input from some tools easily
...
Some new methods are also in active development.
...
If you are interested in the class feature, read more from `Advanced R' book (\url{https://adv-r.hadley.nz/}).
...
\item
  install the packages in advance. We recommend this solution if you are interest in most of the methods in the microeco package and want to run all the examples in tutorial.
\end{enumerate}
...
Plotting the cladogram from the LEfSe result requires the ggtree package in bioconductor (\url{https://bioconductor.org/packages/release/bioc/html/ggtree.html}).
...
i.e.~network.gexf in the \href{https://chiliubio.github.io/microeco_tutorial/extended-class.html\#trans_network-class}{tutorial}.
...
The only thing need to do is to download the blast tool (\textbf{ignore this if the blast tool is in the path}) and Ref99NR/Ref100NR database (select one).
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# seqinr should be installed for reading and writing fasta file}
...
　Many tools can be used for the bioinformatic analysis, such as QIIME \citep{Caporaso_QIIME_2010}, QIIME2 \citep{Bolyen_Reproducible_2019},
...
\textbf{The colnames in otu\_table must be in accordance with rownames of sample\_table}.
Otherwise, the following check can filter all the samples of otu\_table because of no overlap of sample names between otu\_table and sample\_table.
...
It's worth noting that the cal\_abund() function can be used to \textbf{solve more complex cases with special parameters},
...
\item
  cal\_abund(): powerful and flexible to cope with complex cases in tax\_table
\item
...
　The trans\_abund class is used to transform taxonomic abundance data for plotting taxa abundance with the ggplot2 package.
...
\CommentTok{\# create trans\_venn object, see other available options in ratio}
...
　The trans\_beta class is developed for the beta diversity analysis.
When needed, the beta\_diversity in microtable table will be invoked for transformation and ploting using trans\_beta class.
...
\item
  plot\_group\_distance(): pair\_compare\_filter\_select parameter in plot\_group\_distance can be used to select the significance label shown in the plot
\end{itemize}
...
\CommentTok{\# use a loop for more samples}
\end{Highlighting}
\end{Shaded}
...
\item
  cal\_rcbray(): if you just need rcbray, ignore other operations
\end{itemize}
...
We first identify/predict traits of taxa.
...
\CommentTok{\# see https://chiliubio.github.io/microeco\_tutorial/intro.html\#tax4fun2 for downloading ncbi{-}blast and Ref99NR/Ref100NR}
\NormalTok{t1}\SpecialCharTok{$}\FunctionTok{cal\_tax4fun2}\NormalTok{(}\AttributeTok{blast\_tool\_path =} \StringTok{"ncbi{-}blast{-}2.5.0+/bin"}\NormalTok{, }\AttributeTok{path\_to\_reference\_data =} \StringTok{"test\_ReferenceData\_v2"}\NormalTok{,}
...
\tightlist
\item
  blast\_tool\_path parameter in cal\_tax4fun2: if the blast tool has been in `environment variable' of computer, use blast\_tool\_path = NULL
\item
...
\CommentTok{\# revise the species names in tax\_table}
...
\includegraphics{microeco-tutorial_files/figure-latex/unnamed-chunk-195-1.pdf}
...
\includegraphics{microeco-tutorial_files/figure-latex/unnamed-chunk-196-1.pdf}
...
As an R6 object is an encapsulated environment, there is also another very useful advantage, i.e.~the data save and communication.
