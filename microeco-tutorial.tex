...
\hypersetup{
  pdftitle={Tutorial for R microeco package (v0.7.1)},
...
\title{Tutorial for R microeco package (v0.7.1)}
...
as we show in the figure, e.g., function \texttt{dataset\$cal\_alphadiv()} and its returned result \texttt{dataset\$alpha\_diversity}.
...
\begin{center}\includegraphics{Images/plot_bar_allu} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{packages }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\StringTok{"Boruta"}\NormalTok{, }\StringTok{"parallel"}\NormalTok{, }\StringTok{"rsample"}\NormalTok{, }\StringTok{"randomForest"}\NormalTok{, }\StringTok{"caret"}\NormalTok{, }\StringTok{"gridExtra"}\NormalTok{, }\StringTok{"ROCR"}\NormalTok{)}
\CommentTok{\# Now check or install}
\ControlFlowTok{for}\NormalTok{(x }\ControlFlowTok{in}\NormalTok{ packages)\{}
    \ControlFlowTok{if}\NormalTok{(}\SpecialCharTok{!}\FunctionTok{require}\NormalTok{(x, }\AttributeTok{character.only =} \ConstantTok{TRUE}\NormalTok{)) \{}
        \FunctionTok{install.packages}\NormalTok{(x, }\AttributeTok{dependencies =} \ConstantTok{TRUE}\NormalTok{)}
...
Using cal\_ROC and plot\_ROC can get the ROC curve.
...
we also perfom all the analysis without feature selection to show the results.
...
\CommentTok{\# no feature selection, others same with previous steps}
...
In the microtable class part, we show the basic use of the creating of microtable object with the example data.
Actually, constructing the basic microtable object from other tools/platforms (e.g.~QIIME, QIIME2, HUMAnN, Kraken2 and phyloseq)
...
\includegraphics{microeco-tutorial_files/figure-latex/unnamed-chunk-235-1.pdf}
...
\includegraphics{microeco-tutorial_files/figure-latex/unnamed-chunk-236-1.pdf}
